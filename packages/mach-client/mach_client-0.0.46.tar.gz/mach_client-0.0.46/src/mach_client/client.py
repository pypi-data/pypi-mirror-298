...
class MachClient:
    ...
    def submit_order(self, chain: ChainId, place_taker_tx: HexBytes) -> Order:
        data = {
            "chain": str(chain),
    ...
    def get_all_points(self, limit: int = 10) -> list[WalletPoints]:
        data = {
            "limit": limit,
        }

        response = self.session.get(MachClient.points, params=data)
    ...
    def get_points(self, wallet: ChecksumAddress) -> int:
        data: dict = {}

        response = self.session.get(MachClient.points + f"/{wallet}", params=data)

        return int(self._handle_response(response)["points"])
