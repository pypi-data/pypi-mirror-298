...
class C106(Corps):
    ...
    @classmethod
    def make_like_c106(cls, code: str, page: str, title: str) -> dict:
        redis_name = code + '.c106' + '_make_like_c106_' + page + '_' + title
        try:
            cached_data = cls.redis_client.get(redis_name).decode('utf-8')
        except AttributeError:
            # redis에 해당하는 값이 없는 경우
            data = mymongo.C106.make_like_c106(code, page, title)
            # print(data)
            if data:
                # 데이터를 Redis에 캐싱
                cls.redis_client.set(redis_name, json.dumps(data))
                # 60분후 키가 자동으로 제거됨 - c103468 업데이트할때 제거되도록..
                # Base.redis_client.expire(redis_name, 3600)
            print("Mongo에서 데이터 가져오기 - make_like_c106")
            return data
        else:
            print("Redis 캐시에서 데이터 가져오기 - make_like_c106")
            return json.loads(cached_data)
    ...
    def get_rivals(self) -> list:
        ...
        try:
            ...
            if data:
                ...
                # 60분후 키가 자동으로 제거됨 - c103468 업데이트할때 제거되도록..
                # Base.redis_client.expire(redis_name, 3600)
            print("Mongo에서 데이터 가져오기 - get_rivals")
            return data
        else:
            print("Redis 캐시에서 데이터 가져오기 - get_rivals")
            return json.loads(cached_data)
