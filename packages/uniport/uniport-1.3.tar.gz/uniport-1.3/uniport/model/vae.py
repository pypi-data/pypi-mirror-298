                    ...
                    for i, (x, y, idx) in tk0:
                        ...
                        if label_weight is None:
                            ...
                            for j, weight in enumerate(label_weight):
                                ...
                                if len(loc[j])>0:
                                    if weight is None:
                                        recon_loss += 1/self.n_domain * loss_func(recon_x_c[loc[j]], x_c[loc[j]]) * 2000                            
                                    else:
                                        weight = weight.to(device)
                                        recon_loss += 1/self.n_domain * F.binary_cross_entropy(recon_x_c[loc[j]], x_c[loc[j]], weight=weight[idx[j]]) * 2000
