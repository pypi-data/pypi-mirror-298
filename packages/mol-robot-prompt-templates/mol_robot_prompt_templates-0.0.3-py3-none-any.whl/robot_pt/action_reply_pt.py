...
def action_reply_pt(state_chunks: str) -> str:
    ...
    instruction_template = textwrap.dedent('''
        Generate a vocal reply to fullfill the user's request or answer the user's question based on contextual information in the robot world state:
        {}
        Reply to the user in a brief, concise, and affectionate conversational manner. Generally the robot pursues conversation with the user but remains quiet unless the user is speaking with the robot. Reply only in a single sentence if possible.
    ''')
