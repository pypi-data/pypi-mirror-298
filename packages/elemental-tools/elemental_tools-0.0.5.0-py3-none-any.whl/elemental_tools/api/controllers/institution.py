...
from elemental_tools.api.config import db_url, log_path, database_name
...
class InstitutionController:
	...
	def update(self, selector, content):
		self.logger('info', f'Updating Institution Information for Selector: {selector} with: {str(content)} ')
		_update_result = None
		...
		try:
			...
			self.logger('error', f'Cannot update institution information: {str(e)}')
