# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from payi import Payi, AsyncPayi
from tests.utils import assert_matches_type
from payi.types.shared import EvaluationResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestRequests:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: Payi) -> None:
        request = client.evaluations.requests.create(
            request_id="request_id",
            evaluation=0,
        )
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: Payi) -> None:
        request = client.evaluations.requests.create(
            request_id="request_id",
            evaluation=0,
            user_id="user_id",
        )
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: Payi) -> None:
        response = client.evaluations.requests.with_raw_response.create(
            request_id="request_id",
            evaluation=0,
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        request = response.parse()
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Payi) -> None:
        with client.evaluations.requests.with_streaming_response.create(
            request_id="request_id",
            evaluation=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            request = response.parse()
            assert_matches_type(EvaluationResponse, request, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_create(self, client: Payi) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `request_id` but received ''"):
            client.evaluations.requests.with_raw_response.create(
                request_id="",
                evaluation=0,
            )


class TestAsyncRequests:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_create(self, async_client: AsyncPayi) -> None:
        request = await async_client.evaluations.requests.create(
            request_id="request_id",
            evaluation=0,
        )
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncPayi) -> None:
        request = await async_client.evaluations.requests.create(
            request_id="request_id",
            evaluation=0,
            user_id="user_id",
        )
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncPayi) -> None:
        response = await async_client.evaluations.requests.with_raw_response.create(
            request_id="request_id",
            evaluation=0,
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        request = await response.parse()
        assert_matches_type(EvaluationResponse, request, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncPayi) -> None:
        async with async_client.evaluations.requests.with_streaming_response.create(
            request_id="request_id",
            evaluation=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            request = await response.parse()
            assert_matches_type(EvaluationResponse, request, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_create(self, async_client: AsyncPayi) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `request_id` but received ''"):
            await async_client.evaluations.requests.with_raw_response.create(
                request_id="",
                evaluation=0,
            )
