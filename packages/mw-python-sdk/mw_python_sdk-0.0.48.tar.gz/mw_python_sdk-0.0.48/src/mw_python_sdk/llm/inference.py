    ...
    if model_id == "_echo_":
        ...
        subprocess.call(
            [
                "bash",
                "-c",
                f"python -m vllm.entrypoints.openai.api_server --max-model-len={max_model_len}  --served-model-name={model_name} --dtype={dtype} --model '{model_dir}' --host {host} --port {port}",
            ]
        )
