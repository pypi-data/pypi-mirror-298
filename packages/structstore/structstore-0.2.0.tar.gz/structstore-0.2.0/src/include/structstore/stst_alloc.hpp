...
public:
    ...
    void assert_owned(const void* ptr) const {
            ...
            try_with_info("pointer " << ptr << " in [" << (void*) buffer << " to " << (void*) (buffer + blocksize) << "]", 
        if (ptr == nullptr) {
            throw std::runtime_error("pointer is NULL");
        }
        if (ptr < buffer || ptr >= buffer + blocksize) {
            throw std::runtime_error("not inside managed memory");
        });
    }
};
...
public:
    ...
    T* allocate(std::size_t n) {
        T* ptr = static_cast<T*>(mm_alloc.allocate(n * sizeof(T)));
        // std::cout << "allocating " << n << " of type " << typeid(T).name() << " at " << ptr << std::endl;
        return ptr;
    }

    void deallocate(T* p, std::size_t s) {
        // std::cout << "deallocating type " << typeid(T).name() << " of size " << s << " at " << p << std::endl;
