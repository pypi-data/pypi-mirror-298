...
from malevich_app.export.secondary.helpers import get_collection_pandas, get_df_object, get_collection_by_id, get_collection_json, \
    DocsRaw, VariantList
...
class JuliusCollectionHolder:   # TODO gc
    ...
    async def interpret(self, julius_app, collections: List[Tuple[Union[Tuple[Collection, ...], List[Tuple[Collection, ...]]], Callable[[VarArg(pd.DataFrame)], JDF], Optional[Tuple[Optional[str], ...]]]], cfg: Cfg) -> List[JDF]:
        async def __get_df(collection: Collection, scheme: Union[Optional[str], Tuple[Optional[str]]]) -> Union[Tuple[Union[pd.DataFrame, List[pd.DataFrame]], Collection], List[Tuple[Union[pd.DataFrame, List[pd.DataFrame]], Collection]], VariantList]:
            ...
            if isinstance(collection, MongoCollection):
                ...
                for coll, scheme_i in zip(colls, scheme):
                    df_i, _ = await __get_df(coll, scheme_i)
                    df.append(df_i)
                coll = collection
                ...
                for coll, scheme_i in zip(colls, scheme):
                    res.append(await __get_df(coll, scheme_i))
