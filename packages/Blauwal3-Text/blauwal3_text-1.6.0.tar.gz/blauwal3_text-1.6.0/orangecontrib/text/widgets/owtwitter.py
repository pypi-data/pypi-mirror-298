...
class OWTwitter(OWWidget, ConcurrentWidgetMixin):
    ...
    class Info(OWWidget.Information):
        nut_enough_tweets = Msg(__("msg.not_enough_tweets"))

    class Error(OWWidget.Error):
        api_error = Msg(__("msg.api_error"))
        empty_query = Msg(__("msg.empty_query"))
        key_missing = Msg(__("msg.key_missing"))
        wrong_author = Msg(__("msg.wrong_authors"))
    ...
    class APICredentialsDialog(OWWidget):
        ...
        def __init__(self, parent):
            super().__init__()
            self.cm_key = CredentialManager(__("btn_twitter_bearer_token"))
            self.parent = parent

            box = gui.vBox(self.controlArea, __("btn_bearer_token"))
            self.key_edit = QPlainTextEdit()
            box.layout().addWidget(self.key_edit)

            self.submit_button = gui.button(self.buttonsArea, self, __("dialog.btn_ok"), self.accept)
