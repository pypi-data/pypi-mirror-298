...
def __(key):
    return i18n.t('text.owannotator.' + key)
...
class OWAnnotator(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Warning(OWDataProjectionWidget.Warning):
        same_axis_features = Msg(__("msg.selected_features"))

    class Error(OWDataProjectionWidget.Error):
        no_continuous_vars = Msg(__("msg.no_continuous_vars"))
        not_enough_inst = Msg(__("msg.not_enough_inst"))
        proj_error = Msg(__("msg.proj_error"))
    ...
    def _add_controls(self):
        ...
        super()._add_controls()
        self.gui.add_control(self._effects_box, gui.hSlider, __("btn.cluster_label"),
        ...
        gui.checkBox(self._plot_box, self.graph, "show_cluster_hull",
                     __("btn.show_cluster_hull"), callback=self.graph.update_clusters)
    ...
    def __add_annotation_controls(self):
        ...
        box = gui.vBox(self.controlArea, box=__("btn.annotation"))
        ...
        gui.checkBox(ibox, self, "use_epsilon", __("btn.epsilon"),  labelWidth=80,
        ...
        gui.appendRadioButton(rbuttons, __("btn.gaussian_mixture_models"))
        self.gmm_box = ibox = gui.indentedBox(rbuttons, 20,
                                              orientation=Qt.Horizontal)
        gui.checkBox(ibox, self, "use_n_components", __("btn.clusters"), labelWidth=80,
        ...
        gui.appendRadioButton(rbuttons, __("btn.from_variable"))
        ...
        gui.doubleSpin(box, self, "p_threshold", 0, 1, 0.01,
                       label=__("btn.fdr"), labelWidth=100,
    ...
    def _run(self):
        ...
        if not self.valid_data.all():
            self.Error.proj_error(__("msg.no_valid_embedding"))
            return
