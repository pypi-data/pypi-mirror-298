...
class NYT:
    ...
    metas = [
        (
            partial(StringVariable, __('headline')),
            lambda doc: doc.get("headline", {}).get("main") or "",
        ),
        (partial(StringVariable, __('abstract')), lambda doc: doc.get("abstract") or ""),
        (partial(StringVariable, __('snippet')), lambda doc: doc.get("snippet") or ""),
        (
            partial(StringVariable, __('lead_paragraph')),
            lambda doc: doc.get("lead_paragraph") or "",
        ),
        (
            partial(StringVariable, __('subject_keywords')),
            partial(keywords, name="subject"),
        ),
        (partial(StringVariable, "URL"), lambda doc: doc.get("web_url") or ""),
        (
            partial(StringVariable, __('locations')),
            partial(keywords, name="glocations"),
        ),
        (partial(StringVariable, __('persons')), partial(keywords, name="persons")),
        (
            partial(StringVariable, __('organizations')),
            partial(keywords, name="organizations"),
        ),
        (
            partial(StringVariable, __('creative_works')),
            partial(keywords, name="creative_works"),
        ),
        (
            partial(TimeVariable, i18n.t('text.guardian.publication_date'), have_time=1, have_date=1),
            parse_date,
        ),
        (
            partial(DiscreteVariable, __('article_type')),
            lambda doc: doc.get("type_of_material", None),
        ),
        (
            partial(ContinuousVariable, __('word_count'), number_of_decimals=0),
            lambda doc: doc.get("word_count", None),
        ),
    ]

    text_features = [__('headline'), __('abstract')]
