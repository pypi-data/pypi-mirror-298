...
class MockEntrez:
    ...
    def epost(self, db, **keywds):
        self.list = keywds["id"].split(",")
        return MockEntrezHandle()

    def parse(self, handle):
        return [x for x in self.mock_responses.get("parse") if x["PMID"] in self.list]
...
CACHE = os.path.join(os.path.dirname(__file__), "data", "pubmed-cache.txt")
...
class PubmedTests(unittest.TestCase):
    ...
    def __empty_cache(self):
        try:
            os.remove(self.pubmed.cache_path + ".db")
        except OSError:
            pass
    ...
    def test_date_to_iso(self):
        ...
        self.assertWarns(RuntimeWarning, _date_to_iso, unexpected_input)
    ...
    def test_pubmed_retrieve_record_batch(self):
        ...
        self.pubmed._search_for_records(
            test_terms, authors, pub_date_start, pub_date_end
        )

        # Retrieve the records.
        data = self.pubmed._retrieve_record_batch(offset, num_requested_records)
    ...
    def test_pubmed_retrieve_records(self):
        ...
        meta_fields = sorted(
            [field_name for field_name, field_tag in PUBMED_TEXT_FIELDS]
        )
        test_meta_fields = sorted([m.name for m in corpus.domain.metas])
    ...
    @patch("Bio.Medline.parse", mock_entrez.parse)
    @patch("Bio.Entrez.epost", mock_entrez.epost)
    def test_pubmed_retrieve_records_cache_and_new_records(self):
        """Test retrieving combination of cached and new instances"""
        test_terms = ["orchid"]
        authors = []
        pub_date_start = "2011/07/07"
        pub_date_end = "2014/07/07"
        ...
        # retrieve records - they also get cached
        self.assertEqual(len(self.pubmed._retrieve_records(3)), 3)
        # retrieve more - three are cached and two are new
        self.assertEqual(len(self.pubmed._retrieve_records(5)), 5)
    ...
    def test_pubmed_retrieve_records_no_cache(self):
        ...
        # Retrieve the records and build a corpus.
        corpus = self.pubmed._retrieve_records(num_records, use_cache=False)
        self.assertEqual(len(corpus), num_records)
    ...
    def test_download_records(self):
        ...
        corpus = self.pubmed.download_records(
            test_terms, authors, pub_date_start, pub_date_end, num_records
        )
        self.assertEqual(len(corpus), num_records)
    ...
    def test_pubmed_retrieve_record_batch_exception(self):
        ...
        self.assertWarns(
            RuntimeWarning, self.pubmed._retrieve_records, num_records, use_cache=False
        )
