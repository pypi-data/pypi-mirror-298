...
def __(key):
    return i18n.t('text.owldavis.' + key)
...
class OWLDAvis(OWWidget):
    ...
    def _create_layout(self):
        self._add_graph()
        box = gui.widgetBox(self.controlArea, __("relevance"))
    ...
    def compute_distributions(data: Topics) -> np.ndarray:
        ...
        """
        topic_frequency = data.get_column(__("mtp"))
    ...
    def set_data(self, data: Optional[Topics]):
        ...
        self.data = Table.transpose(data, __("topics"), __("words"))
        self.topic_list = [var.name for var in self.data.domain.attributes]
        self.num_tokens = data.attributes.get(__("token_num"), "")
