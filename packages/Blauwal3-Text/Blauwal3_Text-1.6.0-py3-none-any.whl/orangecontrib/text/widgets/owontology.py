...
def __(key):
    return i18n.t('text.owontology.' + key)
...
class EditableTreeView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        action = QAction("+", self, toolTip=__("tips.add"))
        action.triggered.connect(self.__on_add)
        actions_widget.addAction(action)

        action = QAction("\N{MINUS SIGN}", self, toolTip=__("tips.remove"))
        action.triggered.connect(self.__on_remove)
        actions_widget.addAction(action)

        action = QAction("\N{MINUS SIGN}R", self,
                         toolTip=__("tips.remove_recursive"))
        action.triggered.connect(self.__on_remove_recursive)
        actions_widget.addAction(action)

        gui.rubber(actions_widget)

        self.__undo_action = action = QAction(__("tips.undo"), self, toolTip=__("tips.undo"))
        action.triggered.connect(self.__on_undo)
        actions_widget.addAction(action)

        self.__redo_action = action = QAction(__("tips.redo"), self, toolTip=__("tips.redo"))
...
def _read_from_source(filename: str) -> Ontology:
    ...
    if ext == ".json":
        ...
        raise NotImplementedError(__("msg.no_readers").format(name=name))
...
class OWOntology(OWWidget, ConcurrentWidgetMixin):
    ...
    RUN_BUTTON, INC_BUTTON =  __("run_button"), __("inc_button")
    ...
    class Warning(OWWidget.Warning):
        no_words_column = __("msg.no_words_column")
        skipped_words = __("msg.skipped_words")
    ...
    def _setup_gui(self):
        # control area
        library_box: QGroupBox = gui.vBox(self.controlArea, __("library"))
        ...
        tool_tip = __("tips.add_ontology")
        action = QAction("+", self, toolTip=tool_tip)
        action.triggered.connect(self.__on_add)
        actions_widget.addAction(action)

        tool_tip = __("tips.remove_ontology")
        action = QAction("\N{MINUS SIGN}", self, toolTip=tool_tip)
        action.triggered.connect(self.__on_remove)
        actions_widget.addAction(action)

        tool_tip = __("tips.save_change")
        action = QAction(__("update"), self, toolTip=tool_tip)
        action.triggered.connect(self.__on_update)
        actions_widget.addAction(action)

        gui.rubber(actions_widget)

        action = QAction(__("more"), self, toolTip="More actions")

        new_from_file = QAction(__("import_file"), self)
        new_from_file.triggered.connect(self.__on_import_file)

        new_from_url = QAction(__("import_url"), self)
        new_from_url.triggered.connect(self.__on_import_url)

        save_to_file = QAction(__("save_file"), self)
        ...
        input_box: QGroupBox = gui.vBox(self.controlArea, __("input"))
        ...
        self.__inc_button = gui.button(
            ...
            toolTip=__("tips.include_selected_words"),
        ...
        gui.checkBox(
            self.controlArea, self, "include_children", __("include_subtree"),
            box=__("output"), callback=self.commit.deferred
        )
        box = gui.vBox(self.controlArea, __("ontology_info"))
    ...
    def __update_score(self):
        ...
        self.ontology_info = __("score_label").format(score=score)
