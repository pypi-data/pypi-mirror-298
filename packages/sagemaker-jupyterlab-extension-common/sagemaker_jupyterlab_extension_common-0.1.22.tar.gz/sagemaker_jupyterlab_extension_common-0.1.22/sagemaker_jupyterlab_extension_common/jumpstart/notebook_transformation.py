...
def _get_substitue_cell(
    model_id: str, current_cell: dict, cluster_id: Optional[str] = None
) -> dict:
    ...
    if current_alteration == JumpStartModelNotebookAlterationType.modelIdVersion.value:
        ...
        current_cell["source"] = [f'cluster_id = "{cluster_id}"']
    return current_cell
...
def update_notebook(
    ...
    if replace_alterations:
        if modelId:
            nb["cells"] = [
                (
                    _get_substitue_cell(modelId, cell)
            ...
            nb["cells"] = [
                (
                    _get_substitue_cell("", cell, clusterId)
                    if cell["cell_type"] == "code"
                    else cell
                )
                for cell in nb["cells"]
...
class HyperpodNotebook(Notebook):
    def transform(self, notebook: str, clusterId: str) -> str:
        alterations = [
            JumpStartModelNotebookAlterationType.clusterId,
        ]
        options = UpdateHubNotebookUpdateOptions([], alterations, [])
        notebook = update_notebook(notebook, None, options, clusterId)
        return notebook
