...
class SACBrain(Brain):
    ...
    * Target Entropy Annealing for SAC — https://arxiv.org/abs/2112.02852
    ...
    def __init__(
        ...
        self._step = 0
        self._muscle_initialized = False
    ...
    def thinking(self, muscle_id, data_from_muscle):
        ...
        update = None
        self._step += 1
        if not self._muscle_initialized:
            update = io.BytesIO()
            T.save(self.actor, update)
            update.seek(0)

        if data_from_muscle is None:  # Okay, happens during initialization
            assert not self._muscle_initialized
            self._muscle_initialized = True
            return update
    ...
    def load(self):
        ...
        self.actor = T.load(actor_dump, map_location=self._device)
        self.critic = T.load(critic_dump, map_location=self._device)
        self.actor_target = T.load(
            actor_target_dump, map_location=self._device
        )
        self.critic_target = T.load(
            critic_target_dump, map_location=self._device
        )
