...
def set_setpoint_to_actuator(setpoint, actuator: ActuatorInformation):
    """Clip actions to make sure they are not higher
    than max or lower than min of allowed action space."""
...
class PPOMuscle(Muscle):
    ...
    def prepare_model(self) -> None:
        ...
        if actor_bio is not None and critic_bio is not None:
            try:
                self.actor = torch.load(actor_bio, map_location=self._device)
                if self.actor is not None:
                    self.actor.to(self._device)

                self.critic = torch.load(critic_bio, map_location=self._device)
                if self.actor is not None:
