...
class Client:
    ...
    class YouTube:
        def get_video(self, url, video_format="bestvideo", audio_format="bestaudio"):
            return self.send_task.YouTube.get_video(url=url, video_format=video_format, audio_format=audio_format).get_result()
        
        def get_audio(self, url, audio_format="bestaudio"):
            return self.send_task.YouTube.get_audio(url=url, audio_format=audio_format).get_result()
        
        def get_live_video(self, url, duration, start=0, video_format="bestvideo", audio_format="bestaudio"):
            return self.send_task.YouTube.get_live_video(url=url, start=start, duration=duration, video_format=video_format, audio_format=audio_format).get_result()
        
        def get_live_audio(self, url, duration, start=0, audio_format="bestaudio"):
            return self.send_task.YouTube.get_live_audio(url=url, start=start, duration=duration, audio_format=audio_format).get_result()
        
        def get_info(self, url):
            return self.send_task.YouTube.get_info(url=url).get_result()
        
    class Spotify:
        def get_track(self, url):
            return self.send_task.Spotify.get_track(url=url).get_result()
        
        def get_info(self, url):
            return self.send_task.Spotify.get_info(url=url).get_result()
