...
class YamlWriter:
    def __init__(
        ...
        self.data = aac_yaml.load_yaml_files(data_paths)
    ...
    def write_1(self, templates_path: str, output_path: str, device_name: str) -> None:
        ...
        for dir, _, files in os.walk(templates_path):
            log_path = os.path.join(output_path, folder_name, device_name)
            if files:
                try:
                    for filename in files:
                        if ".j2" not in filename:
                        ...
                        self.logger.debug(
                            "Generate working file successfully: {}".format(log_path)
                        )
                except Exception as e:
                    self.logger.error(
                        "Generate working file failed: {}".format(log_path)
                    )
                    self.logger.error("Error: {}".format(e))
                    exit()
            else:
