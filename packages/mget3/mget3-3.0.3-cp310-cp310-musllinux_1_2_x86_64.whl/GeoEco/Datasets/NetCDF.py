...
class NetCDFVariable(Grid):
    ...
    def _GetLazyPropertyPhysicalValue(self, name):
        ...
        if self.GetLazyPropertyValue('ScalingFunction', allowPhysicalValue=False) is None:
            ...
            if scale_factor != 1. or add_offset != 0.:
                ...
                scalingFunction = lambda data: numpy.cast[scaledDataType](scale_factor*data + add_offset)
                ...
                self.SetLazyPropertyValue('UnscalingFunction', lambda data: numpy.cast[v.dtype.name](numpy.round((data - add_offset)/scale_factor)))
    ...
    def _ReadNumpyArray(self, sliceList):
        ...
        try:
            return v.__getitem__(tuple(sliceList)), self.GetLazyPropertyValue('UnscaledNoDataValue')
        except Exception as e:
