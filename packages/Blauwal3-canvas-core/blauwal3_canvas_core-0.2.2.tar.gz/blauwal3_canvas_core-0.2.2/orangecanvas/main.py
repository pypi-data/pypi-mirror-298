...
from .registry import WidgetRegistry, set_global_registry, set_style_sheet
...
class Main:
    ...
    def run_discovery(self) -> WidgetRegistry:
        ...
        handler.found_category.connect(
            lambda cd: self.show_splash_message(cd.label)
        )
        ...
        with ExitStack() as stack:
            ...
            def record_path(url: QUrl):
                if not url.isEmpty():
                    ...
                    if result.get('token'):
                        from orangecontrib.blue_whale.canvasmain import set_session
                        set_session({'SESSION': result.get('token', None)}, message=i18n.t('canvas.msg.sign_out'))
