...
from AnyQt.QtWidgets import (
    ...
    QToolBar, QAction, QStackedWidget, QVBoxLayout, QHBoxLayout,QPushButton, QMessageBox, QApplication,
...
from AnyQt.QtCore import (
    ...
    Property,QCoreApplication,QProcess,Signal,QTimer)
...
class UserSettingsDialog(QMainWindow):
    ...
    def __setupUi(self):
        ...
        self.bind(tab, "selectedLanguages_", "application-style/languages")
        ...
        for cat in reg.categories():
            if cat is None or not cat.label:  # 检查 cat 是否为空或 label 为空
                continue
            item = QStandardItem()
...
class StyleConfigWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.languages_cb = languages_cb = QComboBox(objectName="language-cb")
        languages_cb.addItem(__("option_settings.zh"), userData="zh")
        languages_cb.addItem(__("option_settings.en"), userData="en")
        languages_cb.addItem(__("option_settings.de"), userData="de")

        form.addRow(__("option_settings.style"), style_cb)
        form.addRow(__("option_settings.color_theme"), colors_cb)
        form.addRow(__("option_settings.language_settings"), languages_cb)
        # 添加重启按钮
        self.restart_button = QPushButton(__("option_settings.restart_button"))
        self.restart_button.clicked.connect(self._show_restart_message)
        form.addRow(self.restart_button)
        label = QLabel(
        ...
        languages_cb.currentIndexChanged.connect(self.selectedLanguagesChanged)
    ...
    def selectedLanguages(self) -> str:
        """Return the current selected languages key."""
        key = self.languages_cb.currentData()
        return key if key is not None else ""

    def setSelectedLanguages(self, languages: str) -> None:
        """Set the current selected languages key."""
        idx = self.languages_cb.findData(languages, Qt.UserRole, Qt.MatchFixedString)
        if idx == -1:
            idx = 0  # select the default languages
        self.languages_cb.setCurrentIndex(idx)
        
 
    selectedLanguagesChanged = Signal()
    selectedLanguages_ = Property(
        str, selectedLanguages, setSelectedLanguages,
    ...
    def _show_restart_message(self):
        ...
        mb = QMessageBox(
            self,
            windowTitle=__("option_settings.restart_title"),
            text=__("option_settings.restart_message"),
        ...
        if res == QMessageBox.Ok:
            ...
            def restart():
                ...
                if any(w.isVisible() for w in windows):  # 如果有窗口关闭被取消
                    QApplication.setQuitOnLastWindowClosed(quit_temp_val)
                    QMessageBox(
                        text=i18n.t("widget.mainwindow.text_restart_cancel"),
                        informativeText=i18n.t("widget.mainwindow.reset_settings").format(name),
                        icon=QMessageBox.Information
                    ).exec()
                else:
