...
def string_eval(source):
    ...
    if not _is_constant(body, (str,)):
        raise ValueError(__("err_tip.not_string_literal") % source)
    return body.value
...
def tuple_eval(source):
    ...
    if not isinstance(node.body, ast.Tuple):
        raise ValueError(__("err_tip.tuple") % source)
    ...
    if not all(_is_constant(el, (str, float, complex, int)) or
        ...
        raise ValueError(__("err_tip.container_numstr"))
...
def _terminal_value(node):
    ...
    if _is_constant(node, (str, bytes, int, float, complex, type(None))):
        return node.value
    raise ValueError(__("err_tip.not_erminal"))
...
def scheme_to_etree(scheme, data_format="literal", pickle_fallback=False):
    ...
    for node in scheme.nodes:  # type: SchemeNode
        ...
        try:
            resource = node.properties['recent_paths'][0].resource_id
        except:
            resource = ""
        ...
        if resource:
            ids.append(resource) if resource not in ids else ids
    ...
    if ids:
        resource_id = {"resource_id": str(ids)}
        builder.start('resource_id', resource_id)
        builder.end('resource_id')
...
def dumps(obj, format="literal", prettyprint=False, pickle_fallback=False):
    ...
    if format == "literal":
        ...
        raise ValueError(__("err_tip.unsupported") % format)
    ...
    if pickle_fallback:
        ...
        raise Exception(__("err_tip.strange"))
...
def loads(string, format):
    if format == "literal":
        ...
        raise ValueError(__("noun.unknown"))
...
def literal_dumps(obj, indent=None, relaxed_types=True):
    ...
    def check(obj):
        ...
        if id(obj) in memo:
            raise ValueError(__("err_tip.recurisve").format(obj))
        ...
        if type(obj) in builtins_seq:
            ...
            raise TypeError(__("err_tip.noserialzed").format(type(obj)))
