...
class DrugRepurposer:
    def __init__(self, model_path, model2_path):
        ...
        """
        self.model = torch.load(model_path)  # Load the pre-trained embedding model
        self.model2 = torch.load(model2_path)  # Load the pre-trained knowledge graph model training data

    def repurpose_by_drug_ids(self, drugs=['DB09052', 'DB05889', 'DB13881', 'DB08870', 'DB08901'], relation="DRUG_DISEASE_ASSOCIATION"):
        """
    ...
    def main(self):
        """
        Perform drug repurposing and save results to CSV.
        """
        # Repurpose drugs for given drug IDs
        drug_id_drugs = self.repurpose_by_drug_ids()
        print(drug_id_drugs)
        
        # Save results to CSV
        drug_id_drugs.to_csv('example.tsv', index=False, sep='\t')

if __name__ == "__main__":
    repurposer = DrugRepurposer('C:/Users/bpatton23/Downloads/Drugrepurposing (1)/Drugrepurposing/complex_model_gene/trained_model.pkl',
