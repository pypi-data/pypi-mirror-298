...
class CommitType:
    ...
    def __str__(self: CommitType) -> str:
        return self.type

    def __hash__(self: CommitType) -> str:
        return hash(self.type)

    def __eq__(self: CommitType, other) -> bool:
        return self._compare_key(other) == 0

    def __lt__(self: CommitType, other) -> bool:
        return self._compare_key(other) < 0

    def _compare_key(self: CommitType, other) -> int:
    ...
    @classmethod
    def from_dict(cls: CommitType, data: Dict) -> CommitType:
    ...
    @classmethod
    def from_list(cls: CommitType, lst: List[Dict]) -> List[CommitType]:
...
class CzEspressifConfig:
    ...
    @property
    def types(self: CzEspressifConfig) -> list[CommitType]:
        return CommitType.from_list(self.settings.get('types', TYPES))

    @property
    def extra_types(self: CzEspressifConfig) -> list[CommitType]:
        return CommitType.from_list(self.settings.get('extra_types', []))

    @cached_property
    def known_types(self: CzEspressifConfig) -> list[CommitType]:
        return self.types + self.extra_types

    @property
    def incremental(self: CzEspressifConfig) -> bool:
        return any(arg in sys.argv for arg in ['cz bump', 'bump', '--incremental'])

    @property
    def changelog_unreleased(self: CzEspressifConfig) -> bool:
        return self.settings.get('changelog_unreleased', True)

    @property
    def use_emoji(self: CzEspressifConfig) -> bool:
        return self.settings.get('use_emoji', True)

    @property
    def changelog_title(self: CzEspressifConfig) -> str:
        return self.settings.get('changelog_title', CHANGELOG_TITLE)

    @property
    def changelog_header(self: CzEspressifConfig) -> str:
        return self.settings.get('changelog_header', CHANGELOG_HEADER)

    @property
    def changelog_footer(self: CzEspressifConfig) -> str:
        return self.settings.get('changelog_footer', CHANGELOG_FOOTER)

    @property
    def changelog_section_line(self: CzEspressifConfig) -> bool:
        return self.settings.get('changelog_section_line', True)

    @property
    def changelog_show_commits(self: CzEspressifConfig) -> bool:
        return self.settings.get('changelog_show_commits', True)

    @property
    def changelog_show_authors(self: CzEspressifConfig) -> bool:
        return self.settings.get('changelog_show_authors', True)

    @property
    def release_notes_footer(self: CzEspressifConfig) -> str:
