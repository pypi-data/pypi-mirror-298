...
class FuncListCheckTask(TaskCaseTemplate, FuncListCheckPage):
    ...
    def she_zhi_view(self):
        ...
        super().open_ui_from_ui_list("设置", "SettingsPage", "FuncListSwipe", "创建怪物", "配置按需加载")
        super().close_ui("SettingsPage", "CloseSet")

    @put_task
    def hua_zhi_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("画质", "PictureQualityPage", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def reloadall_check(self):
        super().open_list("Func", "功能列表")
        super().click_ui_from_ui_list("reloadall", "FuncListSwipe", "创建怪物", "配置按需加载")

    @put_task
    def ji_neng_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("技能装配UI", "skillView", "FuncListSwipe", "创建怪物", "配置按需加载")
        super().click_ui("skillViewAttackKey")
        super().close_ui("skillView", "BackButton")

    def test_load_rawdata(self):
        super().open_list("Func", "功能列表")
        super().click_ui_from_ui_list("testloadrawdata", "FuncListSwipe", "创建怪物", "配置按需加载")

    @put_task
    def demo_ui_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("DemoUI", "demoUIView", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def sound_setting_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("soudsetingview", "MusicSettingInterface", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def check_for_updates(self):
        super().open_list("Func", "功能列表")
        super().click_ui_from_ui_list("checkForUpdates", "FuncListSwipe", "创建怪物", "配置按需加载")

    def shot_cut(self):
        super().open_list("Func", "功能列表")
        super().click_ui_from_ui_list("镜头切换", "FuncListSwipe", "创建怪物", "配置按需加载")
        super().click_ui_from_ui_list("镜头切换", "FuncListSwipe", "创建怪物", "配置按需加载")
        super().click_ui_from_ui_list("镜头切换", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def xie_yi_view(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("Proto", "测试发送", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def shi_pin(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("AVPro视频", "VideoView", "FuncListSwipe", "创建怪物", "配置按需加载")
        super().close_ui("VideoView", "VideoInterfaceCloseButton")

    def ce_shi_ju_qin1(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("测试剧情1", "TestPlotBlackCurtain1", "FuncListSwipe", "创建怪物", "配置按需加载")
    ...
    def ce_shi_ju_qin2(self):
        super().open_list("Func", "功能列表")
        super().open_ui_from_ui_list("测试剧情2", "TestPlotBlackCurtain2", "FuncListSwipe", "创建怪物", "配置按需加载")
