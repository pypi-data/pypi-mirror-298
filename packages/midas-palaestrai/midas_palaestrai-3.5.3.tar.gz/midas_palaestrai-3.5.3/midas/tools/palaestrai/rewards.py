...
class ExtendedGridHealthReward(Reward):
    ...
    * ``num_out_if_service``
    ...
    def __call__(
        ...
        voltages = np.sort(
            np.array([s() for s in state if "vm_pu" in s.uid]),
            axis=None,
        )
        voltage_rewards = [
            RewardInformation(
        ...
        in_service = np.sort(
            np.array([s() for s in state if "in_service" in s.uid]),
            axis=None,
        )
...
class RetroPsiReward(Reward):
    def __call__(
        ...
        # rewards.append(RewardInformation(sensor.value, Box(-100, 100, (1,), np.double), f"profit_{sensor.uid.split('.')[1]}"))
