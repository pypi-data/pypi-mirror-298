...
class StatusPayment:
    async def statusClosedPayment(
        self: "tripayiin.Tripayiin",
        reference: str
    ) -> Response:
        """
        Get Status Closed Payment

        Args:
            reference (`str`): Transaction reference number to be checked

        Returns:
            dict: Returns a dictionary if successful, otherwise None
        """
        res = await self.get(
            url=f"/transaction/check-status",
