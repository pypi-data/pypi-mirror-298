...
async def async_summarizer(summarizer_config, inference_model_details, text, trace_details, pipeline_id, system_prompt=None, session_id=None, trace_api=None,metadata=None):
    ...
    if summarization_type == SummarizerTypeMaster.EXTRACTIVE.value:
        ...
        result = await chain.ainvoke({"context": summary}, config={"callbacks": chain_callbacks})
    else:
        chain = prompt | llm
        result = await chain.ainvoke({"context": summary}, config={"callbacks": chain_callbacks})
...
def summarizer(summarizer_config, inference_model_details, text, trace_details, pipeline_id, system_prompt=None, session_id=None, trace_api=None,metadata=None):
    ...
    if summarization_type == SummarizerTypeMaster.EXTRACTIVE.value:
        chain = prompt | llm | parser
        result = chain.invoke({"context": summary}, config={"callbacks": chain_callbacks})
    else:
        chain = prompt | llm
        result = chain.invoke({"context": summary}, config={"callbacks": chain_callbacks})
