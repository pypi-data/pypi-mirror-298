...
class PipelineService:
    ...
    async def async_invoke_pipeline(
        ...
        metadata=None
            ...
            if pipeline_ids:
                combined_retrievers = [await get_store(pid).as_retriever() for pid in pipeline_ids]
                retriever = MergerRetriever(retrievers=combined_retrievers)
            else:
                retriever = (await get_store(pipeline_id)).as_retriever()
    ...
    async def invoke_pipeline(
        ...
        if self.therix_api_key:
            ...
            if pipeline_ids:
                combined_retrievers = [
                    get_store(pid).as_retriever() for pid in pipeline_ids
                ]
                retriever = MergerRetriever(retrievers=combined_retrievers)
            else:
                retriever = get_store(pipeline_id).as_retriever()
    ...
    async def async_search_keywords(self, keyword_search_params):
        ...
        if "pipeline_ids" in keyword_search_params:
            combined_retrievers = [await get_store(pipeline_id).as_retriever() for pipeline_id in keyword_search_params["pipeline_ids"]]
            retriever = MergerRetriever(retrievers=combined_retrievers)
        else:
    ...
    async def search_keywords(self, keyword_search_params):
        # required_params = ["prompt", "keywords", "output_parser"]
        # if not all(param in keyword_search_params for param in required_params):
        #     return "Request is missing required parameters, please provide all the parameters, i.e. pipeline_id, prompt, keywords, output_parser"
        if (self.therix_api_key):
            ...
            if keyword_search_params['pipeline_ids'] is not None:
                combined_retrievers = [get_store(pipeline_id).as_retriever() for pipeline_id in keyword_search_params["pipeline_ids"]]
                retriever = MergerRetriever(retrievers=combined_retrievers)
            else:
                pipeline_id = keyword_search_params.get("pipeline_id")
                retriever = get_store(pipeline_id).as_retriever()
