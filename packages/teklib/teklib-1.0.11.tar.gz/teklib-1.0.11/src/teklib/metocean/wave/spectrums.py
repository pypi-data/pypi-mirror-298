...
def compute_wavenumber(wi:float, depth:float, g=9.80665)->float:
    ...
    sol = root_scalar(lambda k_, d, w: w**2 - g*k_*np.tanh(k_*d), 
                            args=(depth, wi), method='brentq', bracket=[0, 30])
...
def tma(f:np.ndarray, depth:float, Hm0:float, Tp:float, gamma:float=None, sigma_low:float=.07, sigma_high:float=.09,
            g:float=9.80665, method:str='yamaguchi', normalize:bool=False):
    """TMA spectrum , DNV-RP-C205, 2019, Section 3.5.6

    Args:
        f (np.ndarray): wave frequencies [Hz]
        depth (float): water depth [m]
        Hm0 (float): significant wave height [m]
        Tp (float): spectral peak period [s]
        gamma (float, optional): JONSWAP peakedness. Defaults to None.
        sigma_low (float, optional): JONSWAP sigma_low. Defaults to .07.
        sigma_high (float, optional): JONSWAP sigma_high. Defaults to .09.
        g (float, optional): gravitational constant [m/s/s]. Defaults to 9.80665.
        method (str, optional): alpha method. Defaults to 'yamaguchi'.
        normalize (bool, optional): normalization flag. Defaults to False.
    ...
    # get the JONSWAP spectrum
    S_js, f = jonswap(f=f, Hm0=Hm0, Tp=Tp, gamma=gamma, sigma_low=sigma_low, sigma_high=sigma_high, 
            g=g, method=method, normalize=normalize)
    
    # compute wave numbers
    k = np.r_[[compute_wavenumber(wi, depth, g) for wi in 2*np.pi*f]]
    kd = k*depth
    
    # quality control
    assert np.all(np.abs(np.sqrt(g*k*np.tanh(kd)) - 2*np.pi*f)) < 1E-6, "Err"
    
    # depth function
    depth_function = np.tanh(kd)**2 / (1 + 2*kd/np.sinh(2*kd))
    
    # TMA spectrum
    S_tma = S_js * depth_function
    
    # normalizing factor
    a = Hm0**2 / (16*np.trapz(S_js*depth_function, f))
    
    # normalized TMA spectrum
    return a * S_tma, f
