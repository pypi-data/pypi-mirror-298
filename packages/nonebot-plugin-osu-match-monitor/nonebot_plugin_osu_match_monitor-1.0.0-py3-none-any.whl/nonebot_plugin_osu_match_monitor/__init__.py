...
monitor = on_command(".osu match monitor", aliases={"monitor"}, priority=5)
stop_monitor = on_command(".osu match stop", aliases={"stopmonitor"}, priority=5)
    ...
    while room_id in monitoring_rooms:
        try:
            ...
            if new_match_info and "games" in new_match_info:
                previous_match_info = monitoring_rooms[room_id]
                ...
                for game_id, game in new_games_dict.items():
                    ...
                    if not prev_game:
                        ...
                        if game["end_time"] and not prev_game.get("end_time"):
                            # 比赛结束
                            room_name = new_match_info["match"].get("name", "未知房间")
                            await bot.send(event, f"房间 #{room_id} {room_name} 的比赛已结束。")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"请求地图 {beatmap_id} 信息失败，状态码：{response.status_code}")
    except Exception as e:
        logger.exception(f"获取地图信息时出错：{e}")
    return {}
...
async def format_game_info(game: Dict) -> str:
    ...
    beatmap_info = await get_beatmap_info(beatmap_id)
    beatmap_title = f"{beatmap_info.get('artist', '')} - {beatmap_info.get('title', '')} [{beatmap_info.get('version', '')}]" if beatmap_info else f"未知地图（ID: {beatmap_id}）"
...
async def format_scores(scores: List[Dict], play_mode_code: str) -> str:
    ...
    for score in scores:
        ...
        user_display = f"{country_name} {username} ({user_id})"
        ...
        score_messages.append(
            ...
            f"100：{count100}  50：{count50}  MISS：{countmiss}  PASS：{pass_status}\n"
            f"Mods：{enabled_mods}\n"
        )
    return "\n".join(score_messages)
