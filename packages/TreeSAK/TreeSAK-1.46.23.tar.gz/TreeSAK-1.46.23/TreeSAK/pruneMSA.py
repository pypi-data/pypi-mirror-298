...
def pruneMSA(args):
    ...
    for each_cutoff in cutoff_list:
        ...
        current_msa_out     = '%s/%s.chi2prune%s.%s'                            % (msa_path, msa_base, each_cutoff, msa_ext)
        ...
        perl_cmd            = 'perl %s --file %s --chi2_prune %s > %s'          % (alignment_pruner_pl,   msa_in, cutoff_formatted, current_msa_out)
        perl_cmd_for_report = 'perl %s --file %s --chi2_prune %s > %s'          % ('alignment_pruner.pl', msa_in, cutoff_formatted, current_msa_out)
