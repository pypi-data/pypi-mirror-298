...
OPTIONS_VALUES = {
    "select_option_11": "Los últimos 30 días",
...
def handle_find_wait_and_click(driver: WebDriver, find_xpath: str) -> None:
    element = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, find_xpath))
    )
    element.click()

def handle_find_visibility_and_click(driver: WebDriver, find_xpath: str) -> None:
    element = WebDriverWait(driver, 10).until(
        EC.visibility_of_element_located((By.XPATH, find_xpath))
    )
    element.click()
...
def handle_filters(pbar: tqdm, driver: WebDriver, select_option="select_option_21"):
    ...
    # class datepicker
    handle_find_wait_and_click(driver, '//control-layout-wrapper[@class="datepicker"]')

    # select selectedDateRangeOption
    handle_find_wait_and_click(driver, '//md-select[@ng-model="$ctrl.selectedDateRangeOption"]')

    # select contains text
    handle_find_visibility_and_click(driver, f'//md-option//div[contains(text(), "{OPTIONS_VALUES[select_option]}")]')

    # class md-raised
    handle_find_wait_and_click(driver, '//*[contains(@class, "md-raised")]')
