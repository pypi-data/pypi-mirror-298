...
import boraq
from boraq.exceptions import NotInBoraqDirectoryError
from boraq.utils import (
	UNSET_ARG,
	fetch_details_from_tag,
	get_app_cache_extract_filter,
	get_available_folder_name,
	get_boraq_cache_path,
	is_boraq_directory,
	is_git_url,
...
from boraq.utils.boraq import build_assets, install_python_dev_dependencies
from boraq.utils.render import step

if typing.TYPE_CHECKING:
	from boraq.boraq import Boraq


logger = logging.getLogger(boraq.PROJECT_NAME)
...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/clairview/healthcare.git
		        2. git@github.com:clairview/healthcare.git
		        3. clairview/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"clairview/clairerp": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.boraq and os.path.exists(os.path.join(self.boraq.name, "apps", self.name)):
			self.mount_path = os.path.join(self.boraq.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.boraq.shallow_clone else ""
		...
		self.boraq.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.boraq.name, "apps"),
		)
	...
	def install(
		...
		import boraq.cli
		from boraq.utils.app import get_app_name

		self.validate_app_dependencies()

		verbose = boraq.cli.verbose or verbose
		app_name = get_app_name(self.boraq.name, self.app_name)
		if not resolved and self.app_name != "clairview" and not ignore_resolution:
			click.secho(
		...
		install_app(
			app=app_name,
			tag=self.tag,
			boraq_path=self.boraq.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_boraq=restart_boraq,
	...
	def uninstall(self):
		self.boraq.run(f"{self.boraq.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from boraq.utils.app import get_required_deps, required_apps_from_hooks
	...
	def get_pyproject(self) -> Optional[dict]:
		from boraq.utils.app import get_pyproject

		if self.pyproject:
			return self.pyproject

		apps_path = os.path.join(os.path.abspath(self.boraq.name), "apps")
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("boraq", {}).get("clairview-dependencies")
		)
		if not deps:
			return
	...
	boraq sub directories. If we assume deterministic builds
	...
	def get_app_path(self) -> Path:
		return Path(self.boraq.name) / "apps" / self.app_name

	def get_app_cache_temp_path(self, is_compressed=False) -> Path:
		cache_path = get_boraq_cache_path("apps")
	...
	def get_cached(self) -> bool:
		...
		click.secho(
			f"Boraq app-cache: extracting {self.app_name} from {cache_path.as_posix()}",
		)
		...
		with tarfile.open(cache_path, mode) as tar:
			...
			try:
				...
				click.secho(
					f"Boraq app-cache: extraction succeeded for {self.app_name}",
					fg="green",
				)
			except Exception:
				message = f"Boraq app-cache: extraction failed for {self.app_name}"
				click.secho(
					message,
					fg="yellow",
				)
	...
	def set_cache(self, compress_artifacts=False) -> bool:
		...
		message = f"Boraq app-cache: caching {self.app_name}"
		...
		try:
			...
			click.secho(
				f"Boraq app-cache: caching succeeded for {self.app_name} as {hashed_path.as_posix()}",
				fg="green",
			)

			success = True
		except Exception as exc:
			log(f"Boraq app-cache: caching failed for {self.app_name} {exc}", level=3)
			success = False
		finally:
			os.chdir(cwd)
		return success
	...
	def prune_app_directory(self):
		app_path = self.get_app_path()
		if can_clairview_use_cached(self):
...
def can_clairview_use_cached(app: App) -> bool:
	min_clairview = get_required_clairview_version(app)
	if not min_clairview:
		return False

	try:
		return sv.Version(min_clairview) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_clairview)
	except ValueError:
		click.secho(
			f"Boraq app-cache: invalid value found for clairview version '{min_clairview}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	dep_path = Path(app.boraq.name) / "apps" / dep
	if not dep_path.is_dir():
		click.secho(f"Required clairview-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed clairview-dependency '{dep}' version '{dep_version}' "
...
def get_required_clairview_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/clairview/boraq/issues/1524
	req_clairview = (
		pyproject.get("tool", {})
		.get("boraq", {})
		.get("clairview-dependencies", {})
		.get("clairview")
	)

	if not req_clairview:
		click.secho(
			"Required clairview version not set in pyproject.toml, "
			"please refer: https://github.com/clairview/boraq/issues/1524",
			fg="yellow",
		)

	return req_clairview
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Clairview apps use Vite to build their frontends,
	...
	for p in app_path.iterdir():
		...
		if can_delete:
			click.secho(
				f"Boraq app-cache: removing {node_modules.as_posix()}",
				fg="yellow",
			)
			shutil.rmtree(node_modules)


def make_resolution_plan(app: App, boraq: "Boraq"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, boraq=boraq)
		is_valid_clairview_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, boraq))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(boraq_path="."):
	try:
		with open(os.path.join(boraq_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, boraq_path="."):
	if app == "clairview":
		raise ValueError("Clairview app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(boraq_path=boraq_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, boraq_path=boraq_path)


def write_excluded_apps_txt(apps, boraq_path="."):
	with open(os.path.join(boraq_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, boraq_path="."):
	apps = get_excluded_apps(boraq_path=boraq_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, boraq_path=boraq_path)
...
def get_app(
	...
	"""boraq get-app clones a Clairview App from remote (GitHub or any other git server),
	and installs it on the current boraq. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the boraq_path is not a boraq directory, a new boraq is created named using the
	git_url parameter.
	"""
	import boraq as _boraq
	import boraq.cli as boraq_cli
	from boraq.boraq import Boraq
	from boraq.utils.app import check_existing_dir
	...
	app = App(
		git_url, branch=branch, boraq=boraq, soft_link=soft_link, cache_key=cache_key
	)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	boraq_setup = False
	restart_boraq = not init_boraq
	clairview_path, clairview_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, boraq)
		...
		if "clairview" in resolution:
			# Todo: Make clairview a terminal dependency for all clairview apps.
			clairview_path, clairview_branch = resolution["clairview"].url, resolution["clairview"].tag

	if not is_boraq_directory(boraq_path):
		if not init_boraq:
			raise NotInBoraqDirectoryError(
				f"{os.path.realpath(boraq_path)} is not a valid boraq directory. "
				"Run with --init-boraq if you'd like to create a Boraq too."
			)

		from boraq.utils.system import init

		boraq_path = get_available_folder_name(f"{app.repo}-boraq", boraq_path)
		init(
			path=boraq_path,
			clairview_path=clairview_path,
			clairview_branch=clairview_branch or branch,
		)
		os.chdir(boraq_path)
		boraq_setup = True

	if boraq_setup and boraq_cli.from_command_line and boraq_cli.dynamic_feed:
		_boraq.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(boraq_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_boraq=restart_boraq)
...
def install_resolved_deps(
	...
	if "clairview" in resolution:
		# Terminal dependency
		del resolution["clairview"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(boraq_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = boraq.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, boraq_path="."):
	if boraq.CLAIRVIEW_VERSION in (0, None):
		raise NotInBoraqDirectoryError(
			f"{os.path.realpath(boraq_path)} is not a valid boraq directory."
		)
	...
	apps = os.path.abspath(os.path.join(boraq_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if boraq.CLAIRVIEW_VERSION < 14:
			click.secho("Clairview v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_clairview_cmd(*args, boraq_path=boraq_path)
	install_app(app, boraq_path=boraq_path)
...
def install_app(
	...
	verbose = boraq_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(boraq_path, "apps", app))

	boraq.run(
		f"{boraq.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, boraq_path=boraq_path, verbose=verbose)
	...
	if not using_cached and os.path.exists(os.path.join(app_path, "package.json")):
		...
		boraq.run(yarn_install, cwd=app_path)

	boraq.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(boraq_path=boraq_path, app=app, using_cached=using_cached)

	if restart_boraq:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		boraq.reload(_raise=False)


def pull_apps(apps=None, boraq_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from boraq.boraq import Boraq
	from boraq.utils.app import get_current_branch, get_remote

	boraq = Boraq(boraq_path)
	rebase = "--rebase" if boraq.conf.get("rebase_on_pull") else ""
	apps = apps or boraq.apps
	excluded_apps = boraq.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, boraq_path=boraq_path)
...
2. Temporarily remove your changes with "git stash" or discard them completely
	with "boraq update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, boraq_path=boraq_path)
				print(
			...
			if not boraq.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					boraq.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, boraq_path=boraq_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if boraq.conf.get("shallow_clone"):
					boraq.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					boraq.run(reset_cmd, cwd=app_dir)
					boraq.run("git reflog expire --all", cwd=app_dir)
					boraq.run("git gc --prune=all", cwd=app_dir)
				else:
					boraq.run("git fetch --all", cwd=app_dir)
					boraq.run(reset_cmd, cwd=app_dir)
			else:
				boraq.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			boraq.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(boraq_path):
	boraq_path = os.path.abspath(boraq_path)
	celery_app = os.path.join(boraq_path, "apps", "clairview", "clairview", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, boraq_path="."):
	return os.path.join(boraq_path, "apps", app)
...
def install_apps_from_path(path, boraq_path="."):
...
def validate_cache_and_get_path(app_name: str, cache_key: str) -> "Optional[Path]":
	...
	if not cache_path.is_file():
		click.secho(
			f"Boraq app-cache: file check failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
	...
	if not is_cache_hash_valid(cache_path):
		click.secho(
			f"Boraq app-cache: hash validation failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
		unlink_no_throw(cache_path)
		return
