from boraq.config.common_site_config import update_config, put_config

import click


@click.group(help="Change boraq configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable boraq multitenancy on running boraq update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, boraq_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current boraq's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from boraq.boraq import Boraq

	common_site_config = Boraq(".").conf
	for key in keys:
