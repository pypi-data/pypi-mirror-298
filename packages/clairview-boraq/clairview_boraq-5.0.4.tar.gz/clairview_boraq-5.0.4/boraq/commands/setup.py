...
from boraq.utils import exec_cmd, run_playbook, which
from boraq.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Clairview environment")
def setup():
	pass
...
def setup_sudoers(user):
	from boraq.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	from boraq.config.nginx import make_nginx_conf

	make_nginx_conf(boraq_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from boraq.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	from boraq.utils import get_cmd_output
	from boraq.config.supervisor import (
	...
	generate_supervisor_config(boraq_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Clairview fonts to system")
def setup_fonts():
	from boraq.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Clairview production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from boraq.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for boraq backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for boraq")
@click.option(
...
def setup_env(python="python3"):
	...
	return Boraq(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/boraq/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/boraq/tasks/change_ssh_port.yml", {"ssh_port": port})
...
@click.option(
	...
	default=False,
	is_flag=True,
	help="Run command non-interactively. This flag restarts nginx and runs certbot non interactively. Shouldn't be used on 1'st attempt", # noqa
)
def setup_letsencrypt(site, custom_domain, non_interactive):
	from boraq.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, boraq_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant boraq"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from boraq.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, boraq_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for boraq start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		boraq.setup.requirements(apps=apps)

	elif not node and not dev:
		boraq.setup.python(apps=apps)

	elif not python and not dev:
		boraq.setup.node(apps=apps)

	else:
		from boraq.utils.boraq import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup boraq-manager.local site with the boraq_manager app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run boraq manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run boraq manager")
def setup_manager(yes=False, port=23624, domain=None):
	from boraq.boraq import Boraq
	from boraq.config.nginx import make_boraq_manager_nginx_conf

	create_new_site = True

	if "boraq-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("boraq new-site --force boraq-manager.local")

	if "boraq_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("boraq get-app boraq_manager")

	exec_cmd("boraq --site boraq-manager.local install-app boraq_manager")

	boraq_path = "."
	boraq = Boraq(boraq_path)

	if boraq.conf.get("restart_supervisor_on_update") or boraq.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want "
				"to host boraq-manager using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in boraq.sites:
			raise Exception("No such site")

		make_boraq_manager_nginx_conf(boraq_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from boraq.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, boraq_path=".")
...
def remove_domain(domain, site=None):
	...
	from boraq.config.site_config import remove_domain

	remove_domain(site, domain, boraq_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from boraq.config.site_config import sync_domains

	changed = sync_domains(site, domains, boraq_path=".")
...
@click.command(
	"fail2ban",
	help="Setup fail2ban, an intrusion prevention software framework that protects computer servers from brute-force attacks",	# noqa
)
@click.option(
	"--maxretry",
	default=6,
	help="Number of matches (i.e. value of the counter) which triggers ban action on the IP. Default is 6 seconds",	# noqa
)
@click.option(
	"--bantime",
	default=600,
	help="Duration (in seconds) for IP to be banned for. Negative number for 'permanent' ban. Default is 600 seconds",	# noqa
)
@click.option(
	"--findtime",
	default=600,
	help="The counter is set to zero if match found within 'findtime' seconds doesn't exceed 'maxretry'. Default is 600 seconds",	# noqa
)
...
@click.option("--stop", help="Stop boraq services", is_flag=True, default=False)
...
def setup_systemd(
	...
	from boraq.config.systemd import generate_systemd_config
