...
def is_version_upgrade(app="clairview", boraq_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, boraq_path=boraq_path)
	...
	local_version = get_major_version(get_current_version(app, boraq_path=boraq_path))
...
def switch_branch(branch, apps=None, boraq_path=".", upgrade=False, check_upgrade=True):
	import git
	from boraq.boraq import Boraq
	from boraq.utils import log, exec_cmd
	...
	apps_dir = os.path.join(boraq_path, "apps")
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, boraq_path=boraq_path, branch=branch)
	...
	if switched_apps:
		...
		log(
			"Please run `boraq update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		Boraq(boraq_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, boraq_path=".", upgrade=False):
	switch_branch(branch, apps=apps, boraq_path=boraq_path, upgrade=upgrade)


def switch_to_develop(apps=None, boraq_path=".", upgrade=True):
	switch_branch("develop", apps=apps, boraq_path=boraq_path, upgrade=upgrade)
...
def get_develop_version(app, boraq_path="."):
	repo_dir = get_repo_dir(app, boraq_path=boraq_path)
...
def get_upstream_version(app, branch=None, boraq_path="."):
	repo_dir = get_repo_dir(app, boraq_path=boraq_path)
	if not branch:
...
def get_current_clairview_version(boraq_path="."):
	try:
		return get_major_version(get_current_version("clairview", boraq_path=boraq_path))
	except OSError:
		return 0


def get_current_branch(app, boraq_path="."):
...
def get_app_name(boraq_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(boraq_path), "apps")
...
def check_existing_dir(boraq_path, repo_name):
	cloned_path = os.path.join(boraq_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, boraq_path="."):
