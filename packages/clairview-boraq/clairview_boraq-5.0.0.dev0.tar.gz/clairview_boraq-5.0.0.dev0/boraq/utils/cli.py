...
def print_boraq_version(ctx, param, value):
	"""Prints current boraq version"""
	if not value or ctx.resilient_parsing:
		return

	import boraq

	click.echo(boraq.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		boraq.cli.dynamic_feed = True
		boraq.cli.verbose = True
	else:
		from boraq.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from boraq.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: boraq is using it's new CLI rendering engine. This behaviour has"
