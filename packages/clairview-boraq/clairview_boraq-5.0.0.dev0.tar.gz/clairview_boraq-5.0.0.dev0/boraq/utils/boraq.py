...
import boraq
from boraq.exceptions import PatchError, ValidationError
...
def get_env_cmd(cmd: str, boraq_path: str = ".") -> str:
	exact_location = os.path.abspath(
		os.path.join(boraq_path, "env", "bin", cmd.strip("*"))
	)
	...
	existing_python_bins = glob(
		os.path.join(boraq_path, "env", "**", "bin", cmd), recursive=True
	)
...
def update_node_packages(boraq_path=".", apps=None, verbose=None):
	print("Updating node packages...")
	from distutils.version import LooseVersion

	from boraq.utils.app import get_develop_version

	v = LooseVersion(get_develop_version("clairview", boraq_path=boraq_path))

	# After rollup was merged, clairview_version = 10.1
	# if develop_verion is 11 and up, only then install yarn
	if v < LooseVersion("11.x.x-develop"):
		update_npm_packages(boraq_path, apps=apps, verbose=verbose)
	else:
		update_yarn_packages(boraq_path, apps=apps, verbose=verbose)


def install_python_dev_dependencies(boraq_path=".", apps=None, verbose=False):
	import boraq.cli
	...
	if isinstance(apps, str):
		...
		apps = boraq.get_installed_apps()

	for app in apps:
		pyproject_deps = None
		app_path = os.path.join(boraq_path, "apps", app)
		...
		if os.path.exists(pyproject_path):
			...
			if pyproject_deps:
				boraq.run(f"{boraq.python} -m pip install {quiet_flag} --upgrade {pyproject_deps}")

		if not pyproject_deps and os.path.exists(dev_requirements_path):
			boraq.run(
				f"{boraq.python} -m pip install {quiet_flag} --upgrade -r {dev_requirements_path}"
			)
...
def _generate_dev_deps_pattern(pyproject_path):
	...
	with contextlib.suppress(KeyError):
		for pkg, version in pyroject_config["tool"]["boraq"]["dev-dependencies"].items():
			op = "==" if "=" not in version else ""
			requirements_pattern += f"{pkg}{op}{version} "
	return requirements_pattern


def update_yarn_packages(boraq_path=".", apps=None, verbose=None):
	...
	apps_dir = os.path.join(boraq.name, "apps")
...
def update_npm_packages(boraq_path=".", apps=None, verbose=None):
	verbose = boraq.cli.verbose or verbose
	...
	with open(os.path.join(boraq_path, "package.json"), "w") as f:
		f.write(json.dumps(package_json, indent=1, sort_keys=True))

	exec_cmd(npm_install, cwd=boraq_path)
...
def migrate_env(python, backup=False):
	...
	try:
		...
		# Install clairview first
		_install_app("clairview")
		for app in boraq.apps:
			if str(app) != "clairview":
				_install_app(app)
...
def validate_upgrade(from_ver, to_ver, boraq_path="."):
	if to_ver >= 6 and not which("npm") and not which("node") and not which("nodejs"):
		raise Exception("Please install nodejs and npm")


def post_upgrade(from_ver, to_ver, boraq_path="."):
	from boraq.boraq import Boraq
	from boraq.config import redis
	from boraq.config.nginx import make_nginx_conf
	from boraq.config.supervisor import generate_supervisor_config

	conf = Boraq(boraq_path).conf
	print("-" * 80 + f"Your boraq was upgraded to version {to_ver}")

	if conf.get("restart_supervisor_on_update"):
		redis.generate_config(boraq_path=boraq_path)
		generate_supervisor_config(boraq_path=boraq_path)
		make_nginx_conf(boraq_path=boraq_path)
		print(
			"As you have setup your boraq for production, you will have to reload"
...
def patch_sites(boraq_path="."):
	from boraq.boraq import Boraq
	from boraq.utils.system import migrate_site

	boraq = Boraq(boraq_path)

	for site in boraq.sites:
		try:
			migrate_site(site, boraq_path=boraq_path)
		except subprocess.CalledProcessError:
			raise PatchError


def restart_supervisor_processes(boraq_path=".", web_workers=False, _raise=False):
	from boraq.boraq import Boraq
	...
	if cmd:
		boraq.run(cmd, _raise=_raise)

	else:
		sudo = ""
		try:
			supervisor_status = get_cmd_output("supervisorctl status", cwd=boraq_path)
		except subprocess.CalledProcessError as e:
			if e.returncode == 127:
				log("restart failed: Couldn't find supervisorctl in PATH", level=3)
				return
			sudo = "sudo "
			supervisor_status = get_cmd_output("sudo supervisorctl status", cwd=boraq_path)
		...
		if web_workers and f"{boraq_name}-web:" in supervisor_status:
			groups = [f"{boraq_name}-web:\t"]

		elif f"{boraq_name}-workers:" in supervisor_status:
			groups = [f"{boraq_name}-web:", f"{boraq_name}-workers:"]

		# backward compatibility
		elif f"{boraq_name}-processes:" in supervisor_status:
			groups = [f"{boraq_name}-processes:"]
		...
		for group in groups:
			failure = boraq.run(f"{sudo}supervisorctl restart {group}", _raise=_raise)
			if failure:
				log(
					f"restarting supervisor group `{group}` failed. Use `boraq restart` to retry.",
					level=3,
				)


def restart_systemd_processes(boraq_path=".", web_workers=False, _raise=True):
	boraq_name = get_boraq_name(boraq_path)
	exec_cmd(
		f"sudo systemctl stop -- $(systemctl show -p Requires {boraq_name}.target | cut"
		" -d= -f2)",
		_raise=_raise,
	)
	exec_cmd(
		f"sudo systemctl start -- $(systemctl show -p Requires {boraq_name}.target |"
		" cut -d= -f2)",
		_raise=_raise,
	)


def restart_process_manager(boraq_path=".", web_workers=False):
	# only overmind has the restart feature, not sure other supported procmans do
	if which("overmind") and os.path.exists(os.path.join(boraq_path, ".overmind.sock")):
		worker = "web" if web_workers else ""
		exec_cmd(f"overmind restart {worker}", cwd=boraq_path)


def build_assets(boraq_path=".", app=None, using_cached=False):
	command = "boraq build"
	if app:
		command += f" --app {app}"

	env = {"BORAQ_DEVELOPER": "1"}
	if using_cached:
		env["USING_CACHED"] = "1"

	exec_cmd(command, cwd=boraq_path, env=env)


def handle_version_upgrade(version_upgrade, boraq_path, force, reset, conf):
	from boraq.utils import log, pause_exec

	if version_upgrade[0]:
		if force:
			log(
				"""Force flag has been used for a major version change in Clairview and it's apps.
This will take significant time to migrate and might break custom apps.""",
				level=3,
			)
		else:
			print(
				f"""This update will cause a major version change in Clairview/CLAIRerp from {version_upgrade[1]} to {version_upgrade[2]}.
This would take significant time to migrate and might break custom apps."""
	...
	if not reset and conf.get("shallow_clone"):
		log(
			"""shallow_clone is set in your boraq config.
However without passing the --reset flag, your repositories will be unshallowed.
To avoid this, cancel this operation and run `boraq update --reset`.
...
To avoid seeing this warning, set shallow_clone to false in your common_site_config.json
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		validate_upgrade(version_upgrade[1], version_upgrade[2], boraq_path=boraq_path)
...
def update(
	...
	"""command: boraq update"""
	import re

	from boraq import patches
	from boraq.app import pull_apps
	from boraq.boraq import Boraq
	from boraq.config.common_site_config import update_config
	from boraq.exceptions import CannotUpdateReleaseBoraq
	from boraq.utils.app import is_version_upgrade
	from boraq.utils.system import backup_all_sites

	boraq_path = os.path.abspath(".")
	boraq = Boraq(boraq_path)
	patches.run(boraq_path=boraq_path)
	conf = boraq.conf

	if conf.get("release_boraq"):
		raise CannotUpdateReleaseBoraq("Release boraq detected, cannot update!")
	...
	handle_version_upgrade(version_upgrade, boraq_path, force, reset, conf)

	conf.update({"maintenance_mode": 1, "pause_scheduler": 1})
	update_config(conf, boraq_path=boraq_path)

	if backup:
		print("Backing up sites...")
		backup_all_sites(boraq_path=boraq_path)

	if pull:
		print("Updating apps source...")
		pull_apps(apps=apps, boraq_path=boraq_path, reset=reset)

	if requirements:
		print("Setting up requirements...")
		boraq.setup.requirements()

	if patch:
		print("Patching sites...")
		patch_sites(boraq_path=boraq_path)
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		post_upgrade(version_upgrade[1], version_upgrade[2], boraq_path=boraq_path)

	boraq.reload(web=False, supervisor=restart_supervisor, systemd=restart_systemd)

	conf.update({"maintenance_mode": 0, "pause_scheduler": 0})
	update_config(conf, boraq_path=boraq_path)

	print(
		"_" * 80 + "\nBoraq: Deployment tool for Clairview and Clairview Applications"
		" (https://clairview.io/boraq).\nOpen source depends on your contributions, so do"
		" give back by submitting bug reports, patches and fixes and be a part of the"
		" community :)"
	)


def clone_apps_from(boraq_path, clone_from, update_app=True):
	from boraq.app import install_app

	print(f"Copying apps from {clone_from}...")
	subprocess.check_output(["cp", "-R", os.path.join(clone_from, "apps"), boraq_path])

	node_modules_path = os.path.join(clone_from, "node_modules")
	if os.path.exists(node_modules_path):
		print(f"Copying node_modules from {clone_from}...")
		subprocess.check_output(["cp", "-R", node_modules_path, boraq_path])
	...
	def setup_app(app):
		...
		install_app(app, boraq_path, restart_boraq=False)
...
def remove_backups_crontab(boraq_path="."):
	...
	user = Boraq(boraq_dir).conf.get("clairview_user")
...
def set_mariadb_host(host, boraq_path="."):
	update_common_site_config({"db_host": host}, boraq_path=boraq_path)


def set_redis_cache_host(host, boraq_path="."):
	update_common_site_config({"redis_cache": f"redis://{host}"}, boraq_path=boraq_path)


def set_redis_queue_host(host, boraq_path="."):
	update_common_site_config({"redis_queue": f"redis://{host}"}, boraq_path=boraq_path)


def set_redis_socketio_host(host, boraq_path="."):
	update_common_site_config({"redis_socketio": f"redis://{host}"}, boraq_path=boraq_path)


def update_common_site_config(ddict, boraq_path="."):
	filename = os.path.join(boraq_path, "sites", "common_site_config.json")
...
def validate_app_installed_on_sites(app, boraq_path="."):
	print("Checking if app installed on active sites...")
	ret = check_app_installed(app, boraq_path=boraq_path)

	if ret is None:
		check_app_installed_legacy(app, boraq_path=boraq_path)
	else:
		return ret


def check_app_installed(app, boraq_path="."):
	try:
		out = subprocess.check_output(
			["boraq", "--site", "all", "list-apps", "--format", "json"],
...
def check_app_installed_legacy(app, boraq_path="."):
	site_path = os.path.join(boraq_path, "sites")

	for site in os.listdir(site_path):
		req_file = os.path.join(site_path, site, "site_config.json")
		if os.path.exists(req_file):
			out = subprocess.check_output(
				["boraq", "--site", site, "list-apps"], cwd=boraq_path
			).decode("utf-8")
...
def validate_branch():
	from boraq.boraq import Boraq
	from boraq.utils.app import get_current_branch

	apps = Boraq(".").apps

	installed_apps = set(apps)
	check_apps = {"clairview", "clairerp"}
...
To switch to your required branch, run the following commands: boraq switch-to-branch [branch-name]"""
			)
...
def cache_list() -> None:
	...
	for item in get_boraq_cache_path("apps").iterdir():
