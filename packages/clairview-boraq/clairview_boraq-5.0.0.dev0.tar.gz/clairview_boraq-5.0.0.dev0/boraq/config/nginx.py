...
from boraq.utils import get_boraq_name


def make_nginx_conf(boraq_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(boraq_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"boraq_name_hash": hashlib.sha256(boraq_name).hexdigest()[:16],
...
def make_boraq_manager_nginx_conf(boraq_path, yes=False, port=23624, domain=None):
	from boraq.config.site_config import get_site_config

	template = boraq.config.env().get_template("boraq_manager_nginx.conf")
	boraq_path = os.path.abspath(boraq_path)
	sites_path = os.path.join(boraq_path, "sites")

	config = Boraq(boraq_path).conf
	site_config = get_site_config(domain, boraq_path=boraq_path)
	boraq_name = get_boraq_name(boraq_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"boraq_manager_site_name": "boraq-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	boraq_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(boraq_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and boraq-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(boraq_manager_nginx_conf)


def prepare_sites(config, boraq_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(boraq_path=boraq_path)
...
def get_sites_with_config(boraq_path):
	from boraq.boraq import Boraq
	from boraq.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, boraq_path=boraq_path)
	...
	use_wildcard_certificate(boraq_path, ret)

	return ret


def use_wildcard_certificate(boraq_path, ret):
	"""
	stored in common_site_config.json as:
	"wildcard": {
	        "domain": "*.clairerp.com",
	        "ssl_certificate": "/path/to/clairerp.com.cert",
	        "ssl_certificate_key": "/path/to/clairerp.com.key"
	}
	"""
	from boraq.boraq import Boraq
	...
	for site in ret:
		...
		if (site.get("domain") or site["name"]).endswith(domain):
			# example: ends with .clairerp.com
...
def get_error_pages():
	boraq_app_path = os.path.abspath(boraq.__path__[0])
	templates = os.path.join(boraq_app_path, "config", "templates")
