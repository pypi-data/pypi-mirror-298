...
from boraq.utils import get_boraq_name, which

logger = logging.getLogger(boraq.PROJECT_NAME)


def generate_supervisor_config(boraq_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective boraq path"""
	if not user:
		user = getpass.getuser()

	config = Boraq(boraq_path).conf
	template = boraq.config.env().get_template("supervisor.conf")
	...
	config = template.render(
		**{
			...
			"multi_queue_consumption": can_enable_multi_queue_consumption(boraq_path),
			"supervisor_startretries": 10,
		}
	)

	conf_path = os.path.join(boraq_path, "config", "supervisor.conf")
	...
	update_config({"restart_supervisor_on_update": True}, boraq_path=boraq_path)
	update_config({"restart_systemd_on_update": False}, boraq_path=boraq_path)
	sync_socketio_port(boraq_path)
...
def sync_socketio_port(boraq_path):
	# Backward compatbility: always keep redis_cache and redis_socketio port same
	common_config = get_config(boraq_path=boraq_path)
...
def can_enable_multi_queue_consumption(boraq_path: str) -> bool:
	try:
		...
		clairview_version = Version(get_current_version("clairview", boraq_path=boraq_path))

		return clairview_version > supported_version
	except Exception:
		return False


def check_supervisord_config(user=None):
	"""From boraq v5.x, we're moving to supervisor running as user"""
	# i don't think boraq should be responsible for this but we're way past this now...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@clairview.io
	import configparser
