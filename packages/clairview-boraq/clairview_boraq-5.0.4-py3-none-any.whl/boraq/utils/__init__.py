...
from boraq import PROJECT_NAME, VERSION
from boraq.exceptions import (
...
paths_in_boraq = ("apps", "sites", "config", "logs", "config/pids")
sudoers_file = "/etc/sudoers.d/clairview"
UNSET_ARG = object()


def is_boraq_directory(directory=os.path.curdir):
	is_boraq = True

	for folder in paths_in_boraq:
		path = os.path.abspath(os.path.join(directory, folder))
		is_boraq = is_boraq and os.path.exists(path)

	return is_boraq


def is_clairview_app(directory: str) -> bool:
	is_clairview_app = True

	for folder in paths_in_app:
		if not is_clairview_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_clairview_app = is_clairview_app and path

	return bool(is_clairview_app)


def get_boraq_cache_path(sub_dir: Optional[str]) -> Path:
	relative_path = "~/.cache/boraq"
...
def is_valid_clairview_branch(clairview_path: str, clairview_branch: str):
	...
	:param clairview_path: git url
	:type clairview_path: str
	:param clairview_branch: branch to check
	:type clairview_branch: str
	...
	if clairview_branch:
		try:
			res = g.ls_remote("--heads", "--tags", clairview_path, clairview_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {clairview_branch} for the remote {clairview_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid clairview path: {clairview_path}") from e
...
def log(message, level=0, no_log=False, stderr=False):
	...
	if boraq.cli.from_command_line and boraq.cli.dynamic_feed:
		boraq.LOG_BUFFER.append({"prefix": prefix, "message": message, "color": color})
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/clairview-boraq/json")
	except Exception:
	...
	if pypi_request.status_code == 200:
		...
		if pypi_version > local_version:
			log(
				f"A newer version of boraq is available: {local_version} → {pypi_version}",
				stderr=True,
			)
...
def setup_logging(boraq_path=".") -> logging.Logger:
	LOG_LEVEL = 15
	...
	if os.path.exists(os.path.join(boraq_path, "logs")):
		log_file = os.path.join(boraq_path, "logs", "boraq.log")
...
def run_clairview_cmd(*args, **kwargs):
	from boraq.cli import from_command_line
	from boraq.utils.boraq import get_env_cmd

	boraq_path = kwargs.get("boraq_path", ".")
	f = get_env_cmd("python", boraq_path=boraq_path)
	sites_dir = os.path.join(boraq_path, "sites")
	...
	p = subprocess.Popen(
		(f, "-m", "clairview.utils.boraq_helper", "clairview") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_boraq_name(boraq_path):
	return os.path.basename(os.path.abspath(boraq_path))


def set_git_remote_url(git_url, boraq_path="."):
	"Set app remote git url"
	from boraq.app import get_repo_dir
	from boraq.boraq import Boraq

	app = git_url.rsplit("/", 1)[1].rsplit(".", 1)[0]

	if app not in Boraq(boraq_path).apps:
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(boraq.__path__[0], "playbooks"))


def find_boraqes(directory: str = None) -> List:
	if not directory:
	...
	if is_boraq_directory(directory):
		if os.path.curdir == directory:
			print("You are in a boraq directory!")
		else:
			print(f"{directory} is a boraq directory!")
		return
	...
	for sub in sub_directories:
		...
		if os.path.isdir(sub) and not os.path.islink(sub):
			if is_boraq_directory(sub):
				print(f"{sub} found!")
				boraqes.append(sub)
			else:
				boraqes.extend(find_boraqes(sub))
...
def find_parent_boraq(path: str) -> str:
	"""Checks if parent directories are boraqes"""
	if is_boraq_directory(directory=path):
		return path
	...
	if path not in {home_path, root_path}:
		...
		return find_parent_boraq(parent_dir)


def get_env_clairview_commands(boraq_path=".") -> List:
	"""Caches all available commands (even custom apps) via Clairview
	Default caching behaviour: generated the first time any command (for a specific boraq directory)
	"""
	from boraq.utils.boraq import get_env_cmd

	python = get_env_cmd("python", boraq_path=boraq_path)
	sites_path = os.path.join(boraq_path, "sites")

	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m clairview.utils.boraq_helper get-clairview-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo, using_cached: bool = False):
	...
	raise InvalidRemoteException(
		f"{org_repo} not found under clairview or clairerp GitHub accounts"
	)
...
class _dict(dict):
	...
	# boraq port of clairview._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix clairview#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `boraq --profile --site clairviewframework.com migrate --no-backup`
	sys.argv: ["/home/clairview/.local/bin/boraq", "--profile", "--site", "clairviewframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to clairview's boraq_helper
	from boraq.boraq import Boraq

	clairview_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in clairview_context.flags:
			continue

		elif arg in clairview_context.params:
			skip_next = True
			continue

		if sys_argv.index(arg) == 0 and arg in Boraq(".").apps:
			continue
...
def get_app_cache_extract_filter(
	...
	def filter_function(member: TarInfo, dest_path: str) -> Optional[TarInfo]:
		...
		try:
			...
			# Links created by `clairview` after extraction
			return None
