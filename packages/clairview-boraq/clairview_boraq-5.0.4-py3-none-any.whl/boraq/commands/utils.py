...
@click.command("start", help="Start Clairview development processes")
@click.option("--no-dev", is_flag=True, default=False)
@click.option(
	"--no-prefix",
	is_flag=True,
	default=False,
	help="Hide process name from boraq start log",
)
...
def start(no_dev, concurrency, procfile, no_prefix, man):
	from boraq.utils.system import start
...
def restart(web, supervisor, systemd):
	...
	Boraq(".").reload(web, supervisor, systemd)
...
def set_nginx_port(site, port):
	from boraq.config.site_config import set_nginx_port
...
def set_ssl_certificate(site, ssl_certificate_path):
	from boraq.config.site_config import set_ssl_certificate
...
def set_ssl_certificate_key(site, ssl_certificate_key_path):
	from boraq.config.site_config import set_ssl_certificate_key
...
def set_url_root(site, url_root):
	from boraq.config.site_config import set_url_root

	set_url_root(site, url_root)


@click.command("set-mariadb-host", help="Set MariaDB host for boraq")
@click.argument("host")
def set_mariadb_host(host):
	from boraq.utils.boraq import set_mariadb_host

	set_mariadb_host(host)


@click.command("set-redis-cache-host", help="Set Redis cache host for boraq")
@click.argument("host")
def set_redis_cache_host(host):
	"""
	Usage: boraq set-redis-cache-host localhost:6379/1
	"""
	from boraq.utils.boraq import set_redis_cache_host

	set_redis_cache_host(host)


@click.command("set-redis-queue-host", help="Set Redis queue host for boraq")
@click.argument("host")
def set_redis_queue_host(host):
	"""
	Usage: boraq set-redis-queue-host localhost:6379/2
	"""
	from boraq.utils.boraq import set_redis_queue_host

	set_redis_queue_host(host)


@click.command("set-redis-socketio-host", help="Set Redis socketio host for boraq")
@click.argument("host")
def set_redis_socketio_host(host):
	"""
	Usage: boraq set-redis-socketio-host localhost:6379/3
	"""
	from boraq.utils.boraq import set_redis_socketio_host
...
def download_translations():
	from boraq.utils.translation import download_translations_p
...
def renew_lets_encrypt():
	from boraq.config.lets_encrypt import renew_certs

	renew_certs()


@click.command("backup-all-sites", help="Backup all sites in current boraq")
def backup_all_sites():
	from boraq.utils.system import backup_all_sites

	backup_all_sites(boraq_path=".")


@click.command(
	"disable-production", help="Disables production environment for the boraq."
)
def disable_production():
	from boraq.config.production_setup import disable_production

	disable_production(boraq_path=".")


@click.command(
	"src", help="Prints boraq source folder path, which can be used as: cd `boraq src`"
)
def boraq_src():
	from boraq.cli import src

	print(os.path.dirname(src))


@click.command("find", help="Finds boraqes recursively from location")
@click.argument("location", default="")
def find_boraqes(location):
	from boraq.utils import find_boraqes

	find_boraqes(directory=location)
...
def migrate_env(python, backup=True):
	from boraq.utils.boraq import migrate_env

	migrate_env(python=python, backup=backup)


@click.command("app-cache", help="View or remove items belonging to boraq get-app cache")
...
def app_cache_helper(clear=False, remove_app="", remove_key=""):
	from boraq.utils.boraq import cache_helper
