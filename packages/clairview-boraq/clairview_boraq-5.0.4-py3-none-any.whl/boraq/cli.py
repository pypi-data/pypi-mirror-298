...
change_uid_msg = "Don't run this command as root"
...
def cli():
	setup_clear_cache()
	global from_command_line, boraq_config, is_envvar_warn_set, verbose

	from_command_line = True
	command = " ".join(sys.argv)
	argv = set(sys.argv)
	is_envvar_warn_set = not (os.environ.get("BORAQ_DEVELOPER") or os.environ.get("CI"))
	...
	if (
		is_envvar_warn_set
		and is_cli_command
		and not boraq_config.get("developer_mode")
		and is_dist_editable(boraq.PROJECT_NAME)
	):
		log(
			"boraq is installed in editable mode!\n\nThis is not the recommended mode"
			" of installation for production. Instead, install the package from PyPI"
			" with: `pip install clairview-boraq`\n",
			level=3,
		)

	in_boraq = is_boraq_directory()
	...
	if (
		...
		log("Command not being executed in boraq directory", level=3)

	if len(sys.argv) == 1 or sys.argv[1] == "--help":
		print(click.Context(boraq_command).get_help())
		if in_boraq:
			print(get_clairview_help())
		return

	_opts = [x.opts + x.secondary_opts for x in boraq_command.params]
	...
	if cmd_from_sys in boraq_command.commands:
...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/clairview_boraq_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		clairview_user = boraq_config.get("clairview_user")
		if clairview_user:
			drop_privileges(uid_name=clairview_user, gid_name=clairview_user)
			os.environ["HOME"] = pwd.getpwnam(clairview_user).pw_dir
		else:
			log(change_uid_msg, level=3)
			sys.exit(1)


def app_cmd(boraq_path="."):
	f = get_env_cmd("python", boraq_path=boraq_path)
	os.chdir(os.path.join(boraq_path, "sites"))
	os.execv(f, [f] + ["-m", "clairview.utils.boraq_helper"] + sys.argv[1:])


def clairview_cmd(boraq_path="."):
	f = get_env_cmd("python", boraq_path=boraq_path)
	os.chdir(os.path.join(boraq_path, "sites"))
	os.execv(f, [f] + ["-m", "clairview.utils.boraq_helper", "clairview"] + sys.argv[1:])


def get_clairview_commands():
	if not is_boraq_directory():
		return set()

	return set(get_env_clairview_commands())


def get_clairview_help(boraq_path="."):
	python = get_env_cmd("python", boraq_path=boraq_path)
	sites_path = os.path.join(boraq_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -m clairview.utils.boraq_helper get-clairview-help", cwd=sites_path
		)
...
def change_working_directory():
	"""Allows boraq commands to be run from anywhere inside a boraq directory"""
	cur_dir = os.path.abspath(".")
	boraq_path = find_parent_boraq(cur_dir)
	boraq.current_path = os.getcwd()
	boraq.updated_path = boraq_path
...
def setup_exception_handler():
	...
	from boraq.exceptions import CommandFailedError
