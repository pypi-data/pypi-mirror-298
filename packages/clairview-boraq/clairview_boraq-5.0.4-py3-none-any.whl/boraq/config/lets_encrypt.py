...
from boraq.config.production_setup import service
from boraq.config.site_config import get_domains, remove_domain, update_site_config
from boraq.boraq import Boraq
from boraq.utils import exec_cmd, which
from boraq.utils.boraq import update_common_site_config
from boraq.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, boraq_path, interactive):

	site_path = os.path.join(boraq_path, "sites", site, "site_config.json")
	...
	if not Boraq(boraq_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, boraq_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, boraq_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, boraq_path)
		domains = get_domains(site, boraq_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, boraq_path=boraq_path)
	else:
		update_site_config(site, ssl_config, boraq_path=boraq_path)

	make_nginx_conf(boraq_path)
	service("nginx", "start")
...
def get_certbot_path():
	try:
		...
		raise CommandFailedError(
			"Certbot is not installed on your system."
		)
...
def setup_wildcard_ssl(domain, email, boraq_path, exclude_base_domain):
