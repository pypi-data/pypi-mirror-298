...
default_config = {
	...
	"clairview_user": getpass.getuser(),
	"shallow_clone": True,
...
def setup_config(boraq_path, additional_config=None):
	make_pid_folder(boraq_path)
	boraq_config = get_config(boraq_path)
	boraq_config.update(default_config)
	boraq_config.update(get_gunicorn_workers())
	update_config_for_clairview(boraq_config, boraq_path)
	if additional_config:
		boraq_config.update(additional_config)

	put_config(boraq_config, boraq_path)


def get_config(boraq_path):
	return get_common_site_config(boraq_path)


def get_common_site_config(boraq_path):
	config_path = get_config_path(boraq_path)
...
def put_config(config, boraq_path="."):
	config_path = get_config_path(boraq_path)
	with open(config_path, "w") as f:
		return json.dump(config, f, indent=1, sort_keys=True)


def update_config(new_config, boraq_path="."):
	config = get_config(boraq_path=boraq_path)
	config.update(new_config)
	put_config(config, boraq_path=boraq_path)


def get_config_path(boraq_path):
	return os.path.join(boraq_path, "sites", "common_site_config.json")
...
def update_config_for_clairview(config, boraq_path):
	ports = make_ports(boraq_path)
...
def make_ports(boraq_path):
	from urllib.parse import urlparse

	boraqes_path = os.path.dirname(os.path.abspath(boraq_path))
	...
	existing_ports = {}
	for folder in os.listdir(boraqes_path):
		boraq_path = os.path.join(boraqes_path, folder)
		if os.path.isdir(boraq_path):
			boraq_config = get_config(boraq_path)
			for key in list(default_ports.keys()):
				value = boraq_config.get(key)
...
def make_pid_folder(boraq_path):
	pids_path = os.path.join(boraq_path, "config", "pids")
