...
def __(key):
    return i18n.t('geo.owgeotransform.' + key)
...
class OWGeoTransform(OWWidget):
    ...
    def __init__(self):
        ...
        layout = QFormLayout()
        gui.widgetBox(self.controlArea, box=__("coordinates"), orientation=layout)
        ...
        layout.addRow(__("row_latitude"), gui.comboBox(None, self, "attr_lat", **args))
        layout.addRow(__("row_longitude"), gui.comboBox(None, self, "attr_lon", **args))
        layout.addWidget(
            gui.checkBox(
                None, self, "replace_original", __("replace"),
                tooltip=__("tooltip")))

        layout = QFormLayout()
        gui.widgetBox(self.controlArea, __("transform"), orientation=layout)
        ...
        self.commit_button = gui.button(self.controlArea, self, __("commit"),
                                        self.apply)
