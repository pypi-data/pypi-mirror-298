...
AGG_FUNCS = {
    __("func.instance_count"): AggDesc("size", True, True),
    __("func.defined_values_count"): AggDesc("count", True, True),
    __("func.sum"): AggDesc("sum", False, False),
    __("func.mean"): AggDesc("mean", False, True),
    __("func.median"): AggDesc("median", False, True),
    __("func.mode"): AggDesc(lambda x: stats.mode(x, nan_policy='omit', keepdims=True).mode[0], True, True),
    __("func.maximal"): AggDesc("max", False, True),
    __("func.minimal"): AggDesc("min", False, True),
    __("func.std"): AggDesc("std", False, False)
}


DEFAULT_AGG_FUNCS = {
    DiscreteVariable: __("func.mode"),
    ContinuousVariable: __("func.mean"),
    TimeVariable: __("func.median")
}
...
class OWChoropleth(OWWidget):
    ...
    def _add_controls(self):
        ...
        lat_lon_box = gui.vBox(self.controlArea, box=__("row.map_set"), spacing=0)
        ...
        agg_box = gui.vBox(self.controlArea, box=__("row.control"), spacing=0)
        self.agg_attr_model = DomainModel(valid_types=(ContinuousVariable,
                                                       DiscreteVariable))
        gui.comboBox(agg_box, self, 'agg_attr', label=__("row.value"),
