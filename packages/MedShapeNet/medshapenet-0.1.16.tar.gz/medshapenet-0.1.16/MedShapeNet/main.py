...
class Transformations:
        ...
        - Transform an STL (3D shape with vertices and meshes) or dataset of STL files to NPZ (numpy array)
        - Transfrom an STL file or a dataset of STL files to PLY (pointcloud - vertices)

        *Under construction*
        '''
        def __init__(self, create_download_dir = True):
            # Define the download directory
            self.download_dir = Path("msn_downloads")
            if create_download_dir:
                self.download_dir.mkdir(parents=True, exist_ok=True)
                print("Created the self.download dir 'msn_downloads")
...
list_of_objects = msn_instance.dataset_files(list_of_datasets[0], None, False)
