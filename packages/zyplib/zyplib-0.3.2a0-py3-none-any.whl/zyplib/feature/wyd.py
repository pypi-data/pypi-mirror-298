...
names = [
    ...
    '1st Difference Mean',
    '1st Difference Max',
    'Wavelet Approximate Mean',
    'Wavelet Approximate Std Deviation',
    'Wavelet Detailed Mean',
...
def hjorth(input):  # function for hjorth
    ...
    for j in realinput:
        ...
        k = k + 1
    return (
        np.sum(hjorth_activity) / 14,
        np.sum(hjorth_mobility) / 14,
        np.sum(hjorth_complexity) / 14,
    ...
    for i in b:
        ...
        for j in range(len(i) - 2):
            if abs(temp1[j + 1] - temp1[j]) > t:
                t = (
                    temp1[j + 1] - temp1[j]
                )  # Comparing current Diff with the last updated Diff Max
