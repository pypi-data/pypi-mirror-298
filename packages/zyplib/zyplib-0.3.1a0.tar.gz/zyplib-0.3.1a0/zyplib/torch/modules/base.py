...
def adaptiveavgpool(xdim: XDim = 1) -> AdaptiveAvgPool:
    if xdim == '1d':
        ...
        raise ValueError(f'Invalid parameter xdim: {xdim}; 无法转换为对应的 AdaptiveAvgPool 类')
...
def adaptivemaxpool(xdim: XDim = 1) -> AdaptiveMaxPool:
    if xdim == '1d':
        ...
        raise ValueError(f'Invalid parameter xdim: {xdim}; 无法转换为对应的 AdaptiveMaxPool 类')
...
def convtranspose(xdim: XDim = 1) -> ConvTranspose:
    if xdim == '1d':
        ...
        raise ValueError(f'Invalid parameter xdim: {xdim}; 无法转换为对应的 ConvTranspose 类')
