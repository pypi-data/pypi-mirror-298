...
"""EEGNet Pytorch 实现
    @Author       : Zuo YiPing
    @Date         : 2022-12-05 16:57:43
    @FilePath     : /zypml/zypml/module/backbone/eegnet.py
    @LastEditTime : 2022-12-05 17:19:30
    @Description  : https://github.com/bkozyrskiy/AML_EEG_challenge/blob/master/src/model_torch.py
"""
import math
...
# from zyplib.torch.modules.conv import DepthWiseSplitConv2D
...
def convtransp_output_shape(h_w, kernel_size=1, stride=1, pad=0, dilation=1):
    ...
    h = math.floor(
        (h_w[0] + 2 * pad[0] - dilation[0] * (kernel_size[0] - 1) - 1) / stride[0] + 1
    )
    w = math.floor(
        (h_w[1] + 2 * pad[1] - dilation[1] * (kernel_size[1] - 1) - 1) / stride[1] + 1
    )
    return h, w
...
class deepwise_separable_conv(nn.Module):
    def __init__(self, nin, nout, kernelSize):
        ...
        self.depthwise = nn.Conv2d(
            ...
            padding=(0, self.time_padding),
            groups=nin,
            bias=False
        )
        self.pointwise = nn.Conv2d(
            in_channels=nin, out_channels=nout, kernel_size=1, groups=1, bias=False
        )
    ...
    def get_output_size(self, h_w):
        return convtransp_output_shape(
            h_w,
            kernel_size=(1, self.kernelSize),
            stride=1,
            pad=(0, self.time_padding),
            dilation=1
        )
...
class EEGNet(nn.Module):
    ...
    def __init__(
        ...
        self.output_sizes['conv1'] = convtransp_output_shape(
            (Chans, Samples),
            kernel_size=(1, kernLength1),
            stride=1,
            pad=(0, time_padding)
        ...
        self.output_sizes['depthwise1'] = convtransp_output_shape(
            self.output_sizes['conv1'], kernel_size=(Chans, 1), stride=1, pad=0
        )
        ...
        self.avg_pool_block1 = nn.AdaptiveAvgPool2d(
            (1, int(self.output_sizes['depthwise1'][1] / 4))
        )
        ...
        #block2
        self.separable_block2 = deepwise_separable_conv(
            nin=F1 * D, nout=F2, kernelSize=kernLength2
        )
        ...
        self.avg_pool_block2 = nn.AdaptiveAvgPool2d(
            (1, int(self.output_sizes['separable_block2'][1] / 4))
        )
        self.output_sizes['avg_pool_block2'] = (
            1, int(self.output_sizes['separable_block2'][1] / 4)
        )
    ...
    def get_features_dim(self, Chans, Samples):
        bs = 1
        x = torch.rand((bs, 1, Chans, Samples))
