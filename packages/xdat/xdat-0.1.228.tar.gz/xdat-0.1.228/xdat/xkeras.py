...
class CachedDataGenerator(Sequence):
    ...
    def calc_XYw(self, df, with_tqdm=None):
        ...
        #
        # gather the samples
        samples_raw = xparallel.x_on_iter(df.itertuples(index=False), self._get_sample_wrapper, with_tqdm=with_tqdm, tqdm_total=len(df), n_jobs=self.n_jobs, tqdm_desc=f'calc XYw - {self.mode}')
