...
def movement_stats( list_of_x_coordinates , list_of_y_coordinates , platform_diameter_cm , platform_mask , fps ):
    """
    ...
    """
    platform_diameter_px = platform_mask * 2 #because d=2r
    ...
    print( distance_cm , "cm swum in" , len(xs)/fps , "s")
    return distance_cm , len(xs)/fps
...
def trim_start( list_of_video_frames , pool_mask , rescale_percentile_low , rescale_percentile_hi ):
    ...
    if len(list_of_video_frames) <= 700:
        counts, bins = np.histogram(list_of_video_frames,bins=10)
    elif len(vals) > 400:
        counts, bins = np.histogram(list_of_video_frames[ : np.round( len(list_of_video_frames)*0.5 , 0 ).astype(int) ],bins=10)
    else: pass    
    start = ""
    start_index = 0
    while ( start != "found" ) and ( start_index < len(list_of_video_frames)-1 ):
        ...
        current_val = np.round( list_of_video_frames[ start_index ] , 0 )
        consecutive_vals = np.round( list_of_video_frames[ start_index : start_index+30 ] , 0 )
        bool_list = []
