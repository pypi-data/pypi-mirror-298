...
class MomwhyareyouhereWebsite(HTTPServer):
    def __init__(self, port, root_directory):
        ...
        # Create an instance of the Mom class
        self.mom = self.Mom(self)
    ...
    class Mom:
        ...
        def start_server(self):
            print(f"Serving on port {self.parent.server_address[1]}...")
            self.parent.server_thread.start()

        def stop_server(self):
            print("Stopping the server...")
            self.parent.shutdown()  # Shut down the server
            self.parent.server_close()  # Close the server socket
            self.parent.server_thread.join()  # Wait for the thread to finish
