...
access_token = ""
...
def well_completion():
    payload = ApiPayload(start_date='2021-05-1', end_date='2022-12-25', state_code='CO',
...
def test_ducs_by_operator():
    payload = ApiPayload(start_date='2021-01-01', end_date='2021-01-31', aggregate_by='operator',
...
def test_production_by_county_and_operator():
    payload = ApiPayload(start_date='1929-04-01', end_date='1934-01-01', operator_name='Stephens Production Company',
                         state_code='AR')
...
def test_frac_crews():
    payload = ApiPayload(start_date='2020-01-01', end_date='2024-07-30', aggregate_by="service_company")

    result_df = client.frac_crews(payload)
    print(result_df.count())
    result_df.to_csv('frac_crews.csv', index=False)
