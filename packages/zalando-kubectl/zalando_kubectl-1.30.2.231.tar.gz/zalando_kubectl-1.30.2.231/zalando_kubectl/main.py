...
def request_cluster_access(
    env: Environment, emergency, incident, user, reason, okta, okta_auth_client_id, wait_for_approval, timeout
):
    ...
    if emergency:
        ...
        make_emergency_access_request(
            env, reference_url, user, reason, okta, okta_auth_client_id, wait_for_approval, timeout
        )
    else:
        make_manual_access_request(env, user, reason, okta, okta_auth_client_id, wait_for_approval, timeout)


def make_emergency_access_request(
    env: Environment, reference_url, user, reason, okta, okta_auth_client_id, wait_for_approval, timeout
):
    if okta:
        access_request.create_with_okta(
            env, "emergency", reference_url, " ".join(reason), okta_auth_client_id, wait_for_approval, timeout
        )
        return
...
def make_manual_access_request(
    env: Environment, user_option, reason, okta, okta_auth_client_id, wait_for_approval, timeout
):
    if okta:
        access_request.create_with_okta(
            env, "privileged", None, " ".join(reason), okta_auth_client_id, wait_for_approval, timeout
        )
        return
