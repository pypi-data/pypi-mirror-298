...
def create_with_okta(
    env: Environment, access_type, reference_url, reason, okta_auth_client_id, wait_for_approval, timeout
):
    priv_api = get_privileged_access_api(env.config)
    current_cluster = kube_config.get_context(env)
    if okta_auth_client_id is None:
        _, _, okta_auth_client_id, _ = zalando_kubectl.main.login(env.config, current_cluster)
