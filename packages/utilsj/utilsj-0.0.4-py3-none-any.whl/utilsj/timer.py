...
class MyTimer(object):
    ...
    def log(self, tip='Elapsed time', unit='ms', reset=False, name="", debug=True):
        duration = round(self.elapsed(reset, unit, name=name), 3)
        msg = f'{tip}: {duration:.3f}{unit}'
        if self.logger:
            self.logger.debug(msg) if debug else self.logger.info(msg)
        else:
            print(msg)
        return duration

    def rlog(self, tip='Elapsed time', unit='ms', reset=True, name="", debug=True):
        return self.log(tip=tip, unit=unit, reset=reset, name=name, debug=debug)
