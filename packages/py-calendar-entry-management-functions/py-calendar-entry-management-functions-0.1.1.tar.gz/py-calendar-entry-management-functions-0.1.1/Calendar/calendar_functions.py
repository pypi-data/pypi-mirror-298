...
def load_entries():
    '''Loads entries from the json file or creates a new structure if it doesn't exist.'''
    filename = 'filename.json'
    
    # Check if json file exists, if not create it with default structure
    if not os.path.exists(filename):
        ...
        return default_structure['entries']
...
def save_entry(entries):
    '''Saves entries and stores them in the json file.'''
    filename = 'filename.json'
    
    # Ensure the file is initialized before saving
    if not os.path.exists(filename):
        load_entries()  # This will create the file with the default structure if it doesn't exist
