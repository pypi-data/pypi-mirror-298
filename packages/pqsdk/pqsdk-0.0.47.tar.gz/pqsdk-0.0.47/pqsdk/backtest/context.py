...
class StrategyContext(AbstractStrategyContext):
    ...
    def buy(self,
        ...
        if get_suspend_status(sec_code=stock_code, trade_date=self.current_dt.strftime('%Y-%m-%d')) > 0:
            log.warning(f"股票停牌，取消买入: stock_code={stock_code}, trade_date={self.current_dt.strftime('%Y-%m-%d')}")
            return
    ...
    def sell(self,
        ...
        # 检查股票是否已经停牌
        if get_suspend_status(sec_code=stock_code, trade_date=self.current_dt.strftime('%Y-%m-%d')) > 0:
            log.warning(f"股票停牌，取消卖出: stock_code={stock_code}, trade_date={self.current_dt.strftime('%Y-%m-%d')}")
            return
