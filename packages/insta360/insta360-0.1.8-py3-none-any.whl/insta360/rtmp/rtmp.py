...
class Client:
    ...
    Parameters:
        host: The IP address of the camera (usually always 192.168.42.1).
        port: The port of the camera (usually alwaays 6666).
        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    self.logger.error(
                        "Exception in receive_packet() in thread {name}: {err}"
                        .format(name=thread_name, err=ex)
                    )
                ...
                if self.camera_socket is None:
                    self.logger.debug(
                        'Socket in thread {} disconnected. Polling aborted'
                        .format(thread_name)
                    )
                    break
                ...
                if time.time() - t0 > self.PKT_COMPLETE_TIMEOUT_SEC:
                    self.logger.warning(
                        ...
                        "Discarding buffer: b'{}'"
                        .format(thread_name, bytes_to_hexascii(self.rcv_buffer))
                    )
                    break
