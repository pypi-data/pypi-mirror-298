...
class Database(Storage):
    ...
    def __init__(self, db_uri: str) -> None:
        ...
        self._events: dict[asyncio.Event, list[TableBatchWriter]] = defaultdict(list)
    ...
    def add_entity(
        ...
        if on_event:
            ...
            self._events[on_event].append(writer)
        else:
    ...
    def __await__(self) -> tp.Generator[tp.Any, None, None]:
        yield from self._pool.__await__()
        tasks = (flush_on_event(e, w) for e, w in self._events.items())
...
async def flush_on_event(
        event: asyncio.Event,
        writers: list[TableBatchWriter]
) -> None:
    """
    Wait for an event and flush all database writers when the event is
    triggered.

    :param event: Asyncio event to wait for.
    :param writers: List of writers to flush on event.
    """
    try:
        while True:
            await event.wait()
            items = (w.flush() for w in writers)
            task = asyncio.gather(*items)

            # do nothing on database connection error; next write attempt
            # on next event
            await catch_conn_error(task)
    finally:
        # application is quitting, therefore save data without catching
        # connection errors
        logger.info('flushing database writers for event {}'.format(event))
        items = (w.flush() for w in writers)
