...
class PluginsModel(QStandardItemModel):
    ...
    def createRow(item):
        ...
        if isinstance(item, Installed):
            ...
            dist = None
            # name = ins.label if ins.label else ins.name
            name = ins.name
            summary = ins.summary
            version = ins.version
            item_is_core = False
...
class AddonManagerDialog(QDialog):
    ...
    def __on_add_query_finish(self, f):
        ...
        try:
            ...
            error_text = __("help.query_error")
    ...
    def progressDialog(self):
        ...
        if self.__progress is None:
            ...
            self.__progress.setCancelButtonText(__('btn.cancel'))
    ...
    def __on_installer_finished(self):
        self.__on_installer_finished_common()
        name = __("noun.canvas_core") or 'Orange'
        ...
        if QMessageBox.Ok == message_restart(self):
            ...
            def restart():
                ...
                if any(w.isVisible() for w in windows):  # if a window close was cancelled
                    ...
                    QMessageBox(
                        text=__("msg.restart_cancel"),
                        informativeText=__("msg.restart_cancel_tip").format(name),
                        icon=QMessageBox.Information
                    ).exec()
                else:
