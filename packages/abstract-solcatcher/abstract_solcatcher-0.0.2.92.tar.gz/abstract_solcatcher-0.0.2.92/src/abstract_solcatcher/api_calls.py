...
from abstract_solana import get_rpc_dict,RateLimiter,abstract_solana_rate_limited_call
...
def solcatcher_api_call(endpoint,*args,**kwargs):        
    response,status_code = get_async_response(async_solcatcher_api_call,endpoint, *args, **kwargs):
    if status_code == <500:
        return response
