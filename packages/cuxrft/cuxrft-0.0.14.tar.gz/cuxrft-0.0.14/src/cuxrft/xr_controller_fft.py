...
def sel_chunk(data, chunks, out, resultDim, fftAxis, fftDirection="forward", fftNorm=None, chunkedDims={}, useGPU=0, sc=None, multiple_GPUs=False, FFTOrderCoords={}, debugging=False):
    if (chunkedDims.keys() != chunks.keys()):
        ...
        @dask.delayed
        def calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, debugging=False):
            GPU_avail = {}
            if debugging == True:
            ...
            return fftReturn
        if (sc is not None):
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, debugging=debugging), dtype=np.complex128, shape=out[sc].shape)
        else:
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, debugging=debugging), dtype=np.complex128, shape=out.shape)
        return delayedObject

def fft_cellwise(data, chunks='auto', FFT_dims='', data_vars='', delayed=False, multiple_GPUs=False, GPUs=[0], keepGPUcontrollingServerRunning=False, sel=None, isel=None, debugging=False):
    """
...
def ifft_cellwise(data, chunks='auto', FFT_dims='', data_vars='', delayed=False, multiple_GPUs=False, GPUs=[0], keepGPUcontrollingServerRunning=False, sel=None, isel=None, debugging=False):
    """
