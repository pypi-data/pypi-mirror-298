...
def sel_chunk(data, chunks, out, resultDim, fftAxis, fftDirection="forward", fftNorm=None, chunkedDims={}, useGPU=0, sc=None, multiple_GPUs=False, FFTOrderCoords={}):
    if (chunkedDims.keys() != chunks.keys()):
        ...
        @dask.delayed
        def calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims):
            GPU_avail = {}
            if (multiple_GPUs == False):
                ...
                for key in chunkedDims.keys():
                    print('Calculating chunk ' + str(chunkedDims[key]['chunkIndex']) + ' for ' + key + '.')
            else:
                if (isinstance(useGPU, int)):
            ...
            return fftReturn
        if (sc is not None):
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims), dtype=np.complex128, shape=out[sc].shape)
        else:
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims), dtype=np.complex128, shape=out.shape)
