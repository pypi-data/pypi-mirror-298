...
class Runner():
    ...
    def run(self) -> Dict:
        ...
        if self.__labgrid_tests is True:
            ...
            Utils.do_in_tempdir(self._download_image_in_labgrid_client, current_directory, self.__machine)
            Labgrid.start_coordinator()
            Labgrid.set_env_file(current_directory, self.__machine, self.__image_name)
            Labgrid.set_client_image(current_directory, self.__machine, self.__image_name)
            Labgrid.set_client_run_tests_command(current_directory, self.__machine)
            logging.getLogger(
                    'scotty-test').info('Flashing SD Card with Labgrid client, please wait...')
            Labgrid.start_client(self.__machine)
            Labgrid.reset_client_dockerfile(current_directory, self.__machine)
            _selected_gpio_expander = Labgrid.get_gpio_expander(current_directory, self.__machine)
        else:
        ...
        Utils.do_in_tempdir(self._do_testrun, current_directory, _selected_gpio_expander)
        with open(f'{self.__machine}-{self.__image_name}-{time.time_ns()}-testresults.json', 'w') as o:
            json.dump(self.__results, o)
        if self.__labgrid_tests is True:
            Labgrid.set_client_release_place_command(current_directory, self.__machine)
            logging.getLogger(
                    'scotty-test').info('Releasing place, please wait...')
            output = Labgrid.start_client(self.__machine)
            Labgrid.reset_client_dockerfile(current_directory, self.__machine)
            Labgrid.stop_coordinator(output)
            Labgrid.clean(current_directory, self.__machine, self.__image_name)
    ...
    def _do_testrun(self, current_directory, gpio_expander) -> Dict:
        ...
        if self.__labgrid_tests is False:
            ...
            ip = Labgrid.get_ip_address(current_directory, self.__machine)
        if self.__overlays:
    ...
    def _download_image_in_labgrid_client(self, current_directory, machine):
        _dir = self._unpack_zip(self.__image_dict.get('image'))
        _file = self._find_wic_file(_dir, f'{self.__image_name}-{self.__machine}.wic')
        shutil.copy(_file, f"{current_directory}/labgrid/client-{machine}/")
