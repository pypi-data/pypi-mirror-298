...
from io import TextIOWrapper as _TextIOWrapper, BufferedIOBase as _BufferedIOBase, TextIOBase as _TextIOBase
...
class M3U8Parser: 
    ...
    def __init__(self) -> None:
        ...
        self._encoding = _locale.getencoding()
    ...
    def LoadStream(self, streamOrFilepath:str|_IO):
        fp = None
        streamWrapper = None
        stream = streamOrFilepath
        try:
            if(isinstance(streamOrFilepath, str)):
                stream = fp = open(streamOrFilepath, 'r', encoding=self._encoding)
            elif(isinstance(streamOrFilepath, _BufferedIOBase)):
                stream = streamWrapper = _TextIOWrapper(streamOrFilepath, encoding=self._encoding) #convert from binary to text stream
    ...
    def _LoadStream(self, stream:_TextIOBase):
        stream.seek(0)
        data = stream.read()
    ...
    def SaveStream(self, streamOrFilepath:str|_IO):
        fp = None
        streamWrapper = None
        stream = streamOrFilepath
        try:
            if(isinstance(streamOrFilepath, str)):
                stream = fp = open(streamOrFilepath, 'w', encoding=self._encoding, newline='') #ensure newlines are written as is without translations
            elif(isinstance(streamOrFilepath, _BufferedIOBase)):
                stream = streamWrapper = _TextIOWrapper(streamOrFilepath, encoding=self._encoding, newline='') #convert from binary to text stream
            self._SaveStream(stream)
        finally:
            if(fp): #if we created a new filehandle, then close it
                fp.close()
            if(streamWrapper):
    ...
    def _SaveStream(self, stream: _TextIOBase):
