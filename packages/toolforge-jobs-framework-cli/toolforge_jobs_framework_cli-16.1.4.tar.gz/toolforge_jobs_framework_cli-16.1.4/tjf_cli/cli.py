...
def op_images(api: ToolforgeClient):
    response = api.get("/images/", display_messages=True)
...
def _list_jobs(api: ToolforgeClient):
    response = api.get("/jobs/", display_messages=True)
    jobs = response["jobs"]
    return jobs
...
def op_run(args: argparse.Namespace, api: ToolforgeClient) -> None:
    ...
    api.post("/jobs/", json=payload, display_messages=True)
...
def update_job(args: argparse.Namespace, api: ToolforgeClient) -> None:
    ...
    api.patch("/jobs/", json=payload, display_messages=True)
...
def _show_job(api: ToolforgeClient, name: str, missing_ok: bool):
    try:
        response = api.get(f"/jobs/{name}", display_messages=True)
        job = response["job"]
...
def op_delete(api: ToolforgeClient, name: str):
    api.delete(f"/jobs/{name}", display_messages=True)
    logging.debug("job was deleted")


def op_flush(api: ToolforgeClient):
    api.delete("/jobs/", display_messages=True)
...
def op_restart(api: ToolforgeClient, name: str):
    api.post(f"/jobs/{name}/restart", display_messages=True)
    logging.debug("job was restarted")


def op_quota(api: ToolforgeClient):
    response = api.get("/quotas/", display_messages=True)
    data = response["quota"]
