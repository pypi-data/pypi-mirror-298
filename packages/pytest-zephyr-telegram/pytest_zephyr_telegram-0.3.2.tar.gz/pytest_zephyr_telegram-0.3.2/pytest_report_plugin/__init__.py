...
def pytest_terminal_summary(terminalreporter):
    yield
    with threading_lock:
        if not terminalreporter.config.cache.get("report_stop_run_executed", False):
            if TelegramReporter.enabled:
                ...
                terminalreporter.config.cache.set("report_stop_run_executed", True)


def pytest_sessionstart(session):
    with threading_lock:
        if not session.config.cache.get("report_start_run_executed", False):
