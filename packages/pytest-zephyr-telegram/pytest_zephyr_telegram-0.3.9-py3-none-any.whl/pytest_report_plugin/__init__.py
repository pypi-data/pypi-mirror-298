...
def report_start_and_end_run(request: FixtureRequest, worker_id):
    if not worker_id or (worker_id == "gw0" or worker_id == "master"):
        now = datetime.now(pytz.timezone("Europe/Moscow")).strftime("%d-%m-%Y %H:%M:%S")
        run_name = f"Автотесты {RUN_PREFIX_NAME} {now}"
    ...
    if not worker_id or (worker_id == "gw0" or worker_id == "master"):
        save_test_results(request, worker_id)
...
def save_test_results(request, worker_id):
    if TelegramReporter.enabled:
        failed = request.session.testsfailed
        passed = request.session.testscollected - request.session.testsfailed
        skipped = request.session.testscollected - request.session.testsfailed - request.session.testsfailed
        ...
        if failed != 0:
            ...
            for failed_test in request.session.testsfailed[:list_failed_amount]:
...
def report_run_link():
    if ZephyrReporter.enabled and TelegramReporter.enabled and ZephyrReporter.run_id:
...
def upload_http_logs_to_zephyr(item, test_case_key):
    if ZephyrReporter.enabled and ZephyrReporter.run_id:
...
def telegram_create_run(run_name):
    global RUN_PREFIX_NAME
    message = f"""{emoji.emojize(":rocket:")} Тесты {RUN_PREFIX_NAME} запущены \n<code>{run_name}</code>"""
