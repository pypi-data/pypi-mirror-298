...
def pytest_terminal_summary(terminalreporter):
    yield
    with threading_lock:
        if not terminalreporter.config.cache.get("report_start_run_executed", True):
            if TelegramReporter.enabled:
                ...
                terminalreporter.config.cache.set("report_start_run_executed", True)


@pytest.fixture(scope="session", autouse=True)
def report_start_run(request: FixtureRequest):
    with threading_lock:
        if not request.config.cache.get("report_start_run_executed", True):
            ...
            request.config.cache.set("report_start_run_executed", True)
