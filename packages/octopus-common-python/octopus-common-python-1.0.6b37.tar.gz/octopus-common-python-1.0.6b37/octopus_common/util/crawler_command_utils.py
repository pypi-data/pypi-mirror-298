...
def create_crawler(crawler_name: str, task_id: int):
    ...
    # 读取模板文件内容
    template_file_path = os.path.join(current_dir, "..", "model", "crawler_template.py")
    with open(template_file_path, 'r', encoding='utf-8') as template:
        crawler_content = template.read()

    # 替换模板中的占位符
    crawler_content = crawler_content.replace("TemplateBaiduCrawler", camel_case_name)
