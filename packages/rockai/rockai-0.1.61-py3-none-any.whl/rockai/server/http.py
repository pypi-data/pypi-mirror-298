...
def create_app(
    ...
    request_counter = Counter(
        "http_prediction_requests_total", "Total number of HTTP prediction requests"
    )
    # Create a Gauge to track QPM
    qpm_gauge = Gauge("http_requests_qpm", "Number of HTTP requests per minute")

    # Function to update QPM every second
    def update_qpm():
        while True:
            time.sleep(60)  # Update every second
            qpm_gauge.set(request_counter._value.get())  # Set QPS to the current count
            request_counter.clear()  # Reset the counter for the next interval

    # Start the QPS update thread
    threading.Thread(target=update_qpm, daemon=True).start()
