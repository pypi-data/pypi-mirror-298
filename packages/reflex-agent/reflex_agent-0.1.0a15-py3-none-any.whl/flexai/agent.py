...
class Agent:
    ...
    def is_nonempty_list_of_class(self, content, cls) -> bool:
        """Check if the content is a nonempty list of classes.
        ...
        Returns:
            Whether the content is a nonempty list of the class.
        """
        return (
    ...
    async def modify_messages(self, messages: list[Message]) -> list[Message]:
        ...
        for idx in user_idxs[-2:]:
            if isinstance(messages[idx].content, str):
                messages[idx].content = [TextBlock(text=messages[idx].content)]
            messages[idx].content[0].cache = True
        return messages

    async def get_system_message(self) -> System:
        ...
        return [TextBlock(text=system, cache=True)]
