...
class Update(QueryBuilderBase):
    ...
    def set_value(self, column_name: Column | str, value):
        if isinstance(column_name, Column):
            column_name = column_name.identifier
        if self._table.columns and column_name not in self._table.columns_map:
            raise QueryBuilderException(f'Column "{column_name}" does not exist on table {self._table.identifier}')
