...
class ComponentNode:
    ...
    def get_output_name_from_input(self, input_name: str, upstream_node: "ComponentNode") -> str:
        mapping = self._node_in_args[upstream_node]

        (output_name,) = (out_name for out_name, in_name in mapping.items() if in_name == input_name)
    ...
    def is_mandatory(self, input_name: str) -> bool:
        return self.get_input_socket(input_name).is_mandatory

    def is_variadic(self, input_name: str) -> bool:
        return self.get_input_socket(input_name).is_variadic

    def is_greedy(self, _input_name: Optional[str] = None) -> bool:
        return getattr(self.get_component(), "__haystack_is_greedy__", False)
    ...
    def get_senders_with_names(self, input_name: str) -> List[Tuple["ComponentNode", str]]:
        return [
            (node, out_name)
            for node, mapping in self._node_in_args.items()
