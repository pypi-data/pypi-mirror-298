...
from .constants import DISABLE_OUTPUT
...
class Detection(ABC):
    ...
    def _load_detection(identifier: str, config: Mapping) -> ModuleType:
        ...
        with ExecutionContextManager(disable_output=DISABLE_OUTPUT, patch_logger=DISABLE_OUTPUT):
    ...
    def _run_command(self, function: Callable, event: Mapping, expected_type: Any) -> Any:
        with ExecutionContextManager(
            disable_output=self.suppress_output, patch_logger=self.suppress_output
        ):
