...
class Cloudtrail(o7lib.aws.base.Base):
    ...
    def conformity_report(self, report : o7util.report.Report = None):
        ...
        if report is None:
            report = o7util.report.Report('Account Conformity Report', section_name="Cloud Trail")
        else: report.add_section("Cloud Trail")
        ...
        if len(trailist) < 1:
            report.test_fail("No Trails for this account (recommend name: account-global-trail)")
            return False
        ...
        for trail in resp['trailList'] :
            if trail['IsMultiRegionTrail'] is True and trail['IncludeGlobalServiceEvents'] is True :
                global_trail = trail
        ...
        report.add_test(name="S3 bucket into which CloudTrail delivers your trail files", critical=True)
