...
class SecurityHub(o7lib.aws.base.Base):
    ...
    def load_findings(self, standard_arn : str = None):
        ...
        res_filter = df['ResType'].isin([
            ...
            'AwsApiGatewayV2Route', 'AwsElastiCacheCacheCluster', 'AwsApiGatewayStage',
            'AwsLogsLogGroup',
            'AwsRdsDbInstance', 'AwsRdsDbCluster',
            'AwsAccount'
        ...
        res_filter = df['ResType'].isin([
            'AwsCodeBuildProject',
            'AwsEcsTaskDefinition', 'AwsEcsCluster', 'AwsEcrRepository', 'AwsEcsService',
            'AwsEc2NetworkAcl', 'AwsEc2Volume', 'AwsEc2SecurityGroup', 'AwsCloudTrailTrail',
            'AwsEc2Instance',
            'AwsCloudFrontDistribution',
            'AwsIamPolicy', 'AwsIamRole', 'AwsIamUser', 'AwsIamGroup',
        ...
        df['Suppression_Reason'] = df.apply(lambda x: x['Resources'][0].get('Tags', {}).get(x['Suppression_Tag'], ''), axis=1)
    ...
    def suppress_flagged_finddings(self):
        ...
        #-------------------------------------
        df = self.df_findings
        df = df[(df['WorkflowStatus'].isin(['NEW', 'NOTIFIED'])) & (df['Suppression_Reason'] != '')]

        for index, row in df.iterrows():
            print(f"Suppressing {row['ControlId']} - {row['ResName']} - {row['Suppression_Reason']}")
            self.client.batch_update_findings(
                FindingIdentifiers=[
                    {
                        'Id': row['Id'],
                        'ProductArn': row['ProductArn']
                    },
                ],
        ...
        #-------------------------------------
        df = self.df_findings
        df = df[(df['WorkflowStatus'] == 'SUPPRESSED') & (df['Suppression_Reason'] != '')  & (df['Suppression_Reason'] != df['WorkflowNote'])]

        for index, row in df.iterrows():
            print(f"Correctinf {row['ControlId']} - {row['ResName']} - {row['Suppression_Reason']}")
    ...
    def write_all_pdf_report_standards(self, folder : str = None) -> list[str]:
        ...
        for index, standard in self.df_standards.iterrows():
            ...
            ret.append(self.write_pdf_report_standards(standard_short=short, folder=folder))
    ...
    #
    #*************************************************
    def write_pdf_report_standards(self, standard_short : str = None, folder : str = None) -> str:
        """Save HTML report for standards"""
        ...
        if standard_short is not None:
            standard = self.df_standards[self.df_standards['Short'] == standard_short].iloc[0]
        else:
    ...
    #
    #*************************************************
    def read_history_from_dynamodb(self, table_name : str, number_days : int = 10) -> pd.DataFrame:
