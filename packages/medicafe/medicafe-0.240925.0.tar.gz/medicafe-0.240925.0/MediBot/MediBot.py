...
class ExecutionState:
    def __init__(self, config_path, crosswalk_path) -> None:
        try:
            ...
            crosswalk_update(self.api_client, config, crosswalk)  # Ensure crosswalk_update uses the instance. TODO We need an internet-disabled version of this.
            MediLink_ConfigLoader.log("Crosswalk update complete...", level="INFO")
            print("Crosswalk update complete...")
    ...
    def verify_config_type(self, config):
        MediLink_ConfigLoader.log("Verifying configuration type: {}".format(type(config)), level="INFO")
