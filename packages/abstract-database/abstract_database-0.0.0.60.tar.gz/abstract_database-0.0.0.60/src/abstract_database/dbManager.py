...
class DatabaseManager:
    def __init__(self,env_path=None,dbType=None,dbName=None):
        ...
        self.dbType = dbType or 'abstract'
        self.dbName = dbName or 'ai'
        self.connectionManager = connectionManager(env_path=self.env_path,dbType=self.dbType,dbName=self.dbName)
        self.dbname = self.connectionManager.dbname
        self.user = self.connectionManager.user
        self.password = self.connectionManager.password
        self.host = self.connectionManager.host
        self.port = self.connectionManager.port
        self.table_config_path = get_env_value(key=f"{self.dbType}_{self.dbName}_table_configuration_file_path", path=self.env_path)
...
"""
    ...
    def generate_query_from_recent_response(self,database_query, file_path):
...
def generate_query_from_recent_response(database_query,file_path=None,db_mgr=None,env_path =None):
        """Generate a query based on the most recent response and save the result to an Excel file."""
