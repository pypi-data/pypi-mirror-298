...
class Document:
    def __init__(self, data: Any | list[Any], base_url: str | None = None):
        self.base_url = base_url
        self.data = data

    @classmethod
    def from_html(cls, content: str, base_url: str | None = None) -> Self:
        data = lxml.html.fromstring(content, base_url=base_url)
        return cls(data, base_url)
    ...
    def xpath(self, expr: str, merge: bool = True) -> Self:
        if self.is_empty:
            return self
        data = _apply(self.data, lambda et: et.xpath(expr), merge=merge)
        return Document(data)
