...
class SpiHitfinder:
    ...
    def __init__(self, modules, mode="adaptive", snr=4.0, min_scores=100,
                 ...
                 miss_fraction=1, miss_fraction_base="hit",
    ...
    def _read_litpixels(self, src, litfrm_src=None):
        ...
        if self.xgm_norm and litfrm_src is not None:
            ...
            en[np.isnan(en)] = en_mean
            hitscore = hitscore * en_mean / en
    ...
    def _compute_threshold_adaptive(self):
        ...
        self.mu = np.zeros(self.num_trains, float)
        self.sig = np.zeros(self.num_trains, float)

        for trn_no, (evt0, nevt) in enumerate(zip(self.first, self.count)):
            if nevt > self.min_scores:
                chunk = self.hitscore[evt0:evt0 + nevt]
            elif self.num_events <= self.min_scores:
                chunk = self.hitscore
            else:
                i0 = max(0, evt0 + (nevt - self.min_scores) // 2)
                iN = min(i0 + self.min_scores, self.num_events)
                chunk = self.hitscore[i0:iN]

            q1, self.mu[trn_no], q3 = np.quantile(chunk, [0.25, 0.5, 0.75])
            self.sig[trn_no] = (q3 - q1) / 1.34896
    ...
    def _select_hits(self):
        ...
        self.hit_mask = (self.hitscore > threshold)
        # channel keys
        self.data_keys["hitFlag"] = self.hit_mask
    ...
    def _select_misses(self):
        ...
        ix = np.random.choice(
            np.flatnonzero(~self.hit_mask),
            int(num_misses * self.miss_fraction + 0.5)
        )
