...
def test_dwmemio():
    ...
    ptr, epd = f_cunitepdread_epd(EPD(Db(4)))
    test_equality("f_cunitread_epd -> 0 values", [ptr, epd], (0, 0))
    oldcp = f_getcurpl()
    f_dwwrite(0x6509B0, EPD(Db(i2b4(c1))))
    ptr, epd = f_cunitepdread_cp(0)
    f_setcurpl(oldcp)
    test_equality("f_cunitread_cp %X" % c1, [ptr, epd], [cptr, cepd])
    f_dwwrite(0x6509B0, EPD(Db(4)))
    ptr, epd = f_cunitepdread_cp(0)
    f_setcurpl(oldcp)
    test_equality("f_cunitread_cp -> 0 values", [ptr, epd], (0, 0))
