...
def image_to_2d_array_mode_resize(image_path, resize_shape):
    ...
    # Trim the image to be a multiple of the block size
    trimmed_height = block_size_y * new_height
    trimmed_width = block_size_x * new_width
    img_arr = img_arr[:trimmed_height, :trimmed_width, :]

    # Reshape and compute the mode for each block
    img_arr = img_arr.reshape(new_height, block_size_y, new_width, block_size_x, 3)
    img_arr = img_arr.swapaxes(1,2)
    img_arr = img_arr.reshape(-1, block_size_y * block_size_x, 3)

    # Compute mode along the block axis for each color channel
    mode_data = []
    for channel in range(3):
        channel_data = img_arr[:, :, channel]
        mode_channel, _ = stats.mode(channel_data, axis=1)
        mode_data.append(mode_channel.flatten())

    # Stack the channels back together
    mode_data = np.stack(mode_data, axis=-1).astype(np.uint8)
    mode_img_arr = mode_data.reshape(new_height, new_width, 3)

    # Convert RGB values to a single integer
    mode_img_arr_int = (mode_img_arr[:, :, 0].astype(np.uint32) << 16) | \
                       (mode_img_arr[:, :, 1].astype(np.uint32) << 8) | \
                       mode_img_arr[:, :, 2].astype(np.uint32)
    return mode_img_arr_int

def get_transmittance_center_of_modes_upper_v2(img_path, seg_path, kernel_size, binary_type="brightness", model="tcm", type="q2", resize_shape=None):
    if model == "tcm":
        # Use the dominant value resizing method
        ...
        if resize_shape is not None:
            ...
            # Stack the channels back together
            mode_data = np.stack(mode_data, axis=-1).astype(np.uint8)
            img = Image.fromarray(mode_data.reshape(new_height, new_width, 3))
        ...
        # Use the dominant value resizing method for the segmentation image
        img_arr = image_to_2d_array_mode_resize(seg_path, resize_shape)
        height = img_arr.shape[0]
        ...
        if len(veg_b_value) == 0 or len(sky_b_value) == 0:
            # Handle cases where there are no vegetation or sky pixels
            veg_peak_value = np.min(b_value)
            sky_peak_value = np.max(b_value)
        else:
            veg_hist, veg_bin_edges = np.histogram(veg_b_value, bins=bins_veg)
            veg_peak_bin = np.argmax(veg_hist)
            veg_peak_value = veg_bin_edges[veg_peak_bin] + bin_width / 2
        ...
        # Compute the mean over the neighborhood using cv2.blur
        mean_array = cv2.blur(array_binary.astype(float), (kernel_size, kernel_size))
        ...
        # Use the dominant value resizing method
        img_arr = image_to_2d_array_mode_resize(img_path, resize_shape)
        array_2d = img_arr.astype(np.uint8)
        ...
        img_arr = image_to_2d_array(seg_path, resize_shape=resize_shape)
        sky_indices = img_arr == 4620980
        veg_indices = img_arr == 7048739
