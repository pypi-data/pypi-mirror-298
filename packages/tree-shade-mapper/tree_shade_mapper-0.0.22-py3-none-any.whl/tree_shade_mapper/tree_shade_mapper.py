...
def calc_transmittance(base_dir, models=["tcm"], image_size = (1024, 2048), calc_type = None):
...
def process_location(location, model, res_dir, seg_dir, tra_dir, bin_dir, kernel_size, binary_type):
    ori_path = os.path.join(res_dir, location + ".jpg")
    seg_path = os.path.join(seg_dir, location + "_colored_segmented.png")
    tra_path = os.path.join(tra_dir, location + "_tra.npy")
    bin_path = os.path.join(bin_dir, location + "_bin.npy")
    ...
    if os.path.exists(tra_path) and os.path.exists(bin_path):
        array_binary = np.load(bin_path)
        array_transmittance = np.load(tra_path)
    else:
        array_transmittance, array_binary = get_transmittance_center_of_modes_upper_v2(
            ori_path, seg_path, kernel_size, binary_type=binary_type, model=model, type='q2')
        np.save(bin_path, array_binary)
        np.save(tra_path, array_transmittance)
    ...
    return [location, sky_view_factor]

def process_frame_row(row, model, ori_dir, seg_dir, tra_dir, bin_dir, kernel_size, binary_type):
    ori_path = os.path.join(ori_dir, row["frame_key"] + ".jpg")
    seg_path = os.path.join(seg_dir, row["frame_key"] + "_colored_segmented.png")
    tra_path = os.path.join(tra_dir, row["frame_key"] + "_tra.npy")
    bin_path = os.path.join(bin_dir, row["frame_key"] + "_bin.npy")
...
def calc_transmittance_v2(base_dir, models=["tcm"], image_size=(2048, 1024), calc_type=None, kernel_size=40, binary_type="brightness"):
    import multiprocessing

    # Segmentation
    ori_dir = os.path.join(base_dir, "original")
    res_dir = os.path.join(base_dir, "resized")
    if not os.path.exists(res_dir):
        os.makedirs(res_dir)
    resize_dir(ori_dir, res_dir, new_size=image_size)
    ...
    if calc_type is None:
        locations = [pano.replace(".jpg", "") for pano in os.listdir(res_dir) if pano.endswith(".jpg")]

        df_svf_list = []
        for model in models:
            # Prepare directories for results
            tra_dir = os.path.join(base_dir, f"transmittance_{model}")
            if not os.path.exists(tra_dir):
                os.makedirs(tra_dir)
            bin_dir = os.path.join(base_dir, f"binary_{model}")
            if not os.path.exists(bin_dir):
                os.makedirs(bin_dir)

            # Create a partial function with fixed arguments
            partial_process_location = partial(
                process_location,
                model=model,
                res_dir=res_dir,
                seg_dir=seg_dir,
            ...
            with Pool(processes=cpu_count()) as pool:
                results = pool.map(partial_process_location, locations)

            # Collect SVF values
            svf_list = results  # results is a list of [location, sky_view_factor]
            df_svf = pd.DataFrame(svf_list, columns=['location', f'svf_{model}'])
            df_svf.set_index('location', inplace=True)
            df_svf_list.append(df_svf)
        ...
        df_path = os.path.join(base_dir, "frames.csv")
        df_frames = pd.read_csv(df_path)
        ...
        for model in models:
            ...
            # Create a partial function with fixed arguments
            partial_process_frame_row = partial(
                process_frame_row,
                model=model,
                ori_dir=ori_dir,
                seg_dir=seg_dir,
            ...
            rows = [row for _, row in df_frames.iterrows()]

            # Use multiprocessing Pool
            with Pool(processes=cpu_count()) as pool:
                results = pool.map(partial_process_frame_row, rows)

            # Collect SVF values
            df_frames[f"svf_{model}"] = results

    df_svf_path = os.path.join(base_dir, "frames_svf.csv")
    df_frames.to_csv(df_svf_path)
...
def get_tree_shade(base_dir, time_start, time_end, interval, time_zone, latitude, longitude, models=['tcm'], image_size = (1024, 2048), calc_type=None, vmin=0, vmax=1000, resolution=14):
    calc_transmittance_v2(base_dir, models=models, image_size = (1024, 2048), calc_type=calc_type)
