...
class AssistantModel:
    ...
    def chat_completions(
        ...
        if model is None:
            ...
            raise ValueError(f"Invalid model. Valid options are `gpt-4o` and `claude-3-5-sonnet`")
        
        if stream:
            return self._chat_completions_streaming(messages=messages,  model=model, filter=filter)
        else:
            return self._chat_completions_single(messages=messages, model=model, filter=filter)

    def _chat_completions_single(
        self, messages: List[Message], model: str = "gpt-4o", filter: dict[str, any] = None
    ) -> ChatResultModel:
        messages = [ChatContext(role=ctx.role, content=ctx.content) for ctx in messages]
        context = ChatRequest(messages=messages, filter=filter, model = model) if filter else ChatRequest(messages=messages, model = model)
    ...
    def _chat_completions_streaming(
        self, messages: List[Message], model: str = "gpt-4o", filter: Optional[dict[str, any]] = None
