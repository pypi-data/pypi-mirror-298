...
class TopkRetriever(BaseRetriever):
    ...
    def __init__(self,
                 ...
                 cache=None,
                 index_from_test=False
                 ) -> None:
        super().__init__(dataset_reader, ice_separator, ice_eos_token, prompt_eos_token, ice_num, index_split,
                         test_split, accelerator)
        self.cache = cache
        self.index_from_test = index_from_test
    ...
    def knn_search(self, ice_num):
        cache = f'{self.cache}_retrieve.pkl'
        if os.path.exists(cache):
            with open(cache, "rb") as f:
                logger.info("Loading retrieval results from cache...")
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            if not self.index_from_test:
                near_ids = self.index.search(embed, ice_num)[1][0].tolist()
            else:
                # 第一个是自身，避免召回
                near_ids = self.index.search(embed, ice_num+1)[1][0].tolist()[1:]
            rtr_idx_list[idx] = near_ids

        with open(cache, "wb") as w:
            pickle.dump(rtr_idx_list, w, protocol=4)
            logger.info("Saving retrieval results to disk...")
        return rtr_idx_list

    def forward(self, dataloader, process_bar=False, information=''):
        res_list = []
        # 会有两次 call forward
        # 一次是 encode the index set; 一次是encode of the test set
        if "test" not in information:
            cache = f"{self.cache}_index.pkl"
            if os.path.exists(cache):
                with open(cache, "rb") as f:
                    logger.info("Loading index encoding from cache...")
                    return pickle.load(f)
        ...
        if "test" not in information:
            with open(cache, "wb") as w:
                pickle.dump(res_list, w, protocol=4)
                logger.info("Saving index encoding to disk...")
        return res_list
