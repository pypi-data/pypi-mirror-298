...
def _delvewheel_patch_1_8_2():
    ...
    import platform
    import sys
    libs_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, 'soar_sml.libs'))
    is_conda_cpython = platform.python_implementation() == 'CPython' and (hasattr(ctypes.pythonapi, 'Anaconda_GetVersion') or 'packaged by conda-forge' in sys.version)
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        if os.path.isdir(libs_dir):
            os.add_dll_directory(libs_dir)
    else:
        load_order_filepath = os.path.join(libs_dir, '.load-order-soar_sml-9.6.4.202409280213.dev28')
        if os.path.isfile(load_order_filepath):
            import ctypes.wintypes
            with open(os.path.join(libs_dir, '.load-order-soar_sml-9.6.4.202409280213.dev28')) as file:
...
class Agent(ClientErrors):
    ...
    def KillDebugger(self, ignoreNonExistent=False):
        r"""
        KillDebugger(Agent self, bool ignoreNonExistent=False) -> bool
        ...
        """
        return _Python_sml_ClientInterface.Agent_KillDebugger(self, ignoreNonExistent)
