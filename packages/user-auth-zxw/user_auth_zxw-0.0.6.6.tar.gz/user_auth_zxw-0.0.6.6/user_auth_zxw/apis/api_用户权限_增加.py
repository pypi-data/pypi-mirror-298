...
async def add_new_role(request: 请求_分配或创建角色, db: AsyncSession):
    """
    本函数主要作用：给用户分配角色,
                如果用户不存在则返回404,
                如果角色不存在则创建角色,
...
async def __分配或创建角色__(request: 请求_分配或创建角色, db: AsyncSession = Depends(get_db)):
    return await add_new_role(request, db)
