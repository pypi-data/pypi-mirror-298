...
def complete_with_ext(original_path, filenames):
    """
...
def reset_state(session_state, defaults, but=()):
    """
...
def set_state_for(key, value, session_state):
    """
...
def cycle_fill(lst, expected_length):
    """
...
def get_colormap(keys, colors):
    ...
    """
    colors = cycle_fill(colors, len(keys))
...
def render_html(html_filepath, **kwargs):
    """
    Render HTML file
