...
def write_voc(tree, destination):
    """
...
def write_yolo(instances, destination):
    """
    :param instances: A list of YOLO object detection instance data (list of lists)
    :param destination: File path where the YOLO formatted data should be written.
    :return: None
    """
...
def empty_yolo(image_filename, path):
    """
    ...
    :return: A tuple containing an empty list representing an empty YOLO label and the destination path for the label file.
    """
...
def empty_voc(image_filename, path):
    """
...
def check_voc(voc):
    ...
    def check_subtags(_root, necessary):
        return all(
            _root.find(_tag) is not None and (_root.find(_tag).text is not None or len(_root.find(_tag)) > 0) for _tag
            in necessary)
    ...
    if not check_subtags(root, ['folder', 'filename', 'size']):
        return False
    ...
    for obj in objects:
        ...
        if not all((0 <= xmin < xmax <= width, 0 <= ymin < ymax <= height)):
            return False

    return True


def check_yolo(yolo, labelmap=None):
    """
...
def check_yolo_empty(yolo):
    """
    :param yolo: Path to the file that needs to be checked.
...
def check_voc_empty(voc):
    """
    :param voc: Path to the VOC (Visual Object Classes) XML file that needs to be checked.
