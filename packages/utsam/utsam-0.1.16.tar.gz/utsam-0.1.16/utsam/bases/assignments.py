...
class AssignmentMarking(AssignmentDir):
    ...
    def plot_original_hist_by_tutor(self, employee_col_name='employee_first_name', bins=20):
        return alt.Chart(self.student_marks, title=f'Histogram of original marks by tutor').mark_bar(
            opacity=0.3,
            binSpacing=0
        ).encode(
            alt.X(f'{PgAssignmentCriteriaStudentScoresTable.criteria_student_score}:Q', bin = alt.BinParams(maxbins=20)),
            alt.Y('count()'),
            facet=alt.Facet(f'{employee_col_name}:N', columns=1),
            tooltip=[
                f'{employee_col_name}:N', 
                alt.Tooltip(f'{PgAssignmentCriteriaStudentScoresTable.criteria_student_score}:Q', bin = alt.BinParams(maxbins=bins)),
                'count()',
            ],
        ).resolve_axis(
            x='independent',
    ...
    def plot_new_hist_tutors(self, employee_col_name='employee_first_name', bins=20):        
        return alt.Chart(self.student_marks, title=f'Histogram of standardised marks by tutor').mark_bar(
            opacity=0.3,
            binSpacing=0
        ).encode(
            alt.X(f'{self.new_score_col}:Q', bin = alt.BinParams(maxbins=bins)),
            alt.Y('count()'),
            alt.Color(f'{employee_col_name}:N')
        )
