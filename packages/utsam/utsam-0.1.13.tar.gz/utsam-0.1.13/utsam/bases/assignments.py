...
class AssignmentMarking(AssignmentDir):
    def __init__(
        ...
        self.original_student_marks: pd.DataFrame = None
        self.new_student_marks: pd.DataFrame = None
    ...
    def aggregate_at_mark(self, df, score_col, weight_col, result_col):
        temp_df = df.copy()
        temp_df[result_col] = temp_df[score_col] * temp_df[weight_col]
        temp_df.groupby(self.grouping_cols).agg({result_col: 'sum'}).reset_index()
        return temp_df
    ...
    def set_score_and_grade(self):
        if len(self.all_new_marks_df) > 0:
            ...
            return temp_df.groupby(self.grouping_cols).agg({col_name:'sum' for col_name in result_cols}).reset_index().sort_values([PgStudentsTable.student_name])
