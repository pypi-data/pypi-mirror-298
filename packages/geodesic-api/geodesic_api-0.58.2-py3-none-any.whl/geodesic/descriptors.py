...
class _GeometryDescr(_BaseDescr):
    ...
    Args:
        ...
        geometry/bbox in sync on objects where the bbox is always derived from a geometry field.
...
class _BBoxDescr(_GeometryDescr):
    ...
    Methods:
        ...
        __set__: coerces the input into a shapely geom before taking the bounds and stores internally to the APIObject
                 dict
    """
        ...
        if isinstance(value, str):
            if not self.rfc3339_regex.match(value):
                try:
                    ...
                    raise ValueError(
                        f"{self.public_name} is not an RFC3339 formatted datetime string and not parsable"
                    ) from e
        elif isinstance(value, datetime.datetime):
            # Convert to UTC
            try:
...
class _URLDescr(_StringDescr):
    ...
    Inherits from :class:`_StringDescr<geodesic.descriptors._StringDescr>` for all methods except validate.
