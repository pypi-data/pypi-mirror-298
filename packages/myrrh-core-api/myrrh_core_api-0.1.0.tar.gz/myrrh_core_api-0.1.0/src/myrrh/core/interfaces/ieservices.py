...
class IEServiceVendor(IEService): ...


class IEServiceSystem(IEService): ...


class IEServiceHost(IEService): ...


class EServiceGroup(enum.Enum):
    vendor = IEServiceVendor
    system = IEServiceSystem
    host = IEServiceHost
    ...
    def __call__(cls, name: str):
        ...
        def __new__(cls_):
            ...
            d_.pop("__dict__")  # remove TypeError: descriptor '__dict__' for 'A' objects doesn't apply to 'B' object
...
class IEStreamService(abc.ABC):
    ...
    @abc.abstractmethod
    def open_process(
        self, path: str, wiring: int, args: list[str], working_dir: str | None = None, env: dict[str, str] | None = None, *, extras: dict | None = None
...
class IBaseItem(abc.ABC):
    @property
    @abc.abstractmethod
    def DOM(self) -> datetime.datetime | None:
        return None
...
class IERegistry(dict, abc.ABC):
    ...
    @abc.abstractmethod
    def values(self) -> list[IBaseItem]: ...  # type: ignore[name-defined, override]

    @abc.abstractmethod
    def get(self, path: str, default: IBaseItem): ...  # type: ignore[override]
