...
def __(key):
    return i18n.t("widget.data.data.owunique." + key)
...
class OWUnique(widget.OWWidget):
    ...
    TIEBREAKERS = {__("checkout.last"): itemgetter(-1),
                   __("checkout.first"): itemgetter(0),
                   __("checkout.middle"): lambda seq: seq[len(seq) // 2],
                   __("checkout.random"): np.random.choice,
                   __("checkout.discard"):
    ...
    def __init__(self):
        ...
        var_list = gui.listView(
            self.controlArea, self, "selected_vars", box=__("box.group_by"),
        ...
        gui.comboBox(
            ...
            label=__("label.tiebreaker"),
            items=(__("checkout.last"), __("checkout.first"), __("checkout.middle"), __("checkout.random"),
                   __("checkout.discard")),
            callback=self.commit.deferred, sendSelectedValue=True)
        gui.auto_commit(
            self.controlArea, self, 'autocommit', __("btn_commit"),
