...
def __(key):
    return i18n.t("widget.data.data.owmelt." + key)


DEFAULT_ITEM_NAME = __("row.item")
DEFAULT_VALUE_NAME = __("row.value")
DEFAULT_NAME_FOR_ROW = __("row.row")
...
class OWMelt(widget.OWWidget):
    ...
    class Inputs:
        data = widget.Input("Data", Table, label=i18n.t("widget.data.data.common.data"))

    class Outputs:
        data = widget.Output("Data", Table, label=i18n.t("widget.data.data.common.data"))

    class Information(widget.OWWidget.Information):
        no_suitable_features = Msg(__("msg_no_suitable_features"))
    ...
    def __init__(self, *args, **kwargs):
        ...
        box = gui.widgetBox(self.controlArea, __("box.unique_row"))
        self.idvar_model = itemmodels.VariableListModel(
            [None], placeholder=__("placeholder.idvar_model"))
        self.var_cb = gui.comboBox(
            box, self, "idvar", model=self.idvar_model,
            callback=self._invalidate, minimumContentsLength=16,
            tooltip=__("box.unique_row_tip"))

        box = gui.widgetBox(self.controlArea, __("box.filter"))
        gui.checkBox(
            box, self, "only_numeric", __("checkbox.only_numeric"),
            callback=self._invalidate)
        gui.checkBox(
            box, self, "exclude_zeros", __("checkbox.exclude_zeros"),
            callback=self._invalidate,
            tooltip=__("checkbox.exclude_zeros_tip"))

        form = QFormLayout()
        gui.widgetBox(
            self.controlArea, __("box.names"), orientation=form)
        form.addRow(__("row.item"),
                    gui.lineEdit(
                        None, self, "item_var_name",
                        callback=self._invalidate,
                        placeholderText=DEFAULT_ITEM_NAME,
                        styleSheet="padding-left: 3px"))
        form.addRow(__("row.value"),
                    gui.lineEdit(
    ...
    def send_report(self):
        self.report_items(__("report.settings"), (
            (__("report.row_identifier"), self.controls.idvar.currentText()),
            (__("report.numeric_features"), bool_str(self.only_numeric)),
            (__("report.exclude_zero_values"), bool_str(self.exclude_zeros))
        ))
        if self._output_desc:
            self.report_items(__("report.output"), self._output_desc)

    def _store_output_desc(self, output):
        self._output_desc = {
            __("report.item_column"): output.domain.attributes[1].name,
            __("report.value_column"): output.domain.class_var.name,
            __("report.number_items"): len(output)
        }
