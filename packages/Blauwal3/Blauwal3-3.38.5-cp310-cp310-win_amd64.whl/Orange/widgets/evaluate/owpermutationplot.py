...
def __(key):
    return i18n.t("widget.evaluate.evaluate.owpermutationplot." + key)
N_FOLD = 7
...
class PermutationPlot(PlotWidget):
    def __init__(self):
        ...
        self.setLabel(axis="bottom", text=__("box_label"))
...
class OWPermutationPlot(OWWidget, ConcurrentWidgetMixin):
    ...
    class Error(OWWidget.Error):
        ...
        unknown_err = Msg("{}")
        not_enough_data = Msg(i18n.t('widget.evaluate.evaluate.wpermutationplot.msg.not_enough_data', n_fold=N_FOLD))
    ...
    def _add_controls(self):
        box = gui.vBox(self.controlArea, __("box_set"))
        gui.spin(box, self, "n_permutations", label=__("box_permutationsl"),
                 minv=1, maxv=1000, callback=self._run)
        gui.rubber(self.controlArea)

        box = gui.vBox(self.controlArea, __("box_info"))
        self._info = gui.label(box, self, "", textFormat=Qt.RichText,
                               minimumWidth=180)
        self.__set_info(None)

    def __set_info(self, result: PermutationResults):
        html = __("msg.no_data_available")
...
<table width=100% align="center" style="font-size:11px">
    ...
    def send_report(self):
        if self._data is None or self._learner is None:
            return
        self.report_items(i18n.t("common.software.setting"), [(i18n.t("common.general.permutations"), self.n_permutations)])
        self.report_raw(i18n.t("common.software.info"), self._info.text())
        self.report_name(i18n.t("common.software.plot"))
        self.report_plot()
