...
class Task(namespace):
    ...
    preprocessed_data = None        # type: Optional[Table]normalize = None  # type: Optional[bool]
    normalized_data = None          # type: Optional[Table]

    use_pca_preprocessing = None    # type: Optional[bool]pca_components = None  # type: Optional[int]
    pca_projection = None  # type: Optional[Table]
    distance_metric = None          # type: Optional[str]    perplexity = None  # type: Optional[float]
    multiscale = None  # type: Optional[bool]
    exaggeration = None  # type: Optional[float]initialization_method = None    # type: Optional[str]
    ...
    tsne = None  # type: Optional[manifold.TSNE]# `effective_data` stores the current working matrix which should be used
...
class OWtSNE(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    def _add_controls_start_box(self):
        self.preprocessing_box = gui.vBox(self.controlArea, box=__("preprocessing"))
        self.normalize_cbx = gui.checkBox(
            self.preprocessing_box, self, "normalize", __("normalize_data"),
            callback=self._normalize_data_changed, stateWhenDisabled=False,
        )
        self.pca_preprocessing_cbx = gui.checkBox(
            self.preprocessing_box, self, "use_pca_preprocessing", __("apply_pca"),
            callback=self._pca_preprocessing_changed, stateWhenDisabled=False,
        )
        self.pca_component_slider = gui.hSlider(
            self.preprocessing_box, self, "pca_components", label=__("pca_components"),
            minValue=2, maxValue=_MAX_PCA_COMPONENTS, step=1,
            callback=self._pca_slider_changed,
        )

        self.parameter_box = gui.vBox(self.controlArea, box=__("parameters"))
        form = QFormLayout(
        ...
        form.addRow(__("initialization"), self.initialization_combo)
        ...
        form.addRow(__("distance_metric"), self.distance_metric_combo)
        ...
        form.addRow(gui.checkBox(
            self.controlArea, self, "multiscale", label=__("row.global_structure"),
        ...
        self.run_button = gui.button(
            self.parameter_box, self, __("btn.start"), callback=self._toggle_run
        )
