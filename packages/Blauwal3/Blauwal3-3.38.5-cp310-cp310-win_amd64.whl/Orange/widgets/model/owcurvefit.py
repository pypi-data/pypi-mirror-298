...
def __(key):
    return i18n.t("widget.model.model.owcurvefit." + key)
...
class ParametersWidget(QWidget):
    ...
    def _setup_gui(self):
        ...
        self.__labels = [QLabel(__("label.name")), QLabel(__("label.initial_value")),
                         QLabel(__("label.lower_bound")), QLabel(__("label.upper_bound"))]
...
class OWCurveFit(OWBaseLearner):
    ...
    class Warning(OWBaseLearner.Warning):
        duplicate_parameter = Msg(__("warn.duplicate_parameter"))
        unused_parameter = Msg(__("warn.unused_parameter"))
        data_missing = Msg(__("warn.data_missing"))

    class Error(OWBaseLearner.Error):
        invalid_exp = Msg(__("err.invalid_exp"))
        no_parameter = Msg(__("err.no_parameter"))
        unknown_parameter = Msg(__("err.unknown_parameter"))
        parameter_in_attrs = Msg(__("err.parameter_in_attrs"))
    ...
    FEATURE_PLACEHOLDER = __("btn.feature")
    PARAM_PLACEHOLDER = __("btn.param")
    FUNCTION_PLACEHOLDER = __("btn.function")
    ...
    def add_main_layout(self):
        box = gui.vBox(self.controlArea, __("box.parameters"))
        ...
        self.__expression_edit = gui.lineEdit(
            ...
            placeholderText=__("placeholder.expression"), callback=self.settings_changed
        )
    ...
    def get_learner_parameters(self) -> Tuple[Tuple[str, Any]]:
        return ((__("box.expression"), self.expression),)
    ...
    def check_data(self):
        ...
        if self.data:
            ...
            if len(cont_attrs) == 0:
                self.Error.data_error(__("err.data_error"))
