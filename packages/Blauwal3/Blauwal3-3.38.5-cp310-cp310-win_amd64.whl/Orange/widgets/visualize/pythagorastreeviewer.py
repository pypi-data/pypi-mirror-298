...
def __(key):
    return i18n.t("widget.visualize.visualize.pythagorastreeviewer." + key)
...
class DiscreteTreeNode(TreeNode):
    ...
    def tooltip(self):
        ...
        text = '<p>' \
            ...
            + '<hr>' \
            + (__('split_by') + splitting_attr.name
...
class ContinuousTreeNode(TreeNode):
    ...
    def tooltip(self):
        ...
        return __("mean").format(mean) \
            + __("standard_deviation").format(std) \
            + __("samples").format(num_samples) \
            + '<hr>' \
            + (__('split_by') + splitting_attr.name
               if not self.tree.is_leaf(self.label) else '') \
            + ('<br><br>' if rules_str and not self.tree.is_leaf(self.label) else '') \
