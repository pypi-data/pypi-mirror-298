...
class OWBoxPlot(widget.OWWidget):
    ...
    def compute_box_data(self):
        ...
        if self.group_var:
            self.dist = None
            missing_val_str = __("tip.miss").format(self.group_var.name)
    ...
    def _compute_tests_cont(self):
        ...
        if self.compare == OWBoxPlot.CompareNone or len(self.stats) < 2:
            ...
            if self.compare == OWBoxPlot.CompareMedians:
                ...
                t, p = stat_ttest()
                t = "" if np.isnan(t) else __("tip.students").format(t, p, n)
        else:
            if self.compare == OWBoxPlot.CompareMedians:
                t = ""
                # U, p = -1, -1
                ...
                F, p = stat_ANOVA()
                t = "" if np.isnan(F) else __("tip.anova").format(F, p, n)
        self.stat_test = t
    ...
    def _compute_tests_disc(self):
        if self.group_var is None or self.attribute is None:
            ...
            if np.isnan(p):
                ...
                self.stat_test = __("tip.stat_test").format(chi, p, dof)
    ...
    def strudel(self, dist, group_val=None):
        ...
        cum = 0
        missing_val_str = __("tip.miss").format(attr.name)
        values = attr.values + ("",)
