...
MetricDefs: Dict[int, MetricDef] = {
    metric.id: metric for metric in (
        MetricDef(EuclideanNormalized, __("euclidean_normalized"),
                  "Square root of summed difference between normalized values",
                  distance.Euclidean, normalize=True),
        MetricDef(Euclidean, __("euclidean"),
                  "Square root of summed difference between values",
                  distance.Euclidean),
        MetricDef(ManhattanNormalized, __("manhattan_normalized"),
                  "Sum of absolute differences between normalized values",
                  distance.Manhattan, normalize=True),
        MetricDef(Manhattan, __("manhattan"),
                  "Sum of absolute differences between values",
                  distance.Manhattan),
        MetricDef(Mahalanobis, __("mahalanobis"),
                  "Mahalanobis distance",
                  distance.Mahalanobis),
        MetricDef(Hamming, __("hamming"),"Hamming distance",
                  distance.Hamming),
        MetricDef(Cosine,  __("cosine"),"Cosine distance",
                  distance.Cosine),
        MetricDef(Pearson, __("pearson"),
                  "Pearson correlation; distance = 1 - ρ/2",
                  distance.PearsonR),
        MetricDef(PearsonAbsolute, __("pearson_absolute"),
                  "Absolute value of Pearson correlation; distance = 1 - |ρ|",
                  distance.PearsonRAbsolute),
        MetricDef(Spearman, __("spearman"),
                  "Spearman correlation; distance = 1 - ρ/2",
                  distance.SpearmanR),
        MetricDef(SpearmanAbsolute, __("spearman_absolute"),
                  "Absolute value of Pearson correlation; distance = 1 - |ρ|",
                  distance.SpearmanRAbsolute),
        MetricDef(Jaccard,  __("jaccard"),"Jaccard distance",
                  distance.Jaccard)
    )
}
...
class OWDistances(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        gui.radioButtons(
            ...
            box=__("box_compare"), orientation=Qt.Horizontal, callback=self._invalidate
        )
        box = gui.hBox(self.controlArea, __("box_distance_metric"))
    ...
    def send_report(self):
        ...
        self.report_items((
            ...
            (i18n.t("common.general.metric"), MetricDefs[self.metric_id].name)
        ))
