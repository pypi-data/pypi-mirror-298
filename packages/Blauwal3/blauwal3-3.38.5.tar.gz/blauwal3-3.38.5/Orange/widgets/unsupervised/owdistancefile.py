...
class OWDistanceFile(OWUrlDropBase, RecentPathsWComboMixin):
    ...
    def __init__(self):
        ...
        vbox = gui.vBox(self.controlArea, __("options"))
        gui.checkBox(
            vbox, self, "auto_symmetric",
            __("triangular"),
            tooltip=__("triangular_tips"),
    ...
    def browse_file(self, in_demos=False):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, __('dialog_open_file'), start_file,
    ...
    def send_report(self):
        if not self.distances:
            self.report_paragraph(__("report.load_data"))
        else:
            self.report_items([(__("report.file_name"), self.distances.name)])
