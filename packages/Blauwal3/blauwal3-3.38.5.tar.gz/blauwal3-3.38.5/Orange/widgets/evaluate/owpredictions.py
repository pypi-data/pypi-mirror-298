...
ERROR_TOOLTIPS = [
    __("performance.dont_show_errors"),
    __("performance.show_difference"),
    __("performance.show_absolute_difference"),
    __("performance.show_relative_difference"),
    __("performance.show_absolute_relative_difference"),]
...
class OWPredictions(OWWidget):
    ...
    class Inputs:
        ...
        predictors = MultiInput("Predictors", Model, filter_none=True, label=i18n.t("widget.evaluate.evaluate.common.predictor"))

    class Outputs:
        selected_predictions = Output("Selected Predictions", Orange.data.Table,
                                      default=True, replaces=["Predictions"])
        annotated = Output("Predictions", Orange.data.Table, label=i18n.t("widget.evaluate.evaluate.common.prediction"))
    ...
    def __init__(self):
        ...
        self._prob_controls = [
            gui.widgetLabel(predopts, __("box_show_probability")),
            gui.comboBox(
        ...
        self._cls_error_controls = [
            gui.checkBox(
                ...
                __("box_show_classification_errors"),
                tooltip="Show 1 - probability assigned to the correct class",
                callback=self._update_errors_visibility
            )
        ]

        err_label = gui.widgetLabel(predopts, __("box_shown_regression_error"))
        ...
        gui.rubber(predopts)
        self.reset_button = button = QPushButton(__("btn_restore_original"))
        button.clicked.connect(self._reset_order)
        button.setToolTip(__("tooltip_original_row"))
        ...
        self.score_opt_box = scoreopts = gui.hBox(
            None, sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))
        gui.checkBox(
            scoreopts, self, "show_scores", __("box_show_perfomance_scores"),
            callback=self._update_score_table_visibility
        )
        gui.separator(scoreopts, 32)
        self._target_controls = [
            gui.widgetLabel(scoreopts, __("box_target_class")),
            gui.comboBox(
