...
class SortBy(IntEnum):
    ...
    @staticmethod
    def items():
        return [__("original"),__("alphabetically"),__("absolute"),__("positive"),__("negative")]
...
class OWNomogram(OWWidget):
    ...
    def __init__(self):
        ...
        self.class_combo = gui.comboBox(
            ...
            searchable=True)
        grid.addWidget(QLabel(__("target_class")), 0, 0, lab_align)
        grid.addWidget(self.class_combo, 0, 1)

        self.norm_check = gui.checkBox(
            None, self, "normalize_probabilities", __("checkbox.normalize_probability"),
        ...
        grid.addWidget(QLabel(__("scale")), 2, 0, lab_align)
        grid.addWidget(gui.appendRadioButton(
            group, __("gbox.point_scale"), addToLayout=False), 2, 1)
        grid.addWidget(gui.appendRadioButton(
            group, __("gbox.log_odds_ratio"), addToLayout=False), 3, 1)

        grid = QGridLayout()
        gui.widgetBox(self.controlArea, __("box.display_feature"), orientation=grid)
        ...
        grid.addWidget(QLabel(__("order")), 0, 0, lab_align)
        ...
        grid.addWidget(QLabel(__("show")), 1, 0, lab_align)
        ...
        self.cont_feature_dim_combo = gui.comboBox(
            None, self, "cont_feature_dim_index", label=__("label_numeric_feature"),
