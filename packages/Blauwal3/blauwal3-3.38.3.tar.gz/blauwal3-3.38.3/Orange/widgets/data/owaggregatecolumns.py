...
def __(key):
    return i18n.t("widget.data.data.owaggregatecolumns." + key)
...
class OWAggregateColumns(widget.OWWidget):
    ...
    #
    Operations = {"Sum": OpDesc(__("checkout.sum"), np.nansum),
                  "Product": OpDesc(__("checkout.product"), np.nanprod),
                  "Min": OpDesc(__("checkout.min"), np.nanmin, True),
                  "Max": OpDesc(__("checkout.max"), np.nanmax, True),
                  "Mean": OpDesc(__("checkout.mean"), np.nanmean, True),
                  "Variance": OpDesc(__("checkout.variance"), np.nanvar),
                  "Median": OpDesc(__("checkout.median"), np.nanmedian, True)}
    ...
    operation = ContextSetting(__("checkout.sum"))
    ...
    def __init__(self):
        ...
        self.selection_box = gui.vBox(self.controlArea, __("variable_selection"))
        self.selection_group = QButtonGroup(self.selection_box)
        for i, label in enumerate((__("all"),
                                  __("all_including_meta"),
                                  __("features_from_input_signal"),
                                  __("selected_variables"))):
        ...
        box = gui.vBox(self.controlArea, box=__("label.operator"))
        ...
        self.operation = self.KeyFromDesc[self.operation]
        combo.setCurrentText(self.Operations[self.operation].name)
        box.layout().addWidget(combo)

        gui.lineEdit(
            box, self, "var_name",
            label=__("label.variable"), orientation=Qt.Horizontal,
            callback=self.commit.deferred
        )
