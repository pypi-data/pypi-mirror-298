...
def __(key):
    return i18n.t("widget.visualize.visualize.utils_tree." + key)
...
class DiscreteRule(Rule):
    ...
    def merge_with(self, rule):
        ...
        # be eq or not eq.
        warnings.warn(__('merge_discrete_rule') % (self, rule))
        return rule
...
class ContinuousRule(Rule):
    ...
    def merge_with(self, rule):
        if not isinstance(rule, ContinuousRule):
            raise NotImplementedError(__('continuous_rule_merger'))
