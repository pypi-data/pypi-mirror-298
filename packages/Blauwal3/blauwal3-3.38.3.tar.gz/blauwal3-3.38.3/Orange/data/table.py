...
from Orange.util import OrangeDeprecationWarning, dummy_callback,deprecated
...
class Table(Sequence, Storage):
    ...
    def _check_unlocked(self, partflag):
        if not self._unlocked & partflag:
            raise ValueError(__("tip.unlocked"))
        ...
        try:
            for part, flag, _ in self._lock_parts_val():
                ...
                if sp.isspmatrix_csr(part) or sp.isspmatrix_csc(part):
                    ...
                    raise ValueError(__("tip.sparse"))
                else:
                    sync(part)
        except:
    ...
    def unlocked(self, *parts):
        ...
        for part, flag, name in self._lock_parts_val():
            if not flag & self._unlocked \
                ...
                raise ValueError(__("tip.unlock_view").format(name))
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not args:
            if not kwargs:
                ...
                raise TypeError(__("tip.table_called"))

        if isinstance(args[0], str):
            if len(args) > 1:
                raise TypeError(__("tip.table_str"))
            ...
            if len(args) > 1:
                raise TypeError(__("tip.table_arg"))
        ...
        if isinstance(row_idx, Integral):
            ...
            if not isinstance(col_idx, str) and isinstance(col_idx, Sized):
                if isinstance(value, (Sequence, np.ndarray)):
                    ...
                    raise TypeError(__("tip.set_value"))
                if len(values) != len(col_idx):
                    raise ValueError(__("tip.invalid_value"))
            else:
    ...
    def concatenate(cls, tables, axis=0, *, ignore_domains=None):
        ...
        names = [table.name for table in tables if table.name != __("name.untitled")]
        if names:
            conc.name = names[0]
        return conc
    ...
    def _filter_to_indicator(self, filter):
        ...
        def get_col_indices():
            ...
            if isinstance(filter, FilterDiscrete):
                raise ValueError(__("tip.discrete_filter"))
            ...
            raise TypeError(__("tip.invalid_filter"))
    ...
    def _range_filter_to_indicator(filter, col, fmin, fmax):
        with np.errstate(invalid="ignore"):  # nan's are properly discarded
            ...
            raise TypeError(__("tip.invalid_operator"))
    ...
    def _compute_contingency(self, col_vars=None, row_var=None):
        ...
        if row_var is None:
            ...
            if row_var is None:
                raise ValueError(__("tip.row_var"))

        row_desc = self.domain[row_var]
        if not row_desc.is_discrete:
            raise TypeError(__("tip.row_discrete"))
        ...
        if any(not (var.is_discrete or var.is_continuous)
               for var in col_desc):
            raise ValueError(__("tip.contingency"))
    ...
    def transpose(cls, table, feature_names_column="",
        ...
        progress_callback(0, __("tip.transpose"))
