...
def __(key):
    return i18n.t("widget.data.data.owgroupby." + key)
...
AGGREGATIONS = {
    __("alg.mean"): Aggregation("mean", {ContinuousVariable, TimeVariable}),
    __("alg.median"): Aggregation("median", {ContinuousVariable, TimeVariable}),
    __("alg.q1"): Aggregation(lambda s: s.quantile(0.25), {ContinuousVariable, TimeVariable}),
    __("alg.q3"): Aggregation(lambda s: s.quantile(0.75), {ContinuousVariable, TimeVariable}),
    __("alg.min_value"): Aggregation("min", {ContinuousVariable, TimeVariable}),
    __("alg.max_value"): Aggregation("max", {ContinuousVariable, TimeVariable}),
    __("alg.model"): Aggregation(
        lambda x: pd.Series.mode(x).get(0, nan),
        {ContinuousVariable, DiscreteVariable, TimeVariable}
    ),
    __("alg.standard_deviation"): Aggregation(std, {ContinuousVariable, TimeVariable}),
    __("alg.variance"): Aggregation(var, {ContinuousVariable, TimeVariable}),
    __("alg.sum"): Aggregation("sum", {ContinuousVariable}),
    __("alg.concatenate"): Aggregation(
        concatenate,
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
    ),
    __("alg.span"): Aggregation(span, {ContinuousVariable, TimeVariable}),
    __("alg.first_value"): Aggregation(
        "first", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("alg.last_value"): Aggregation(
        "last", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("alg.random_value"): Aggregation(
        lambda x: x.sample(1, random_state=0),
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
    ),
    __("alg.count_defined"): Aggregation(
        "count", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("alg.count"): Aggregation(
        "size", {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable}
    ),
    __("alg.proportion_defined"): Aggregation(
...
TABLE_COLUMN_NAMES = [__("box.attributes"), __("box.aggregations")]
...
class OWGroupBy(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        data = Input("Data", Table, doc=__("inputdoc"))

    class Outputs:
        data = Output("Data", Table, doc=__("outputdoc"))
    ...
    def __init_main_area(self) -> None:
        ...
        gui.widgetBox(self.mainArea, orientation=grid_layout, box=__("box.aggregations"))
