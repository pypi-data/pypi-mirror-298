...
class OWDataInfo(widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.description = gui.widgetLabel(
            gui.vBox(self.controlArea, box=__("box.set_name")))
        self.attributes = gui.widgetLabel(
            gui.vBox(self.controlArea, box=__("box.data_attr")))
    ...
    def data(self, data):
        if data is None:
            ...
            self.data_desc = {
                label: value
                for label, func in ((__("row.num_var"), self._p_name),
                                    (__("row.location"), self._p_location),
                                    (__("row.size"), self._p_size),
                                    (__("row.features"), self._p_features),
                                    (__("row.targets"), self._p_targets),
                                    (__("row.metas"), self._p_metas),
                                    (__("row.missing_data"), self._p_missing))
            ...
            if is_sql(data):
                def set_exact_length():
                    self.data_desc[__("row.size")] = self._p_size(data, exact=True)
                    self.update_info()
    ...
    def update_info(self):
        ...
        if not self.data_desc:
            self.description.setText(__("msg.no_data"))
        else:
    ...
    def send_report(self):
        if self.data_desc:
            self.report_items(__("report.data_table_properties"), self.data_desc)
        if self.data_attrs:
            self.report_items(__("report.additional_attributes"), self.data_attrs)
