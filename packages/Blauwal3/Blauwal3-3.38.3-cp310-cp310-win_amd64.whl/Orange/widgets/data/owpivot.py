...
class Pivot:
    ...
    def __init__(self, table: Table, agg_funs: Iterable[Function],
        ...
        if not self._row_var.is_primitive():
            raise TypeError(__("msg.row_var"))
        if self._col_var and not self._col_var.is_discrete:
            raise TypeError(__("msg.col_var"))
    ...
    def __get_pivot_tab_domain(self, val_var, X, X_h, X_v, X_t, agg_funs):
        ...
        if create_time_var:
            ...
            attrs.extend([[TimeVariable("Total11", **kwargs)]] * 2)
        ...
        row_var_h = DiscreteVariable(self._row_var.name, values=["Total11"])
