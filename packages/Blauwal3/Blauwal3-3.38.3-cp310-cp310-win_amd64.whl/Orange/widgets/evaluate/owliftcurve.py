...
class ParameterSetter(CommonParameterSetter):
    WIDE_LINE_LABEL = i18n.t("widget.visualize.visualize.customizableplot.line.line")
    DEFAULT_LINE_LABEL = i18n.t("widget.visualize.visualize.customizableplot.line.default_line")

    WIDE_LINE_WIDTH = 3
    LINE_WIDTH = 1
    DEFAULT_LINE_WIDTH = 1

    WIDE_LINE_STYLE = i18n.t("widget.visualize.visualize.customizableplot.line.line.solid_line")
    LINE_STYLE = i18n.t("widget.visualize.visualize.customizableplot.line.line.solid_line")
    DEFAULT_LINE_STYLE = i18n.t("widget.visualize.visualize.customizableplot.line.line.dash_line")
...
class OWLiftCurve(widget.OWWidget):
    ...
    class Outputs:
        calibrated_model = Output(i18n.t("widget.evaluate.evaluate.common.calibrated_model"), Model)
    ...
    class Information(widget.OWWidget.Information):
        no_output = Msg(__("msg_output_model"))
    ...
    XLabels = (__("label_p_rate"), __("label_p_rate") , __("label_recall"))
    YLabels = (__("label_lift"), __("label_tp_rate"), __("label_pr_rate"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=__("box_set"))
        gui.checkBox(box, self, "show_threshold", __("checkbox_show_threshold") ,
                     callback=self._on_show_threshold_changed)
        gui.checkBox(box, self, "show_points", __("checkbox_show_points"),
                     callback=self._on_show_points_changed)

        gui.rubber(self.controlArea)

        box = gui.vBox(self.controlArea, box=__("check_area"))
...
def cumulative_gains(y_true, y_score, target=1):
    if len(y_true) != len(y_score):
        raise ValueError(__("msg_array_dimensions"))
