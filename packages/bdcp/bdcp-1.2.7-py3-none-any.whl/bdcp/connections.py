...
class Connection:
    ...
    def get_check(self, resource_name, verbose=True, inplace=True):
        """
        获取远程的日志文件内容
        """
        hostname_df = self.resources[self.resources['resource_name']==resource_name]
                ...
                for ret_data in ret_list:
                    ...
                    if "cat" in command and "grep" in command:
                        ...
                        if '/home/qingjiao' in command:
                            ret_data['success'] = "echo {},{} >> {}".format('/'.join(keyword, value, path_name)
                        else:
            ...
            if inplace:
                for ret_data in ret_list:
                    if 'success' in ret_data:
                        self.ssh_exec(resource_name=resource_name, command=ret_data['success'])
