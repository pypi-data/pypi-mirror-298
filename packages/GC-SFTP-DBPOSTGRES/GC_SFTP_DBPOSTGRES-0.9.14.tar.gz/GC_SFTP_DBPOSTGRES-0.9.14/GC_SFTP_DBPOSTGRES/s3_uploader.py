...
def process_file(file_name, subfolder, s3_folder, source_folder, bucket_name, s3, log_file):
    ...
    # Verifica si el archivo es un '_cleaned.csv' y evita crear una subcarpeta basada en él
    if '_cleaned.csv' in file_name:
        # Procesar el archivo cleaned pero sin crear la subcarpeta ni subirlo a S3
        try:
            ...
            # Convertir a Parquet localmente (sin subir a S3)
            df.to_parquet(parquet_file_path, index=False, engine='pyarrow')
            write_log(f"El archivo {parquet_file_name} ha sido procesado localmente (no subido a S3).", log_file)

            # Eliminar el archivo Parquet y CSV original después del procesamiento
        ...
        # Crear la subcarpeta solo para los archivos que NO tienen '_cleaned.csv'
        start = file_name.index('-') + 1  # Encuentra el primer guion "-"
        end = file_name.rfind('_')  # Encuentra el último guion bajo "_"
        subfolder_name = file_name[start:end]  # Extrae el nombre de la subcarpeta correctamente

        # Crear la subcarpeta con el nombre correcto basado en el archivo sin '_cleaned.csv'
        create_subfolder(s3_folder, subfolder_name, bucket_name, s3, log_file)

        # Procesar el archivo normal (sin '_cleaned.csv'), convertir a Parquet y subirlo a S3
        try:
            df = pd.read_csv(csv_file_path, low_memory=False, on_bad_lines='skip')
            parquet_file_name = file_name.replace('.csv', '.parquet')
            parquet_file_path = os.path.join(source_folder, subfolder, parquet_file_name)
            ...
            # Eliminar el archivo Parquet y CSV original después de la subida
            os.remove(parquet_file_path)
            os.remove(csv_file_path)
            write_log(f"El archivo CSV original {csv_file_path} ha sido eliminado.", log_file)
