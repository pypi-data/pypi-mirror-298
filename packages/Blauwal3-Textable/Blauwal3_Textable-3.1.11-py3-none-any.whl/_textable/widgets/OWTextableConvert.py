...
def __(key):
    return i18n.t("textable.owtextableconvert." + key)
ColumnDelimiters = [
    ('tabulation (\\t)', u'\t'),
    ('comma (,)',  u','),
    ('semi-colon (;)', u';'),
...
class OWTextableConvert(OWTextableBaseWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.advancedSettings.draw(label=__(u'advanced_settings'))
        ...
        gui.checkBox(
            ...
            master=self,
            value='sortRows',
            label=__(u'sort_rows_by_column'),
            labelWidth=180,
            callback=self.sendButton.settingsChanged,
            tooltip=(
                u"Sort table rows."
        ...
        gui.checkBox(
            ...
            master=self,
            value='sortCols',
            label=__(u'sort_columns_by_row'),
            labelWidth=180,
            callback=self.sendButton.settingsChanged,
            tooltip=(
        ...
        self.reformatCheckbox = gui.checkBox(
            ...
            master=self,
            value='reformat',
            label=__(u'reformat_to_sparse_crosstab'),
            callback=self.sendButton.settingsChanged,
            tooltip=(
        ...
        self.unweightedCheckbox = gui.checkBox(
            ...
            master=self,
            value='unweighted',
            label=__(u'encode_counts_by_repeating_rows'),
            callback=self.sendButton.settingsChanged,
            tooltip=(
        ...
        conversionEncodingCombo = gui.comboBox(
            ...
            callback=self.sendButton.settingsChanged,
            orientation='horizontal',
            tooltip=__(u"msg.encoding_info"),
        )
        ...
        colDelimiterCombo = gui.comboBox(
            ...
            tooltip=__(u"msg.column_delimiter_info"),
        )
        ...
        gui.checkBox(
            ...
            label=__(u'include_orange_headers'),
            tooltip=__(u"msg.include_orange_headers_info"),
        )
        gui.separator(widget=exportBox, height=2)
        ...
        self.exportButton = gui.button(
            ...
            master=self,
            label=__(u'export_to_file'),
            callback=self.exportFile,
            tooltip=__(u"msg.export_to_file_info"),
        )
        self.copyButton = gui.button(
            widget=exportBoxLine3,
            master=self,
            label=__(u'copy_to_clipboard'),
            callback=self.copyToClipboard,
            tooltip=__(u"msg.copy_to_clipboard_info"),
        )
        gui.separator(widget=exportBox, height=2)
    ...
    def sendData(self):
        ...
        if not self.table:
            self.infoBox.setText(__(u'result_empty'), 'warning')
        ...
        if self.displayAdvancedSettings:
            ...
            self.infoBox.setText(__(u'widget_needs_input'), "warning")
