...
def __(key):
    return i18n.t("textable.owtextablecontext." + key)
...
class OWTextableContext(OWTextableBaseWidget):
    ...
    def __init__(self):
        ...
        self.infoBox = InfoBox(
            ...
            stringClickSend=__(u'msg.stringClickSend'),
        )
        ...
        self.separateAnnotationCheckBox = gui.checkBox(
            ...
            label=__(u'separate_annotation'),
            callback=self.sendButton.settingsChanged,
            tooltip=__(u'msg.separate_annotation_info'),
        )
        ...
        self.contextSegmentationCombo = gui.comboBox(
            ...
            labelWidth=180,
            callback=self.sendButton.settingsChanged,
            tooltip=__(u'msg.segmentation_infon'),
        )
        gui.separator(widget=self.contextsBox, height=3)
        self.contextAnnotationCombo = gui.comboBox(
            ...
            labelWidth=180,
            callback=self.sendButton.settingsChanged,
            tooltip=__(u'msg.annotation_infon'),
        )
        ...
        self.maxDistanceSpin = gui.spin(
            ...
            tooltip=__(u'msg.max_distance_info'),
        )
        gui.separator(widget=self.neighboringSegmentsBox, height=3)
        self.useCollocationFormatCheckbox = gui.checkBox(
            ...
            label=__(u'use_collocation_format'),
            callback=self.sendButton.settingsChanged,
            tooltip=__(u'msg.use_collocation_format_info'),
        )
        gui.separator(widget=self.neighboringSegmentsBox, height=3)
        ...
        self.minFrequencySpin = gui.spin(
            ...
            step=1,
            orientation='horizontal',
            label=__(u'min_frequency'),
            labelWidth=160,
            callback=self.sendButton.settingsChanged,
            keyboardTracking=False,
            tooltip=__(u'msg.min_frequency_info'),
        )
        ...
        self.maxLengthSpin = gui.spin(
            ...
            checkCallback=self.sendButton.settingsChanged,
            keyboardTracking=False,
            tooltip=__(u'msg.max_length_info'),
        )
        ...
        gui.checkBox(
            ...
            master=self,
            value='mergeStrings',
            label=__(u'merge_strings'),
            callback=self.sendButton.settingsChanged,
            tooltip=__(u'msg.merge_strings_info'),
        )
        gui.separator(widget=self.contextsBox, height=3)
    ...
    def sendData(self):
        ...
        if not len(table.row_ids):
            self.infoBox.setText(__(u'msg.result_empty'), 'warning')
            self.send('Textable table', None)
            self.send('Orange table', None)
        else:
            self.send('Textable table', table)
            self.send('Orange table', table.to_orange_table())
            self.infoBox.setText(__(u'msg.result_sent'))
