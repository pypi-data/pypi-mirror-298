...
def find_project_root_dir(abs_filepath: Optional[str] = None) -> str:
    ...
    while current_dir != os.path.dirname(current_dir):
        morph_project_db_path = os.path.join(
            current_dir, MorphConstant.MORPH_PROJECT_DB
        )
        if os.path.isfile(morph_project_db_path):
            return os.path.abspath(os.path.dirname(morph_project_db_path))
        current_dir = os.path.dirname(current_dir)

    morph_project_path = os.path.join(Path.home(), MorphConstant.MORPH_PROJECT_YAML)
    if os.path.isfile(morph_project_path):
        return os.path.abspath(os.path.dirname(morph_project_path))
    raise FileNotFoundError(
        f"{MorphConstant.MORPH_PROJECT_YAML} not found in the current directory or any parent directories."
    )
