...
# SnowflakeConnectionKeyPairFile の復元
...
class ProfileYaml(BaseModel):
    ...
    def is_file_exits() -> bool:
        return os.path.isfile(MorphConstant.MORPH_PROFILE_PATH)

    @staticmethod
    def from_dict(data: Dict[str, Any]) -> "ProfileYaml":
    ...
    def load_yaml() -> "ProfileYaml":
        if not ProfileYaml.is_file_exits():
            with open(MorphConstant.MORPH_PROFILE_PATH, "w") as file:
                yaml.dump({"connections": []}, file)

        with open(MorphConstant.MORPH_PROFILE_PATH, "r") as file:
    ...
    @staticmethod
    def find_connection(
        profile_yaml: Optional["ProfileYaml"], connection_slug: str
    ) -> Optional[Connection]:
        profile_yaml = ProfileYaml.load_yaml() if profile_yaml is None else profile_yaml
    ...
    def find_builtin_db_connection() -> tuple[str, PostgresqlConnection]:
        ...
        while True:
            ...
            if response.status_code > 500:
                ...
                return database_id, PostgresqlConnection(
    ...
    def sync_cloud_connection(override: Optional[bool] = False) -> None:
        ...
        try:
            ...
            if response.status_code > 500:
                ...
                items = ExternalConnectionListResponse.model_validate(
                    response_json
                ).items
