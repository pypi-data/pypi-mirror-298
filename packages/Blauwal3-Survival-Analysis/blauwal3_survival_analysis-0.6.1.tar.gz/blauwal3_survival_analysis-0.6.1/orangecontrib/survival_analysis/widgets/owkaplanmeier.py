...
def __(key):
    return i18n.t('survival_analysis.owkaplanmeier.' + key)
...
class OWKaplanMeier(OWWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        box = gui.vBox(self.controlArea, __('group'), margin=0)
        gui.comboBox(
            box,
            self,
            'group_var',
        ...
        box = gui.vBox(self.controlArea, __('display_options'))
        gui.checkBox(
            box,
            self,
            'show_confidence_interval',
            label=__('confidence_intervals'),
