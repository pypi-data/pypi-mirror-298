...
from orangecontrib.survival_analysis.i18n_config import *

def __(key):
    return i18n.t('survival_analysis.owranksurvivalfeatures.' + key)  

class ScoreMethods:
    multivariate_log_rank = 0
    cox_regression = 1

    labels = [__('multivariate_log_rank_test'), __('cox_regression')]
...
class OWRankSurvivalFeatures(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __('score_method'), margin=0)
        gui.comboBox(
            box,
            self,
            'score_method',
        ...
        sel_method_box = gui.vBox(self.buttonsArea, __('select_attributes'))
        grid = QGridLayout()
        ...
        b1 = button(self.tr(__('none')), OWRankSurvivalFeatures.select_none)
        b2 = button(self.tr(__('manual')), OWRankSurvivalFeatures.manual_selection)
        b3 = button(self.tr(__('best_ranked')), OWRankSurvivalFeatures.select_n_best)
        ...
        self.commit_button = gui.auto_commit(
            self.buttonsArea, self, 'auto_commit', __('commit'), box=False
        )
    ...
    def on_done(self, worker_result):
        ...
        if self.score_method == ScoreMethods.multivariate_log_rank:
            self.model.setHorizontalHeaderLabels(
                ['', __('multivariate_log_rank_test'), f'{"p".center(13)}', 'FDR']
            )

        elif self.score_method == ScoreMethods.cox_regression:
            self.model.wrap(worker_result)
            self.model.setHorizontalHeaderLabels(
                ['', __('log_likelihood_ratio_test'), f'{"p".center(13)}', 'FDR']
            )
