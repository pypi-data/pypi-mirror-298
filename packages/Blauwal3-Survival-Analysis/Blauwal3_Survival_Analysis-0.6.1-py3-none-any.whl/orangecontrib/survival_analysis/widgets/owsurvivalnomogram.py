...
def __(key):
    return i18n.t('survival_analysis.owsurvivalnomogram.' + key)  
...
class SortBy(IntEnum):
    ...
    def items():
        return [
            __('original_order'),
            __('alphabetically'),
            __('absolute_importance'),
            __('positive_influence'),
            __('negative_influence'),
        ]
...
class OWSurvivalNomogram(OWWidget):
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _("display_features"))
        grid = QGridLayout()
        radio_group = gui.radioButtonsInBox(
            box, self, "display_index", [], orientation=grid, callback=self.update_scene
        )
        radio_all = gui.appendRadioButton(radio_group, _("all"), addToLayout=False)
        radio_best = gui.appendRadioButton(
            radio_group, _("best_ranked"), addToLayout=False
        )
        ...
        self.cont_feature_dim_combo = gui.comboBox(
            ...
            label=_("numeric_features"),
            items=[_("one_d_projection"), _("two_d_curve")],
            orientation=Qt.Horizontal,
            callback=self.update_scene,
        )
