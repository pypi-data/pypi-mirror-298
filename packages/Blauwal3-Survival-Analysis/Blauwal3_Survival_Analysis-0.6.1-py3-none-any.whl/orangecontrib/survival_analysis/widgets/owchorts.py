...
def __(key):
    return i18n.t('survival_analysis.owchorts.' + key)
...
class OWCohorts(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.stratify_on_options = [
            (StratifyOn.CoxRiskScore, __('cox_risk_score')),
        ]

        self.splitting_criteria_options = [
            (SplittingCriteria.Median, __('median')),
            (SplittingCriteria.Mean, __('mean')),
            (SplittingCriteria.LogRankTest, __('log_rank_test')),
        ]

        box = gui.vBox(self.controlArea, __('stratify_on'), margin=0)
        gui.comboBox(
            box,
            self,
            'stratify_on',
        ...
        box = gui.vBox(self.controlArea, __('splitting_criteria'), margin=0)
        ...
        self.commit_button = gui.auto_commit(
            self.controlArea, self, 'auto_commit', __('commit'), box=False
        )
