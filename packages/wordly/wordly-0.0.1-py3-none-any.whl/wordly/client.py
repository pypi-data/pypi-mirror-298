...
class DictClient:
    ...
    async def _send(self, command: bytes):
        if None in (self.reader, self.writer):
            ...
            new_line_reader.mapping[Status.INITIAL_CONNECTION.name] = self.line_reader.mapping[Status.INITIAL_CONNECTION.name]
        ...
        while Status.COMMAND_COMPLETE.name not in self.line_reader.mapping and Status.NO_MATCH.name not in self.line_reader.mapping:
