...
class YamlBuilder:
    ...
    def _load_template(self):
        templates = load_flatten_settings(self.template, dict_as_value=True)
        tags = {"!TEMPLATE": templates} if templates else {}
        if self.config_file:
    ...
    def _build_config(self):
        # TODO: Not working with template file for now, due to the way yaml parsed
        if self.template:
        ...
        if self.secret:
            ...
            if secret_path.exists():
                tags[TAG_NAME_SECRET] = load_flatten_settings(Path.cwd() / DEFAULT_SECRET_FILE)
            else:
                print(f"Warning: secret file {secret_path} file not found, it will be ignored")
        ...
        if self.settings_file or self.template:
            ...
            tags[TAG_NAME_SET] = load_flatten_settings(self.settings_file, dict_as_value=True, tags=tags) if self.settings_file else {}
            if self.template:
                templates = load_flatten_settings(self.template, dict_as_value=True, tags=tags)
        ...
        def _get_include(relative_path, default=None):
            if os.path.isfile(config_dir / relative_path):                
                ...
                raise FileExistsError(f"{relative_path} not found or is not a file in {config_dir}")
