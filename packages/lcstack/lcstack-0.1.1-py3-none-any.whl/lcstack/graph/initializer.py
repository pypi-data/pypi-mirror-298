...
class WorkflowInitializer(BaseInitializer):
            ...
            if cv:
                if isinstance(cv.agent, AgentConfig):
                    ...
                    dep_info = DpendencyInfo(
                        ...
                        path=InitializerPath(path=[f"{agent_config.config}:{agent_config.name}"]),
                    ...
                    raise ValueError(f"Unsupported agent type after base parsing: {type(cv.agent)}")
    ...
    def _load_from_config_file(self, agent_config: AgentConfig):
        ...
        if False: # agent_config in cached_configs:
            ...
            lcs = LcStack.from_yaml(get_config_root() / agent_config, env=True, secret=True, settings=True)
