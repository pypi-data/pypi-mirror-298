...
def create_llm(
        ...
        **kwargs) -> BaseLanguageModel[str|BaseMessage]: 
        ...
        if tools:
            if isinstance(llm, BaseChatModel) and not llm.bind_tools is BaseChatModel.bind_tools:
                ...
                raise NotImplementedError(f"Language Model Provider {provider} with tag {tag} does not support bind_tools.")
