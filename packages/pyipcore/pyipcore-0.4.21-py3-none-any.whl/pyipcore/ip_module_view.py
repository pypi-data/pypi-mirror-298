...
from pyipcore.ui_utils import DraggableGraphicsView, QFont


MODULE_COLOR = QColor(32, 144, 245, 225)
BORDER_COLOR = QColor(0, 0, 0, 25)
FILL_BACKGROUND = QColor(255, 255, 0, 25)
INPUT_LINE_COLOR = QColor(238, 121, 66, 175)
OUTPUT_LINE_COLOR = QColor(65, 105, 225, 175)
class IpCoreView(DraggableGraphicsView):
    ...
    def render_ipcore(self, ipcore: IpCore):
        ...
        # self.pixelize()
        # # save
        # self.save(f"z:/ipcore_{int(time.time())}.png")
        # print("done")
        self.effect_shallow()
        # self.scale(.1, .1)
        # self.scene().update()
...
if __name__ == '__main__':
    IpCore.FromVerilog("", 'test', 'test.v')
    ip = IpCore("", 'test')
    d = ip.dict
    # d['WIDTH'] = 32
    # d['add_clr'] = False
