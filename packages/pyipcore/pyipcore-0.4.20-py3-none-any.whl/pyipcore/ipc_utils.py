...
class IoDefRecord:  # just raw record.
    ...
    def __str__(self):
        return f"<'{self.pin}':'{self.name}' | {self.direction} {self.iotype} {self.pull}>" + (f" # {self.note}" if self.note else "")
...
class PhysicsIoDef(PhysicsIoDef):
    ...
    def _combine_single(one_single_rec:list[IoDefRecord]) -> PhysicsIoDef:
        ...
        d0, iot0, p0 = one_single_rec[0].direction, one_single_rec[0].iotype, one_single_rec[0].pull
        ...
        for i, rec in enumerate(sorted):
            ...
            if rec.direction != d0 or rec.iotype != iot0 or rec.pull != p0:
        ...
        return PhysicsIoDef(pins, name, d0, length, iot0, p0, namegs=rec.namegs[:-1], note=note)
    ...
    def _bundle_singles(singles:list[IoDefRecord])-> list[PhysicsIoDef]:
        ...
        for rec in iso_recs:
            iso_pios.append(PhysicsIoDef(rec.pin, rec.name, rec.direction, rec.iotype, rec.pull, namegs=rec.namegs, note=rec.note))
    ...
    def __init__(self,
                 ...
                 iotype=IoType.LV33,
                 pull=Pull.FLOAT,* ,namegs=None, note:str=""):
    ...
    def __str__(self):
        return f"<'{self.rpins}':'{self.name}{('[' + str(self._width - 1) + ':0]') if self._width else ''}' | {self.direction} {self.iotype} {self.pull}>"

    def __repr__(self):
        return f"<'{self.rpins}':'{self.name}'>"
...
class LogicIoDef(LogicIoDef):
    ...
    # not pins
    def __init__(self, name:str, direction=Direction.IN, width=1, iotype=IoType.LV33, pull=Pull.FLOAT, *, namegs=None, note:str=""):
