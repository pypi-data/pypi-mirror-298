...
class FrascoPush(Extension):
    ...
    def _init_app(self, app, state):
        ...
        @click.option('--reuse-addr', is_flag=True)
        @click.option('--presence-session-id')
        def cli_server(port=None, access_logs=False, debug=False, presence_session_id=None, reuse_addr=False):
            ...
            run_server(port or state.options["server_port"], debug=debug or app.debug or app.testing, access_logs=access_logs,
                ...
                presence_session_id=presence_session_id, reuse_addr=reuse_addr)
