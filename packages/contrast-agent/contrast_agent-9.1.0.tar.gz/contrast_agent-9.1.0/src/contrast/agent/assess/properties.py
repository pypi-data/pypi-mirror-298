...
class Properties:
    ...
    def tags_to_ts_obj(self, tag_override: Optional[Iterable[Tag]] = None):
        """
        Convert Tag objects to TraceTaintRange objects for teamserver

        tag_override can be used to override the spans of tags. All tags will
        use the same spans. This is used representing tags on non-string types
        or truncated values.
        """
        if tag_override:
            range_strs = [f"{tag.start_index}:{tag.end_index}" for tag in tag_override]
            return [
                TraceTaintRange(tag=label, rng=range_str)
        ...
        for label, tags in self.tags.items():
            ...
            for tag in tags:
                taint_range = TraceTaintRange(
                    label, f"{tag.start_index}:{tag.end_index}"
                )
    ...
    def report_source(self, event):
        ...
        context.observed_route.sources.append(source_event_obj)
