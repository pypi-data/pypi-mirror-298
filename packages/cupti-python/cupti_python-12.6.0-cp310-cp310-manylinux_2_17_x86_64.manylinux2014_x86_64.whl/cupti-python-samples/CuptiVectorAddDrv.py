...
from common import checkCudaErrors, get_gpu_architecture
...
def main(argv):
    ...
    # Prepare Host Data
    hX = np.ones(common.vector_length, dtype=np.int32)
    hY = np.ones(common.vector_length, dtype=np.int32)
    hOut = np.empty_like(hX, dtype=np.int32)
    BUFFER_SIZE = common.vector_length * hX.itemsize
    ...
    n = np.array(common.threads_per_block * common.blocks_per_grid, dtype=np.uint32)
    args = [dX, dY, dOut, n]
    ...
    checkCudaErrors(
        cuda.cuLaunchKernel(
            kernel,
            common.blocks_per_grid,  # grid x dim
            1,  # grid y dim
            1,  # grid z dim
            common.threads_per_block,  # block x dim
            1,  # block y dim
            1,  # block z dim
    ...
    if common.verify_result(hX, hY, hOut) != 0:
        return -1
