...
def driver_api_callback(user_data, domain, callback_id, cbdata):
    ...
    if domain == cupti.CallbackDomain.DRIVER_API:
        ...
        if callback_id == cupti.CallbackIdState.FATAL_ERROR:
            print(f"notification.result = {cupti.Result(cbdata.notification.result).name}")
            print(f"notification.message = {cbdata.notification.message}")


def display_api_records(driver_api_records, prof_output):
    if prof_output == common.ProfOutput.BRIEF:
        print(
            "{:<20} {:<20} {:<20} {:<20} {}".format(
    ...
    elif prof_output == common.ProfOutput.DETAILED:
        for record in driver_api_records:
            pprint.pp(record)  # pretty print the driver_api record


def main(argv):
    command_line_config: common.CommandLineConfig = common.get_app_args(
        argv,
        "hpo:",
        ["help", "profile", "output="],
    ...
    A = np.random.rand(common.vector_length)
    B = np.random.rand(common.vector_length)
    C = np.zeros_like(A)

    print(f"vector_length: {common.vector_length}")
    print(f"threads_per_block: {common.threads_per_block}")
    print(f"blocks_per_grid: {common.blocks_per_grid}")

    vector_add[common.blocks_per_grid, common.threads_per_block](A, B, C)
    ...
    if common.verify_result(A, B, C) != 0:
        return -1

    if common.global_error_count != 0:
        return -2
