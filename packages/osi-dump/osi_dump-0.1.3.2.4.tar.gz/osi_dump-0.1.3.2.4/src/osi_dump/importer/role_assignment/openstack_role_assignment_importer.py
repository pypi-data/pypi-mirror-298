...
class OpenStackRoleAssignmentImporter(RoleAssignmentImporter):
    ...
    def _get_role_assignment_info(
        ...
        """
        - Added enabled field to Role Assignment sheet 
        - Should be removed in the future when User Sheet is added 
        """
        user_enabled = None
        try:
            user = self.connection.identity.get_user(user=user_id)
            user_enabled = user.is_enabled
        except Exception as e: 
            logger.warning(f"Can not get user_enabled: {e}")
