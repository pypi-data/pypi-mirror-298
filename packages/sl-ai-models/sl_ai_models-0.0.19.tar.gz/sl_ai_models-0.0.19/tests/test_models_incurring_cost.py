...
async def find_number_of_hard_limit_exceptions_in_run(mocker: Mock, subclass: type[AiModel], max_cost: float, number_of_calls_to_make: int, number_of_expected_exceptions: int) -> int:
...
async def test_error_thrown_with_many_calls_and_tiny_limit(mocker: Mock, subclass: type[AiModel]) -> None:
    if not issubclass(subclass, IncursCost):
        raise ValueError(NOT_INCURS_COST_ERROR_MESSAGE)
    ...
    num_hard_limit_errors = await find_number_of_hard_limit_exceptions_in_run(mocker, subclass, max_cost, number_of_calls_to_make, number_of_expected_exceptions)
...
async def test_error_thrown_when_many_calls_with_a_larger_than_0_cost_limit(mocker: Mock, subclass: type[AiModel]) -> None:
    if not issubclass(subclass, IncursCost):
        raise ValueError(NOT_INCURS_COST_ERROR_MESSAGE)
    ...
    num_hard_limit_errors = await find_number_of_hard_limit_exceptions_in_run(mocker, subclass, max_cost, number_of_calls_to_make, number_of_expected_exceptions_given_perfect_cost_prediction)
    assert num_hard_limit_errors >= number_of_expected_exceptions_given_perfect_cost_prediction
