...
def test_download_file(tmp_path):
    ...
    url = "https://www.example.com/file/definition.json"
    dest = tmp_path / "build_definition.json"
    req_mock = Mock()

    # case: successful response
    req_mock.status_code = 200
    req_mock.json.return_value = {"sources": {"repo": {}}}
    with patch("requests.get") as request:
        request.return_value = req_mock
        assert download_file(url, dest) == dest
    req_mock.json.assert_called_once()
    ...
    request.assert_called_once_with(url)
    assert req_mock.json.call_count == 0
