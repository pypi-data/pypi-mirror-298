...
IMAGE_dst_mesh_TYPE dst_mesh) {
  ...
  for (int pointBIndex = pointIndex + 1; pointBIndex < num_pointBs; pointBIndex++) {
    float touchingA = READ_src_touch_matrix_IMAGE(src_touch_matrix, sampler, POS_src_touch_matrix_INSTANCE(pointIndex + 1, pointBIndex + 1, 0, 0)).x;
    float touchingB = READ_src_touch_matrix_IMAGE(src_touch_matrix, sampler, POS_src_touch_matrix_INSTANCE(pointBIndex + 1, pointIndex + 1, 0, 0)).x;

    const float touching = max(touchingA, touchingB);
