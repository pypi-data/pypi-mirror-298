...
class strInt:
    ...
    def select_cells(self, user_sc_exp = None, user_sc_agg_meta = None, p = 0.1, mean_num_per_spot = 10, mode = 'strict', max_rep = 3, repeat_penalty = 10):
        ...
        if user_sc_agg_meta is not None:
            ...
            # TODO scale the expression data
    ...
    def run_gradient(self):
        ...
        def check(alter_exp, sc_agg_meta):
            fake_gene = [i for i in alter_exp.columns if 'true' in i]
            max_not_exp = 0
            max_e = 0
            gene_not_exp = 0
            df = pd.DataFrame()
            for gene in fake_gene:
                a = alter_exp[gene]
                b = sc_agg_meta.loc[a[a>0].index]['celltype']
                tp1 = gene.split('_')[1]
                tp2 = gene.split('_')[2]
                if b[~b.isin([tp1,tp2])].index.shape[0] > 0:
                    num = len(b[~b.isin([tp1,tp2])].index)
                    max_e = alter_exp.loc[b[~b.isin([tp1,tp2])].index,gene].max()
                    df = df.append(pd.DataFrame({'gene':[gene],'num':[num],'max_e':[max_e]}))
                    print(gene,num,max_e)
                if max_e > max_not_exp:
                    max_not_exp = max_e
                    gene_not_exp = gene
            print(max_not_exp,gene_not_exp)
            print('-----------------')
            return df
        df = check(self.alter_sc_exp, self.sc_agg_meta)
