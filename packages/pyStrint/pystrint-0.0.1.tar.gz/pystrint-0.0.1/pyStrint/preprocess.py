...
def make_adata(mat,meta,species,save_path = False, save_adata = False):
...
def prep_adata(sc_exp=None, st_exp=None, sc_distribution=None, 
                             sc_meta=None, st_coord=None, lr_df=None, SP='Human'):
    '''
    Prepares the single-cell (sc) and spatial transcriptomics (st) data for analysis.
    
    This function checks the number of genes in st_exp and calls the appropriate
    preparation function based on the gene count.
    
    1. Remove unexpressed genes.
    2. Select shared genes.
    3. Transform to AnnData format.
    '''
    if SP not in ['Human', 'Mouse']:
        raise ValueError('Species should be chosen among either Human or Mouse.')
    
    if lr_df is None:
        lr_df = load_lr_df(species=SP)
    # Check the number of genes in st_exp and call the appropriate preparation function
    if st_exp.shape[1] < 1e4:
        # Use the MERFISH-specific preparation method
        return prep_all_adata_merfish(sc_exp, st_exp, sc_distribution, sc_meta, st_coord, lr_df, SP)
    else:
        # Use the standard preparation method
        return prep_all_adata(sc_exp, st_exp, sc_distribution, sc_meta, st_coord, lr_df, SP)
