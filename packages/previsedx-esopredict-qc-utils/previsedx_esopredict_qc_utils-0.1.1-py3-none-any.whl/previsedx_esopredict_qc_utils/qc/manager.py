...
class Manager:
    ...
    # def _run_ntc_empty_quantity_mean_checks(self, check_id: str = "ntc_checks") -> None:
    ...
    #         if record.sample_id.upper().startswith("NTC"):
    #             # TODO: Ask Lisa abou EXT-NEG
    #             if (record.hpp1_nmv is None or pd.isna(record.hpp1_nmv)) or record.hpp1_nmv == 0:
    #                 logging.info(f"HPP1_NMV value {record.hpp1_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 pass_list.append(f"HPP1_NMV value {record.hpp1_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #             else:
    #                 logging.error(f"HPP1_NMV value '{record.hpp1_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 fail_list.append(f"HPP1_NMV value '{record.hpp1_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 error_ctr += 1

    #             if record.p16_nmv is None or pd.isna(record.p16_nmv):
    #                 logging.info(f"P16_NMV value {record.p16_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 pass_list.append(f"P16_NMV value {record.p16_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #             else:
    #                 logging.error(f"P16_NMV value '{record.p16_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 fail_list.append(f"P16_NMV value '{record.p16_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 error_ctr += 1

    #             if record.runx3_nmv is None or pd.isna(record.runx3_nmv):
    #                 logging.info(f"RUNX3_NMV value {record.runx3_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 pass_list.append(f"RUNX3_NMV value {record.runx3_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #             else:
    #                 logging.error(f"RUNX3_NMV value '{record.runx3_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 fail_list.append(f"RUNX3_NMV value '{record.runx3_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 error_ctr += 1

    #             if record.fbn1_nmv is None or pd.isna(record.fbn1_nmv):
    #                 logging.info(f"FBN1_NMV value {record.fbn1_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 pass_list.append(f"FBN1_NMV value {record.fbn1_nmv} is empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #             else:
    #                 logging.error(f"FBN1_NMV value '{record.fbn1_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 fail_list.append(f"FBN1_NMV value '{record.fbn1_nmv}' should be empty or 'ND' or 0 for sample '{record.sample_id}' in the intermediate tab-delimited file")
    #                 error_ctr += 1
