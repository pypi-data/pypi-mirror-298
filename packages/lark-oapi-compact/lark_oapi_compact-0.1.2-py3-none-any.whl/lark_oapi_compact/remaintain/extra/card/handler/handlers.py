...
from ..model.card import Challenge, Header, Card
...
class CardHandlers:
    ...
    def validate(self):  # type: () -> None
        ...
        signature = CardHandlers.__signature(
            self.header.nonce, self.header.time_stamp, body, settings.verification_token
        )
    ...
    def __signature(nonce, timestamp, body, token):  # type: (str, str, str, str) -> str
        buf = "{}{}{}{}".format(timestamp, nonce, token, body)
        sha1_obj = sha1()
    ...
    def parse(self):  # type: () -> None
        ...
        if httpCard.type == CALLBACK_TYPE_CHALLENGE and conf.app_settings.verification_token != challenge.token:
            raise TokenInvalidError("AppSettings.verificationToken not equal token(%s)" % challenge.token)
    ...
    def complement(self, exception):  # type: (Union[None, Exception]) -> None
        ...
        if exception is not None:
            ...
            conf.logger.error("request_id={} err_msg={}".format(self.ctx.get_request_id(), err_msg))
            return
