...
class EventHandlers:
    ...
    def parse(self):  # type: () -> None
        ...
        if token != conf.app_settings.verification_token:
            raise RuntimeError(
                "New token invalid, received={}, configured={}".format(token, conf.app_settings.verification_token)
            )
    ...
    def handler(self):  # type: () -> None
        ...
        if h is None:
            raise HandlerNotFoundError("event-type:%s, handler not found" % event.event_type)
    ...
    def complement(self, exception):  # type: (Union[None, Exception]) -> None
        ...
        if exception:
            if isinstance(exception, HandlerNotFoundError):
                ...
                self.write_response(HTTP_STATUS_OK, DEFAULT_CONTENT_TYPE, body.encode("utf-8"))
            else:
