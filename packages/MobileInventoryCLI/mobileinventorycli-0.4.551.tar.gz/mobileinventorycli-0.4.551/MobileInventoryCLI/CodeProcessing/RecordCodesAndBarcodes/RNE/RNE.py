	...
	def dates4barcode(self):
		with Session(ENGINE) as session:
			while True:
				...
				dates_ct=len(dates)
				for num,i in enumerate(dates):
					print(f"{Fore.light_green}{num}/{Fore.light_yellow}{num+1} of {Fore.light_red}{ct}{Fore.dark_goldenrod} -> {Fore.light_magenta}{i.Name}|{Fore.plum_2}{i.Barcode}|{Fore.cyan}{i.BB_Expiry.strftime("%m/%d/%Y")}|{Fore.medium_violet_red}{i.DTOE.strftime("%m/%d/%Y")}{Style.reset}")
				msg=f"{Fore.light_green}Zero-Index/{Fore.light_yellow}Number of {Fore.light_red}Total{Fore.dark_goldenrod} -> {Fore.light_magenta}Name|{Fore.plum_2}Barcode|{Fore.cyan}BB_Expiry|{Fore.medium_violet_red}DTOE{Style.reset}"
				print(msg)
