        ...
        with Session(ENGINE) as session:
            ...
            for num,exp in enumerate(exps):
                ...
                if entry_result == None:
                    ptext=f"{Fore.light_green}{num}/{Fore.light_yellow}{num+1}/{Fore.light_red}{exps_ct} - {Fore.light_magenta}No Entry found to match {exp.Barcode}|{exp.Name} [[c]reate] Entry/[[s]kip]|[[i]gnore]|<Enter|Return>/d[[r]elete] PC"
                    helpText=ptext
                    while True:
