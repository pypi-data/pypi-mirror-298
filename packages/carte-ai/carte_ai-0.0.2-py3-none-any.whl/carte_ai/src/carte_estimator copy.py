...
# Load the FastText model from Hugging Face
fasttext_model_name = "facebook/fasttext-language-identification"
fasttext_tokenizer = AutoTokenizer.from_pretrained(fasttext_model_name)
fasttext_model = AutoModel.from_pretrained(fasttext_model_name)

class BaseCARTEEstimator(BaseEstimator):
    """Base class for CARTE Estimator with Hugging Face FastText integration."""
    ...
    def __init__(
        ...
        *,
        ...
        self.fasttext_tokenizer = fasttext_tokenizer

    def _get_fasttext_embeddings(self, texts):
        """Convert texts to FastText embeddings using Hugging Face model."""
        inputs = self.fasttext_tokenizer(texts, return_tensors='pt', padding=True, truncation=True)
        inputs = {key: val.to(self.device) for key, val in inputs.items()}
        with torch.no_grad():
            outputs = self.fasttext_model(**inputs)
        # Use the last hidden state for embeddings, normalize them
        embeddings = outputs.last_hidden_state.mean(dim=1)
        embeddings = normalize(embeddings, p=2, dim=1)  # L2 normalize
        return embeddings

    def _run_step(self, model, data, optimizer):
        """Run a step of the training with FastText embeddings."""
        optimizer.zero_grad()  # Clear gradients.
        data.to(self.device_)  # Send to device
        out = model(data)  # Perform a single forward pass.
        target = data.y  # Set target

        # Use FastText embeddings for target texts
        target_texts = [str(t) for t in target.cpu().numpy()]  # Assuming target texts are numeric labels
        fasttext_embeddings = self._get_fasttext_embeddings(target_texts)

        # Combine FastText embeddings with model output (example logic)
        out = torch.cat((out, fasttext_embeddings), dim=-1)
    ...
    def _eval(self, model, ds_eval):
        """Run an evaluation of the input data on the input model with FastText embeddings."""
        with torch.no_grad():
            model.eval()
            out = model(ds_eval)
            target = ds_eval.y

            # Use FastText embeddings for evaluation targets
            target_texts = [str(t) for t in target.cpu().numpy()]
            fasttext_embeddings = self._get_fasttext_embeddings(target_texts)

            # Combine FastText embeddings with model output
            out = torch.cat((out, fasttext_embeddings), dim=-1)
