...
class RulesChecker:
    ...
    def __init__(self, rules: Union[AIImageElementRules, ImageElementRules, MemeElementRules, AIVideoElementRules, VideoElementRules, SoundElementRules, CustomStockElementRules, StockElementRules, YoutubeVideoElementRules, TextElementRules, PremadeElementRules]):
        self.rules = rules
    ...
    @classmethod
    def check_need_rules(cls, element: dict, rules: Union[AIImageElementRules, ImageElementRules, MemeElementRules, AIVideoElementRules, VideoElementRules, SoundElementRules, CustomStockElementRules, StockElementRules, YoutubeVideoElementRules, TextElementRules, PremadeElementRules]):
        """
        Receives the 'element' dictionary that must contain the fields that
        are mandatory by the rules configuration. This method will raise an
        Exception if any rule is broken.
        ...
        if not isinstance(rules, (ElementRules.get_rules_classes())):
            raise Exception(f'The provided "rules" parameter is not a valid ElementRules subclass. The valid ones are: {', '.join(ElementRules.get_rules_classes().__str__)}.')
