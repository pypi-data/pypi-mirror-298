...
def get_spaces(space_type: str):
    # Define sensor and action spaces
    if space_type == "discrete":
        sensor_space = gym.spaces.Discrete(MAX_COUNTER + 1)  # 0-MAX_COUNTER (just return the counter)
        action_space = gym.spaces.Discrete(2)  # Increment by 1, or reset to 0
    elif space_type == "multidiscrete":
        sensor_space = gym.spaces.MultiDiscrete([MAX_COUNTER + 1])
        action_space = gym.spaces.MultiDiscrete([2, 3])
    elif space_type == "multibinary":
        sensor_space = gym.spaces.MultiBinary(7)  # Binary representation of MAX_COUNTER
        action_space = gym.spaces.MultiDiscrete([2, 3])  # Increment or reset
    elif space_type == "box":
        sensor_space = gym.spaces.Box(low=np.array([0]), high=np.array([MAX_COUNTER]), dtype=np.float32)
        action_space = gym.spaces.Box(low=np.array([0]), high=np.array([1]), dtype=np.float32)  # Increment by 1 or reset
    elif space_type == "dictionary":
    ...
    return sensor_space, action_space
...
class Sim(gym.Env):
    ...
    def __init__(self, env_init: dict = {}):
        ...
        self.sensor_space, self.action_space = get_spaces(self.space_type)
        # Print Debug
