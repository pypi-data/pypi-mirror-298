...
class mechanism29(bmutils):
    ...
    def setup_plot(self):
        self.ax.set_title(f'Mechanism 29')
    ...
    def plot(self, theta = 0):    
        ...
        for index, point in enumerate(segment_endpoints):
            if index%2 == 0:
                continue
        ...
        for index, instance in enumerate(self.instances):
            ...
            active_segment_type = -1
            
            for seg_index, segment_instance in enumerate(segment_instances):
                gear_is_active = self.is_point_on_circle(R_intersect, segment_instance["segment"][0], segment_instance["segment"][1])
                if gear_is_active:
                    active_segment_type = segment_instance["type"]            
                           
            _d_theta_2 = R1/R2 * d_theta_1
            if active_segment_type == 0: 
                active_rack_r = R_B
                active_planet_r = R2
            else:
                active_planet_r = R4
    ...
    def Settings(R1=5,R2=20, return_speed=2, frequency = 1, animation_rounds=1, speed=1):
        ...
        instances = [{"theta":0}]
        segment_instances = [{"theta":theta, "type":index%2} for index, theta in enumerate(theta_points)]
