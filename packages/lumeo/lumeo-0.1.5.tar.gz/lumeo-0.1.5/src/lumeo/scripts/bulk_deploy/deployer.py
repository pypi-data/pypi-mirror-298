    ...
    async def create_or_queue_regular_deployment(self, input_stream):
        ...
        async with self.queuing_lock: 
            ...
            if pipeline:
                ...
                if self.gateway_id:
                    success, _ = await self.create_deployment(input_stream['id'], self.pipeline_id, deployment_configuration, deployment_name)
                else:
                    success, _ = await self.queue_deployment(None, input_stream['id'], self.pipeline_id, deployment_configuration, deployment_name)                
            else:
    ...
    async def process(self):
        ...
        if self.file_uri:
            ...
            if input_stream and camera:
                ...
                success = await self.create_or_queue_regular_deployment(input_stream)
