...
opt_pattern = re.compile(r"{opt:(\w+)(?::([^}]*}))?}")
full_pattern = re.compile(r"^(?:\[out:(\w+)\])?(?:\[err:(\w+)\])?(.*)")


def shcmd(template: str, options: dict[str, str], strips: str | None = None):
    """generate a command line using the template string

    TEMPLATE= anystring|OPTIONAL|TEMPLATE
    OPTIONAL = {opt:KEY[:KEYPATTERN]}
    KEY= str
    KEYPATTERN = anystring|KEY|KEYPATTERN

    examples:
        template="ls {opt:PATH}" with options={'PATH': '/path'} => 'ls /path'
        template="ls {opt:PATH}" with options={'ANOTHER': '/path'} => 'ls '
        template="ls {opt:PATH: -l {PATH}}" with options={'PATH': '/path'} => 'ls -l /path'
        template="ls {opt:PATH: -l {PATH}}" with options={'ANOTHER': '/path'} => 'ls'
    ...
    for opt in opt_pattern.finditer(template):
    ...
    splitter = full_pattern.search(cmd)
    if splitter:
        out, err, cmd = splitter.groups()

    return (out, err), (cmd % options).strip(strips)
