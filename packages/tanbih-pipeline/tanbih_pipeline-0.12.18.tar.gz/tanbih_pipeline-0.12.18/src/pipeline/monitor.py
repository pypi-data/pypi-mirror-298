...
class Monitor:
    ...
    def __new__(cls, *args, **kwargs):
        if Monitor._instance is None:
            Monitor._instance = super(Monitor, cls).__new__(cls)
    ...
    def use_counter(self, name, description, labels=list()):
    ...
    def state(self, name, state, labels={}):
        if labels:
            self.metrics[name].labels(**labels).state(state)
        else:
            self.metrics[name].state(state)

    def use_histogram(self, name, description, labels=[]):
        if name not in self.metrics:
    ...
    def histogram(self, name, labels=[]):
        if labels:
            return self.metrics[name].labels(**labels)
        else:
            return self.metrics[name]
...
class WorkerMonitor:
    ...
    def __getattribute__(self, name: str):
        # for attributes not defined in WorkerMonitor,
        # return the attribute from Monitor self.metrics
        try:
    ...
    def record_worker_info(self) -> None:
        self.info(
            ...
            labels={
                ...
                "in-topic": self.worker.source.topic if self.worker.has_input() else "",
