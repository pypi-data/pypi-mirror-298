...
from Orange.widgets.utils.concurrents import (
    Task,
    FutureWatcher,
    ThreadExecutor,
    methodinvoke,
...
class OWGOBrowser(widget.OWWidget):
    ...
    inputs = [
        (
        i18n.t("bioinformatics.common.cluster_data"),
            Orange.data.Table,
            "set_dataset",
    ...
    outputs = [
        (i18n.t("bioinformatics.common.select_gene_data"), Orange.data.Table),
    ...
    def __init__(self, parent=None):
        ...
        box = gui.widgetBox(
            self.filterTab, __("box.annotation_evidence_code"), addSpace=True
        )
        ...
        if not self.use_reference_dataset or self.ref_data is None:
            ...
            if ref_count == 0:
                ...
                self.referenceRadioBox.buttons[1].setText(
                    __("text.ref_count").format(ref_count)
                )
    ...
    def show_info(self):
        ...
        label = QLabel(dialog)
        label.setText(
            __("text.ontology") + self.ontology.header
            if self.ontology
            else __("text.ontology_not_loaded"
        ))
