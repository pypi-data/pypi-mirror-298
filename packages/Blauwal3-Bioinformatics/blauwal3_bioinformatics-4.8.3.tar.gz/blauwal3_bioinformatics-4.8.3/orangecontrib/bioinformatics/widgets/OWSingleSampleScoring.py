...
def __(key):
    return i18n.t('bioinformatics.OWSingleSampleScoring.' + key)
...
class OWSingleSampleScoring(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        box = gui.widgetBox(self.controlArea, __('box.Method'), margin=0)
        gui.comboBox(
            box,
            self,
            'scoring_method',
        ...
        self.commit_button = gui.auto_commit(
            self.buttonsArea, self, 'auto_commit', __("box.commit"), box=False
        )
