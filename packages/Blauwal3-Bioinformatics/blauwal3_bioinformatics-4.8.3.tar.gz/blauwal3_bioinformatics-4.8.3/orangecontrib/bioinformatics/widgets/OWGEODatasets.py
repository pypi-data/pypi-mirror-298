...
def run_gds_info_task(state: TaskState):
    state.set_status(__("state.fetching"))
    gds_info = GDSInfo()
...
class OWGEODatasets(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        radioButtonsInBox(
            ...
            self,
            'genes_as_rows',
            [__("btn_sample_row"), __("btn_gene_row")],
            callback=self._run,
        )
        separator(box)
        ...
        self.annotations_widget.setHeaderLabels(
            [__("label.type"), __("label.sample_count")]
        )
