...
class OWDifferentialExpression(widget.OWWidget):
    ...
    def __init__(self, parent=None):
        ...
        check = gui.checkBox(
            ...
            self,
            "compute_null",
            __("checkbox_compute_null_distribution"),
                             callback=self.update_scores,
        )
        ...
        pval_select = gui.button(
            box1, self, __("btn_select"), callback=self.select_p_best, autoDefault=False
        )
    ...
    def update_selected_info_label(self):
        if self.data is not None and self.scores is not None:
            ...
            defined_txt = __("text.undefined_score").format(
                count_undef, count_scores
            )
