...
def __(key):
    return i18n.t('bioinformatics.resolwe.' + key)
...
class SignIn(QDialog):
    def __init__(self, flags, *args, server_type: str = 'resolwe', **kwargs):
        ...
        self.setWindowTitle(__("title.sign_in"))
        self.setFixedSize(400, 250)

        self.server_cb_label = QLabel(__("label.server"))
        ...
        self.username_label = QLabel(__("label.username"))
        self.username_line_edit = QLineEdit(self)
        self.username_line_edit.setPlaceholderText(__("label.username_tip"))
        ...
        self.password_label = QLabel(__("label.password"))
        self.password_line_edit = QLineEdit(self)
        self.password_line_edit.setPlaceholderText(__("label.password_tip"))
        ...
        self.sign_in_btn = QPushButton(__("btn.sign_in"), self)
        self.sign_in_btn.setDisabled(True)
        self.sign_in_btn.clicked.connect(self.sign_in)

        self.error_msg = QLabel(__("label.err"))
...
class FilterByDateModified(IntEnum):
    ...
    @staticmethod
    def labels():
        return [__("time.any_time"), __("time.past_hour"), __("time.past_24_hours"), __("time.past_week"),
                __("time.past_month")]
...
class CollapsibleFilterComponent(OWComponent, QObject):
    ...
    FILTER_FULL_TEXT_LABEL = __("filter.search")
    TOGGLE_BTN_LABEL = __("filter.narrow_search")
    FILTER_NAME_LABEL = __("filter.filter_name")
    FILTER_CONTRIB_LABEL = __("filter.contributor")
    FILTER_OWNER_LABEL = __("filter.owner")
    FILTER_MODIFIED_LABEL = __("filter.date_modified")
    SORTING_LABEL = __("filter.sorting")
...
class PaginationComponent(OWComponent, QObject):
    ...
    PAGE_LIMIT_LABEL = __("label.per_page")
...
class NormalizationComponent(OWComponent, QObject):
    ...
    BOX_TITLE = __("label.normalization")
    QUANTILE_NORM_LABEL = __("label.quantile_normalization")
    LOG_NORM_LABEL = __("label.log_norm")
    Z_SCORE_LABEL = __("label.z_score")
    QUANTILE_TRANSFORM_LABEL = __("label.quantile_transform")
