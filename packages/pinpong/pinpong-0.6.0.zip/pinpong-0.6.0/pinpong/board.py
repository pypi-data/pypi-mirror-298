...
class SYSFSPin:
  ...
  def irq(self, trigger, handler):
    self.old_value = self.read_digital()
    self.board.res["irq"](self,trigger,handler)
  
  def my_function(self):
    while True:
      #上升沿
      a_value = self.read_digital()
...
class SYSFSPWM:
  ...
  def freq(self, v=None):
    if v == None:
      ...
      os.system('echo 0 > '+self.pwm_path+self.channel+'/duty_cycle')
      os.system('echo '+str(self.period_ns)+' > '+self.pwm_path+self.channel+'/period')
  ...
  def duty(self, v=None):
    if v == None:
      ...
      duty_ns = int(self.period_ns*(100-v)/100.0)
      '''
      ...
      if(self.board.boardname == "UNIHIKER"):
        # 防止重复配置，重复配置相同的周期无效
        if self.duty_value != duty_ns:
          self.duty_value = duty_ns
          os.system('echo normal > '+self.pwm_path+self.channel+'/polarity')  #此逻辑必须设置PWM极性相反
          os.system('echo inversed > '+self.pwm_path+self.channel+'/polarity')  #此逻辑必须设置PWM极性相反
        else:
          return self.duty_value 
      else:
        os.system('echo inversed > '+self.pwm_path+self.channel+'/polarity')
...
class PWM:
  ...
  def freq(self, v=None):
    ...
    if v == None:
      ...
      if(self.board.boardname == "UNIHIKER"):
        #print('echo '+str(self.freq_value)+' > '+self.pwm_path+self.channel+'/period')
         
        fd = os.open(self.period_path, os.O_RDONLY)
        data = os.read(fd, 12).decode('utf-8')
        os.close(fd)
        period_data = int(data)
        fd = os.open(self.duty_path, os.O_RDONLY)
        data = os.read(fd, 12).decode('utf-8')
        os.close(fd)
        duty_data = int(data)
        
        if self.freq_value < duty_data:
          os.system('echo 0 > '+self.pwm_path+self.channel+'/duty_cycle')
          return
        os.system('echo '+str(self.freq_value)+' > '+self.pwm_path+self.channel+'/period')
        os.system('echo 0 > '+self.pwm_path+self.channel+'/duty_cycle')
        #print("period --------")
        if self.isStart == False:
          fd = os.open(self.enable_path, os.O_RDONLY)
          data = os.read(fd, 12).decode('utf-8')
          os.close(fd)
          enable_data = int(data)
          # 已经使能需要先禁止
          if enable_data == 1:
            os.system('echo 0 > '+self.pwm_path+self.channel+'/enable')
          os.system('echo 1 > '+self.pwm_path+self.channel+'/enable')
  ...
  def duty(self, v=None):
    if v == None:
      ...
      self.duty_value = v
    if self.duty_value > self.freq_value:
      self.duty_value = self.freq_value
    ...
    if v == None:
      ...
      if(self.board.boardname == "UNIHIKER"):
        ...
        duty_data = int(data)        
        #print("duty   -------")
        if period_data < self.duty_value:
          print("pwm  error period less duty_cycle no match")
          return
        os.system('echo '+str(self.duty_value)+' > '+self.duty_path)
        if self.isStart == False:
          #print('echo 1 > /sys/class/pwm/pwmchip0/pwm'+self.channel+'/enable')
          #os.system('echo 1 > '+self.pwm_path+self.channel+'/enable')
          #print('echo 1 > '+self.pwm_path+self.channel+'/enable')
  ...
  def deinit(self):
    if(self.board.boardname == "UNIHIKER"):
      ...
      #if os.path.exists("/sys/class/pwm/pwmchip2/pwm"+self.channel):
        #os.system('echo '+self.channel+ ' > /sys/class/pwm/pwmchip2/unexport')
...
class LinuxSPI:
  def __init__(self, board, bus_num=0, device_num=0, baudrate=31200000):
    self.board = board
    board.res["SPI"](bus_num, device_num)
    self.spi = spidev.SpiDev(bus_num, device_num)
    self.spi.open(bus_num, device_num)
    self.spi.max_speed_hz = 12900000
    self.spi.mode = 0
...
class SPI:
  ...
  def __init__(self, board=None, bus_num=0, device_num=0, sck=None, mosi=None, miso=None, cs=None, baudrate=25600000, polarity=0, phase=0, bits=8):
  ...
  def read(self, num, default_value=0xff):
    tx_buf= [0]*num
    ret = self.spi.xfer3(tx_buf)
    return ret

  def readinto(self, buf):
    num = len(buf)
    tx_buf= [0]*num
    buf[:] = self.spi.xfer3(tx_buf)

  

  def send_large_buffer(self, buf):
    # 将缓冲区数据拆分为小于等于 MAX_CHUNK_SIZE 的块
    MAX_CHUNK_SIZE = 4096
...
class DuinoI2C:
  ...
  def readfrom_mem(self, i2c_addr, reg, read_byte, stop=True):
    return self.board.board.i2c_read(i2c_addr, reg, read_byte, None)

  def readfrom_mem_restart_transmission(self, i2c_addr, reg, read_byte, stop=False):
...
class LinuxI2C:
  ...
  def readfrom_mem(self, i2c_addr, reg, read_byte,stop=True):
    ...
    msg = []
    buf.append(reg)
    msg.append(writing(i2c_addr, buf, stop))
    msg.append(reading(i2c_addr, read_byte))
    return self.i2c.read_mem(msg, read_byte)
  
  # 调用此函数流程（不发送停止位读取数据）
  # write  addr  reg  no stop
  # read   addr  xxx  stop
  def readfrom_mem_restart_transmission(self, i2c_addr, reg, read_byte, stop=False):
    return self.readfrom_mem(i2c_addr, reg, read_byte, stop)
...
class I2C:
  ...
  def readfrom_mem(self, i2c_addr, reg, read_byte, stop=True):
    return self.obj.readfrom_mem(i2c_addr, reg, read_byte, stop)

  def readfrom_mem_restart_transmission(self, i2c_addr, reg, read_byte, stop=False):
    return self.obj.readfrom_mem_restart_transmission(i2c_addr, reg, read_byte, stop)
...
class TTYUART:
  def __init__(self, board, tty_name, baud_rate):
    self.board = board
    board.res["UART"](tty_name)
    self.tty_name = tty_name
    #self.uart = serial.Serial(tty_name, baud_rate, timeout=0.1)
    self.uart = None
    
  def init(self, baud_rate = 115200, bits = 0, parity = 0, stop = 1):
    self.uart = serial.Serial(self.tty_name, baudrate=baud_rate, timeout=0.01)

  def deinit(self):
    if self.board.boardname == 'UNIHIKER':
      board.res["pinReuse"](202, STATE_GPIO)
      board.res["pinReuse"](203, STATE_GPIO)
    pass
...
class UART:
  def __init__(self, board=None, tty_name="", bus_num=1, baud_rate=9600):
    ...
    if self.board.boardname == 'UNIHIKER':
      self.uart_num = "/dev/ttySP0"   #x新版本unihiker 使用的串口
      board.res["UART"](self.uart_num)
...
class IRRecv(IRRecvExtension):
  def __init__(self, board=None, pin_obj=None, callback=None):
    ...
    self.board = gboard
    if board.boardname == 'UNIHIKER':
      IRRecvExtension.__init__(self, board, pin_obj, callback)
    else:
...
class IRRemote(IRRemoteExtension):
  def __init__(self, board=None, pin_obj=None):
    if isinstance(board, Pin):
      num = pin_obj
      pin_obj = board
    ...
    self.board = board
    if board.boardname == 'UNIHIKER':
      IRRemoteExtension.__init__(self, board, pin_obj)
    else:
...
class LinuxTone:
  ...
  def on(self):
    if self.board.boardname == 'UNIHIKER':
      #self.pwm.duty(int(1000000/self.freq_value))
      self.pwm.duty(int(1000000/self.freq_value/2))
    else:
...
class Tone:
  ...
  def freq(self, v=None):
    if v < 0:
      sys.exit("Error: Frequency cannot be negative.")
    if v == None:
      return self.obj.freq(v)
    else: 
      self.obj.freq(v)
...
class Board:
  def __init__(self, boardname="", port=None):
    ...
    self.modbus = {}
    self.spi = [[None, None], [None, None], [None, None], [None, None], [None, None]]
    signal.signal(signal.SIGINT, self._exit_handler)


    if self.boardname != "":
      self.__boardname = boardname.upper()
      pass
      
    if boardname == "" and port == None:
      # 不传端口和开发板名 ,自动识别板子
      find_board(self)
    elif boardname == "" and port != None:
      # 不传入开发板名, 传端口，识别板子开发板名
      find_board(self)
      self.port = self.__port
    elif boardname != "" and port != None:
      # 根据传入的开发板名和端口号初始化
      self.boardname = self.__boardname
      self.port = self.__port
    elif boardname != "" and port == None:
      # 传入开发板名,不传端口，识别板子端口
    ...
    if self.port == None or self.boardname == "":
      #self.res = None
      print("Please connect the development board !!!")
      print("Please connect the development board !!!")
      print("Please connect the development board !!!")
      exit(1)
  ...
  def begin(self, firmware = 0):
    ...
    if self.boardname == "UNIHIKER":
      ...
      self.res["begin"](self) # begin 不能在烧录之前
      pass
    else:
  ...
  def detect_firmata(self):
    ...
    self.res["reset"]() #复位
    if self.boardname == "UNIHIKER":
      print("new pinpong unihiker special opertior ")
    else:
