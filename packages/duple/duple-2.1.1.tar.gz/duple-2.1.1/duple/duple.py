...
def followlog():
    ...
    log_path = Path(LOGS_PATH).joinpath("log.jsonl")
    ...
    with open(log_path, "r") as file:
        for line in follow(file):
            ...
            style_level = click.style(f"[{parsed['level']}]", fg=level_formats[parsed["level"]])
            style_module = click.style(parsed["module"], fg="white")
            style_function = click.style(parsed["function"], fg="white")
            style_message = click.style(parsed["message"], fg=level_formats[parsed["level"]])
            style_msg_pointer = click.style("->", fg="bright_blue")

            click.secho(f"{style_ts} {style_level} {style_module}.{style_function} {style_msg_pointer} {style_message}")
