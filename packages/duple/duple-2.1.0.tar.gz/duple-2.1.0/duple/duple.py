...
from duple.decorators import log_func_with_args
...
def cli():
    setup_logging()
    flag = "".ljust(20, "=")
    logger.debug(f"{flag} Starting Command Line Interface Session {flag}")
    pass
...
def wherelog():
    ...
    click.secho(f"{LOGS_PATH}")


@cli.command()
def followlog():
    """follow the log until user interupts (ctrl-c), (tail -f)"""
    log_path = Path(LOGS_PATH).joinpath("log.log")
...
def scan(
    ...
    """
    start = perf_counter()
    logger.debug('Starting "duple scan"')
    ...
    if path == "":
        ...
        logger.debug("Finished reading file list from stdin")
    else:
        filelist = list()
        ...
        logger.debug("Finished reading files from os.walk")
    ...
    logger.debug("Finished creating file objects")
