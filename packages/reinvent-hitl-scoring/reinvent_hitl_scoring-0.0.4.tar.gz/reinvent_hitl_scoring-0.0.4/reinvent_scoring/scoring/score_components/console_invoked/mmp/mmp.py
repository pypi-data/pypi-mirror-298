...
class MMP(BaseConsoleInvokedComponent):
    ...
    def _calculate_score(self, molecules: List, step) -> np.array:
        if not os.path.exists(self._mmp_parameters.mmp_temporary_files_path):
            os.makedirs(self._mmp_parameters.mmp_temporary_files_path)

        input_smi_path = os.path.join(self._mmp_parameters.mmp_temporary_files_path, "mmp_input.smi")
        output_fragment_path = os.path.join(self._mmp_parameters.mmp_temporary_files_path, 'mmp_output.fragments')
        output_index_path = os.path.join(self._mmp_parameters.mmp_temporary_files_path, 'mmp_indexed.csv')

        smiles = self._chemistry.mols_to_smiles(molecules)
        ...
        if not self._mmp_parameters.mmp_debug:
            self._clean_up_temporary_folder(self._mmp_parameters.mmp_temporary_files_path)
