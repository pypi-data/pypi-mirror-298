...
from reinvent_chemistry.parallel import ParallelMoleculeHandler
...
def _update_raw_score(summary: ComponentSummary, query_length: int, valid_indices: List[int], scikit = scikit) -> ComponentSummary:
    if summary.raw_score is not None:
        raw_score = np.full(query_length, 0, dtype=np.float32)
        prob0 = np.full(query_length, 0, dtype=np.float32)
        prob1 = np.full(query_length, 0, dtype=np.float32)
        ...
        if not scikit:
            for idx, value in zip(valid_indices, summary.prob_dists[:,0]):
                prob0[idx] = value
            for idx, value in zip(valid_indices, summary.prob_dists[:,1]):
                prob1[idx] = value        
            summary.dist_bioactivity_0 = prob0
            summary.dist_bioactivity_1 = prob1
    return summary
...
class BaseScoringFunction(ABC):
    ...
    def _parallel_final_score(self, smiles: List[str]) -> FinalSummary:
        ...
        parallel_handler = ParallelMoleculeHandler()
        parallel_handler.prepare_local_parallel()
