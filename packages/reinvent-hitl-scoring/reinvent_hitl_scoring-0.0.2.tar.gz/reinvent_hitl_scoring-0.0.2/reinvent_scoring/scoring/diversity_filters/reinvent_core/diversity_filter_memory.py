...
class DiversityFilterMemory:
    ...
    def _include_raw_score(self, indx: int, component_scores: dict, components: List[ComponentSummary]):
        ...
        if stan:
            dist_bioactivity_0 = {f'dist_{c.parameters.name}_0': float(c.dist_bioactivity_0[indx]) for c in components if
                        c.dist_bioactivity_0 is not None}
            dist_bioactivity_1 = {f'dist_{c.parameters.name}_1': float(c.dist_bioactivity_1[indx]) for c in components if
                        c.dist_bioactivity_1 is not None}
            all_scores = {**component_scores, **raw_scores, **dist_bioactivity_0, **dist_bioactivity_1}
        else:
