...
class EnsembleHumanComponent(BaseScoreComponent):
    def __init__(self, parameters: ComponentParameters):
        super().__init__(parameters)
        self._activity_model = self._load_container(parameters)
    ...
    def _predict_and_transform(self, molecules: List):
        score = self._activity_model.predict(molecules, self.parameters.specific_parameters)
    ...
    def _load_container(self, parameters: ComponentParameters):
        ...
        for path in model_path:
            model.append(pickle.load(open(path, "rb")))
