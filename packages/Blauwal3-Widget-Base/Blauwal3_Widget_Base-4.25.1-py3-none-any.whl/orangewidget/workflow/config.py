...
def __(key):
    return i18n.t("widget.config." + key)
...
class Config(config.Default):
    ...
    ApplicationName = __("application_name")
    try:
...
def cache_dir():
    ...
    """
    warnings.warn(
        __("warn_cache_dir_deprecated").format(__name__),
...
def log_dir():
    ...
    """
    warnings.warn(
        __("warn_log_dir_deprecated").format(__name__),
        DeprecationWarning, stacklevel=2
    )
...
def widget_settings_dir(versioned=True):
    ...
    """
    warnings.warn(
        __("warn_setting_dir_deprecated").format(__name__),
        DeprecationWarning, stacklevel=2
    )
