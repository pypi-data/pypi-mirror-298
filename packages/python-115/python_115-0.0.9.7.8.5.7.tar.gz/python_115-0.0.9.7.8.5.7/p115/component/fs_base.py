...
class P115PathBase(Generic[P115FSType], Mapping, PathLike[str]):
    ...
    def __and__(self, path: str | PathLike[str], /) -> Self:
        attr = self.fs.attr(commonpath((str(self.path), self.fs.abspath(path))))
    ...
    def media_type(self, /) -> None | str:
        if not self.is_file():
            return None
        return guess_type(str(self.path))[0] or "application/octet-stream"
    ...
    def parts(self, /) -> tuple[str, ...]:
        return ("/", *splits(str(self.path), do_unescape=False)[0][1:])

    @cached_property
    def patht(self, /) -> tuple[str, ...]:
        return tuple(splits(str(self.path))[0])
    ...
    def samefile(self, path: str | PathLike[str], /) -> bool:
        if type(self) is type(path):
            return self == path
        return path in ("", ".") or str(self.path) == self.fs.abspath(str(path))
    ...
    @cached_property
    def stem(self, /) -> str:
        return splitext(basename(str(self.path)))[0]

    @cached_property
    def suffix(self, /) -> str:
        return splitext(basename(str(self.path)))[1]

    @cached_property
    def suffixes(self, /) -> tuple[str, ...]:
        return tuple("." + part for part in basename(str(self.path)).split(".")[1:])
