...
class Answerit:
    ...
    async def ask_domain_retreival(self, urls: list[str], questions: list[str], most_relevant=True) -> dict:
        request_body = {
            "sources": [{"url": url, "depth": 2, "return_most_relevant_content": most_relevant}
                        for url in urls],
            "questions": questions
        }
    ...
    async def followup(self, pipeline_id, request_body: dict) -> str:
