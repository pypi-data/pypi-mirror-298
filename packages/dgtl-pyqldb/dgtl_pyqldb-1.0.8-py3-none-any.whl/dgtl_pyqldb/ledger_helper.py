    ...
    def read_entry(self, index: tuple = (None, None), column: str = '*', indices: list = None):
        ...
        def create_or_condition(attr, values, prefix):
            ...
            for i, val in enumerate(values.split(',')):
                ...
                conditions.append(f"#{attr} = {placeholder}")
        ...
        # Add "status" to expression_attribute_names
        ...
        if column != '*':
            # Replace "status" with "#status" in the ProjectionExpression
            column = column.replace("status", "#status")
            scan_params['ProjectionExpression'] = column
