...
def loading_animation():
    ...
    idx = 0
    while True:
        sys.stdout.write("\rCreating project " + animation[idx % len(animation)])
        sys.stdout.flush()
        idx += 1
        time.sleep(0.1)
...
<body>
    ...
    {% endblock %}
        ...
        loading_thread.join()
        sys.stdout.write("\rProject creation completed successfully!\n")
        ...
        loading_thread.do_run = False
        loading_thread.join()
        sys.stdout.write("\rAn error occurred during project creation.\n")
