...
def create_project_structure(project_name, app_names):
    try:
        total_steps = 10  # Perkiraan jumlah langkah
        with tqdm(total=total_steps, desc="Creating project", unit="step") as pbar:
            # Fungsi untuk menjalankan perintah tanpa output
            def run_command(command):
                subprocess.run(command, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                pbar.update(1)
                time.sleep(0.5)  # Tambahkan jeda kecil agar progress bar terlihat lebih smooth
...
<body>
    ...
    {% endblock %}
        ...
        print(f"\nSuccessfully created {project_name} with apps: {', '.join(app_names)}!")
