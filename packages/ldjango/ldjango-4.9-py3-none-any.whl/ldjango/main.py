...
<body>
    ...
    {% endblock %}
            ...
            urls_content += "urlpatterns += [path('', include('apps.urls'))]\n"
...
''')
        ...
        click.echo(click.style("\n1. Unleash the power of Tailwind CSS:", fg="yellow", bold=True))
        click.echo(click.style(f"   cd {project_name} && npx tailwindcss -i ./static/css/input.css -o ./static/css/output.css --watch", fg="magenta", italic=True))
        click.echo(click.style("   (Remember to close this terminal when you're done with the Tailwind magic!)", fg="yellow"))
        
        click.echo(click.style("\n2. Gather your static assets:", fg="yellow", bold=True))
        click.echo(click.style(f"   cd {project_name} && python manage.py collectstatic", fg="magenta", italic=True))
        
        click.echo(click.style("\n3. Launch your development server:", fg="yellow", bold=True))
        click.echo(click.style(f"   cd {project_name} && python manage.py runserver", fg="magenta", italic=True))
