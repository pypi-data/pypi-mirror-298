...
def makeproject():
    ...
    project_name = click.prompt(f"{Fore.CYAN}{Style.BRIGHT}? {Fore.MAGENTA}Enter your project name{Style.RESET_ALL}", default="MyProject", prompt_suffix=": ", show_default=True)
    
    app_count = click.prompt(
        f"{Fore.CYAN}{Style.BRIGHT}? {Fore.MAGENTA}How many applications do you want to create?{Style.RESET_ALL}",
    ...
    for i in range(app_count):
        app_name = click.prompt(
            f"{Fore.CYAN}{Style.BRIGHT}? {Fore.MAGENTA}Django Application Name {i + 1}{Style.RESET_ALL}",
            default=f"MyApp{i + 1}",
            prompt_suffix=": ",
            show_default=True,
        )
