    ...
    if os.path.exists(settings_path):
        ...
        if apps_index != -1:
            ...
            if end_index != -1:
                ...
                # Menulis kembali file settings.py
                with open(settings_path, 'w') as settings_file:
                    settings_file.write(updated_content)
            else:
                print("Warning: Format INSTALLED_APPS is not as expected")
        else:
            print("Warning: INSTALLED_APPS not found in settings.py")
    else:
        print(f"Warning: settings.py file not found in {settings_path}")
