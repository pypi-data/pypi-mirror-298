...
class CodebaseEmbeddings:
    ...
    async def build_embeddings(self, codebase_path: str, batch_size: int = 10) -> None:
        ...
        async def process_file(file_path: str):
            ...
            try:
                ...
                console.print(f"Error extracting snippets from {file_path}: {e}")
                return None
            ...
            try:
                ...
                console.print(f"Error generating embeddings for {file_path}: {e}")
                return None
            ...
            try:
                ...
                console.print(f"Error assigning embeddings to snippets for {file_path}: {e}")
                return None
                ...
                while retry_count < 3:
                    try:
                        ...
                        if e.status == 429:
                            ...
                            console.print(f"[red]Rate limit hit. Retrying after {wait_time} seconds...[/red]")
        ...
        try:
            with Progress(
                ...
                task_id = progress.add_task("[cyan]Processing files...", total=total_files)
            ...
            if not stop_event.is_set():
                self._save_cache()
                console.print("[green]Embedding build complete![/green]")
            else:
                console.print("[yellow]Embedding build interrupted. Partial results saved.[/yellow]")
    ...
    async def get_relevant_snippets(self, query: str, top_k: int = 5, snippet_type: str = "function") -> List[CodeSnippet]:
        ...
        if len(valid_snippets) == 0:
            console.print(f"[yellow]No relevant {snippet_type} snippets found for the given query.[/yellow]")
            return []
