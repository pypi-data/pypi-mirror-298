...
class ImageCreate(ImageCommand):
    ...
    def __init__(self, conf):
        ...
        # Just to allow tracing, source is not use in qemu.create.
...
class Qemu(vms.VM):
    ...
    def ips_from_ip_neigh(self):
        ...
        This relies on `ip neigh show dev {qemu.bridge}`.
        """
        known = []
        mac_addr = self.conf.get('qemu.mac.address')
        bridge = self.conf.get('qemu.bridge')
        ipn_cmd = ['ip', 'neigh', 'show', 'dev', bridge]
    ...
    def capture_ip(self):
        ...
        """
        mac_addr = self.conf.get('qemu.mac.address')
        ping_cmd = 'ping -4 -c1 {}'.format(self.conf.get('vm.name'))
