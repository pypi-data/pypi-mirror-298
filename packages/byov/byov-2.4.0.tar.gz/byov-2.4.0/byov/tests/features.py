...
class PipFeature(features.ExecutableFeature):
...
class LxcClient(features.ExecutableFeature):
...
geniso_feature = features.ExecutableFeature('genisoimage')
git_feature = features.ExecutableFeature('git')
lxd_client_feature = LxcClient()
...
qemu_feature = features.ExecutableFeature('qemu-system-x86_64')
