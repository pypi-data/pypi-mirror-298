...
register(options.Option('vm.name', default=name_from_conf, invalid='error',
                        help_string='''\
...
If options expansion is needed, '@script' ill expand the options inside
...
def default_byov_scripts(conf=None):
    from importlib import resources
    return resources.files(byov).joinpath('scripts').as_posix()


register(options.Option('byov.scripts',
...
'''))
register(options.ListOption('qemu.uefi.vars', separator=' ',
                            default='''\
cp {source} {target}''',
                            help_string='''\
...
register(options.PathOption('qemu.uefi.vars.seed',
                            default='/usr/share/OVMF/OVMF_VARS_4M.fd',
                            help_string='''\
The default values for UEFI variables.
Provided by the ovmf package.
'''))
register(options.PathOption('qemu.uefi.vars.path',
...
'''))
register(options.PathOption('qemu.uefi.code.path',
                            default='/usr/share/OVMF/OVMF_CODE_4M.fd',
                            help_string='''\
The default UEFI code.
Provided by the ovmf package.
'''))
register(options.ListOption('qemu.disks.uefi.code', separator=' ',
                            default='''\
-drive if=pflash,format=raw,readonly,file={qemu.uefi.code.path}''',
                            help_string='''\
The drive qemu option containing the uefi code.
'''))
register(options.ListOption('qemu.disks.uefi.vars', separator=' ',
                            default='''\
...
register(options.ListOption('qemu.disks.uefi', separator=' ',
                            default='''\
...
# FIXME: qemu.setup.digest.options is missing -- vila 2024-09-07
...
'byovm setup' command).
'''))
register(options.PathOption('lxd.idmap.path',
                            help_string='''\
...
register(options.Option('lxd.privileged', default=False, invalid='error',
                        from_unicode=options.bool_from_store,
                        help_string='''\
...
The values for these options are hashed to produce 'vm.setup.digest'.
'''))
register(options.ListOption('ephemeral-lxd.setup.digest.options',
                            default='vm.backing,'
                            'lxd.user_mounts,'
                            'lxd.user_mounts.host.uid,'
                            'lxd.user_mounts.host.gid,'
...
register(options.ListOption('docker.setup.digest.options',
                            default='docker.file, docker.file.expanded,'
...
register(options.Option('debian.release.stable',
                        default='bookworm',
                        help_string='''The stable release for debian.'''))
register(options.Option('debian.docker.image',
...
register(options.Option('ubuntu.release.stable',
                        default='noble',
...
def default_ubuntu_qemu_download_url(conf=None):
    # FIXME: This is incomplete as it should at least handle stable, testing
...
register(options.Option('ubuntu.qemu.download.url',
