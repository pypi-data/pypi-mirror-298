...
class ApiAcademicoService:
    ...
    def get(self, query: str) -> List[Usuario]:
        ...
        response = requests.post(
            url=self._base_url, headers=self.__DEFAULT_HEADERS, json={'query': _query, 'operationName': 'buscarAlunos', 'variables': {}})
        ...
        if content['data'] and content['data']['getDadosAlunos'] and content['data']['getDadosAlunos']['items']:
        ...
        if len(content['data']['getDadosAlunos']['items']) == 0:
            logger.warning(
                msg='Não foram encontrados resultados para alunos')
            return []
