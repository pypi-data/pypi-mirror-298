...
@dataclass
class DataKeycloakAttributes:
    employeeType: List[str] = field(default_factory=list, compare=False)
    LDAP_ENTRY_DN: List[str] = field(default_factory=list, compare=False)
    eduPersonAffiliation: List[str] = field(
        default_factory=list, compare=False)
    ou: List[str] = field(default_factory=list, compare=False)
    departmentNumber: List[str] = field(default_factory=list, compare=False)
    employeeNumber: List[str] = field(default_factory=list, compare=False)
    LDAP_ID: List[str] = field(default_factory=list, compare=False)
...
class DataKeycloakUser:
    ...
    attributes: Optional[DataKeycloakAttributes] = None
    disableableCredentialTypes: List[str] = field(
        default_factory=list, compare=False)
    requiredActions: List[str] = field(default_factory=list, compare=False)
    notBefore: Optional[int] = None
    access: Optional[DataKeycloakAccess] = None
...
class DataGroup:
    ...
    path: str
    attributes: Optional[DataKeycloakAttributes] = None
    realmRoles: Optional[list] = field(default_factory=list, compare=False)
    clientRoles: Optional[dict] = field(default_factory=dict)
    subGroups: list["DataGroup"] = field(default_factory=list, compare=False)
...
class KeycloakService():
    ...
    def get_groups(self) -> List[DataGroup]:
        response = requests.get(url=self.GROUP_URL, headers=self.__headers)
        if response.status_code == 200:
            return [DataGroup(**d) for d in response.json()]
        return []

    def get_groups_by_path(self, path: str) -> DataGroup:
        response = requests.get(url=self.GROUP_BY_PATH_URL+path, headers=self.__headers)
        if response.status_code == 200:
            return DataGroup(**response.json())
        return []

    def get_group(self, id: str) -> Optional[DataGroup]:
    ...
    def get_group_by_path(self, path: str) -> Optional[DataGroup]:
    ...
    def get_members_by_group(self, id: str) -> List[DataKeycloakUser]:
        ...
        response = requests.get(url=url, headers=self.__headers)
        if response.status_code == 200:
            return [DataKeycloakUser(**u) for u in response.json()]
        return []
