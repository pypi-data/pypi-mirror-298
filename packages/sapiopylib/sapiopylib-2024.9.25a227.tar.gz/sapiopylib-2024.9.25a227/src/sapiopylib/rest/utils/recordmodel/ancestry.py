...
class RecordModelAncestorManager(RecordModelManagerBase):
    ...
    def load_ancestors_of_type(self, models_to_load: Iterable[PyRecordModel], ancestor_data_type: str) -> None:
        """
        ...
        for model in models_to_load:
            if self.is_ancestors_loaded(model, ancestor_data_type):
                continue
            actual_load_set.put(model.data_type_name, model)
        ...
        for data_type_name in actual_load_set.keys():
            ...
            for source_record_id, ancestor_records in result_map.store.items():
                ...
                ancestor_model_list: List[PyRecordModel] = inst_man.add_existing_records(ancestor_records)
    ...
    def is_ancestors_loaded(self, model: PyRecordModel, ancestor_data_type: str):
        """
    ...
    def get_ancestors_of_type(self, model: PyRecordModel, ancestor_data_type: str) -> Set[PyRecordModel]:
        """
    ...
    def is_descendant_loaded(self, model: PyRecordModel, descendant_data_type: str):
        """
    ...
    def load_descendant_of_type(self, models_to_load: Iterable[PyRecordModel], descendant_data_type: str) -> None:
        """
        ...
        for source_record_id, desc_records in result_map.store.items():
            source_model: PyRecordModel = inst_man.get_known_record_with_record_id(source_record_id)
            desc_model_list: List[PyRecordModel] = inst_man.add_existing_records(desc_records)
    ...
    def get_descendant_of_type(self, model: PyRecordModel, descendant_data_type: str) -> Set[PyRecordModel]:
        """
