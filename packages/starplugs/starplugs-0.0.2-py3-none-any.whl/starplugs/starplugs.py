...
#  pip install pandas scikit-learn openpyxl matplotlib
  ...
  print()
  print("subpackages: arule01, arule, cluster, dtree, linear, sequence, test, liquidity, profitability, solvency")
  # print()
  # print()
  print("**********************************************************")
...
#-----------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------
# functions: identify sequential patterns
#***********************************************************************************************************
def sequence():
  import pandas as pd
  ...
  # Step 4: Get user input for min_support and min_threshold
  min_support = float(input("Enter the minimum support (e.g., 0.01 for 1%): "))
  min_threshold = float(input("Enter the minimum threshold for lift (e.g., 1): "))

  # Step 5: Preprocess the data
  # Assuming the data contains columns 'Date' and 'Symbol'
  transactions = df.groupby('Date')['Symbol'].apply(list).tolist()

  # Step 6: Apply Transaction Encoder
  te = TransactionEncoder()
  te_ary = te.fit(transactions).transform(transactions)
  df_encoded = pd.DataFrame(te_ary, columns=te.columns_)

  # Step 7: Apply Apriori algorithm to find frequent itemsets
  ...
  print("Sequential pattern derived from the itemsets:")
  print(rules)

  # Save the results to Excel if needed
  rules.to_excel("sequential_pattern_output.xlsx", index=False)
  print("Results saved to 'sequential_pattern_output.xlsx'")
