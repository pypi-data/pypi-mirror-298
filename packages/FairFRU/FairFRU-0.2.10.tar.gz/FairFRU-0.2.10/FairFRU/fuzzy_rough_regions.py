...
def compute_membership_values(df, case_name, path, columns, alpha = 0.5, complete = False, hide_progress = False):
  '''
  ...
  if not complete:
    ...
    Writearray(df.iloc[:,:-1], alpha, 'full').sim_array(h5file = h5file, group = group, hide_progress = hide_progress)
  ...
  for s_attr in columns:
    ...
    Writearray(dataset, alpha, s_attr).sim_array(h5file = h5file, group = group, hide_progress = hide_progress)
