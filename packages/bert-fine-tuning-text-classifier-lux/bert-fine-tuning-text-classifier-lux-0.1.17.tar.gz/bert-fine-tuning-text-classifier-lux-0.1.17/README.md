...
# Extract columns from the training data
train_columns = train_data.columns.tolist()

# Step 4: User selects the label column, text columns, and numeric columns
...
# Step 5: Process the selected columns for training (No DataFrame passed)
...
# Process the selected columns for validation (No DataFrame passed)
...
# Process the selected columns for the test dataset (No DataFrame passed)
