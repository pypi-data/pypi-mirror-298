...
def fetch_timing_data(
    ...
    if source_type in scan_types or target_type in scan_types:
        ...
        first_roi_delay_im = (
            pipe.ScanInfo.Field & scan_restriction & {"z": field_z}
        ).fetch("delay_image", order_by="field ASC", limit=1)[0]
        field_delay_im = (pipe.ScanInfo.Field & scan_key).fetch1("delay_image")
        average_field_delay = np.mean(field_delay_im) - np.min(
            first_roi_delay_im
        )  # Field delay counts from first field at first depth, so we need to subtract the minimum delay of the first field at that depth
        ...
        if "unit_id" in scan_key or "mask_id" in scan_key:
            if len(pipe.ScanSet.Unit & scan_key) > 0:
                ...
                ms_delay = (
                    (pipe.ScanSet.UnitInfo & unit_key).fetch1("ms_delay") / 1000
                ) - np.min(
                    first_roi_delay_im
                )  # Delay is stored in milliseconds here, but timings is in seconds
                if debug:
                    print(
