...
def main():
    ...
    if not opts.data[
        ...
        raise NotImplementedError(
            "Data must be provided via CLI or json_params at this time."
        )
    ...
    moviehandle.play(
        ...
        magnification=2,
        save_movie=True,
        movie_name="/home/rbo/caiman_data/test.avi",
    )
    ...
    if not cfg.no_play:
        ...
        ds_ratio = 0.2
        moviehandle = caiman.concatenate(
            [
    ...
    if not cfg.no_play:
        ...
        cnm2.estimates.view_components(
            images, img=Cn, idx=cnm2.estimates.idx_components
        )
        cnm2.estimates.view_components(
            images, img=Cn, idx=cnm2.estimates.idx_components_bad
        )
...
def handle_args():
    ...
    parser.add_argument(
        "--keep_logs", action="store_true", help="Keep temporary logfiles"
    )
