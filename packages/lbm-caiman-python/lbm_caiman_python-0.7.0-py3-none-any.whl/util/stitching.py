...
class StitchedSlice:
    def __init__(self, slice_, x, y, dtype):
        """
        Represents a single slice from a stitched region of interest (ROI).

        This class provides functionalities to manipulate and stitch image slices,
        including rotation and merging with other slices, while maintaining the appropriate
        metadata such as position and dimensions.

        Attributes
        ----------
        slice : np.array
            The image data of the slice as a numpy array.
        x : float
        ...
        mask : np.array
            A mask used for operations like blending, initially set to ones with the same shape as the slice.

        Parameters
        ----------
        slice_ : np.array
            The image data of the slice.
        x : float
            The x-coordinate of the center of the slice.
        y : float
            The y-coordinate of the center of the slice.
        dtype : data-type
            The data type to be used for the slice.

        Methods
        -------
        rot90()
            Rotates the slice 90 degrees clockwise.
        join_with(other, x, y, smooth_blend=True)
            Stitches another slice to this slice at the given coordinates.
        """
    ...
    def join_with(self, other, x, y, smooth_blend=True):
        ...
        output_height, output_width = int(round(y_max - y_min)), int(
            round(x_max - x_min)
        )
...
class StitchedROI:
    ...
    def volume(self):
        ...
        for i, slice_ in enumerate(self.slices):
            ...
            delta = (
                (slice_.y - slice_.height / 2) - y_min,
    ...
    def left_or_right(self, other, rel_tol=0.1, minimum_overlap=25):
        ...
        same_depth = (
            self.depth == other.depth and abs(self.z - other.z) < rel_tol * self.depth
        )
        same_height = (
            abs(self.height - other.height) < rel_tol * self.height
            and abs(self.y - other.y) < rel_tol * self.height
        )
        overlap = (
            max(self.x + self.width / 2, other.x + other.width / 2)
    ...
    def join_with(self, other, xs, ys, smooth_blend=True):
        ...
        for roi_coord in other.roi_coordinates:
            ...
            roi_coord.xs = [
                prev_x + (new_x - old_slice.x)
                for prev_x, new_x, old_slice in zip(roi_coord.xs, xs, other.slices)
            ]
            roi_coord.ys = [
                prev_y + (new_y - old_slice.y)
                for prev_y, new_y, old_slice in zip(roi_coord.ys, ys, other.slices)
            ]
...
def linear_stitch(left, right, expected_delta_x, minimum_overlap=10):
    ...
    # Compute best match
    y_shifts, x_shifts = compute_motion_shifts(right_strip, left_strip, in_place=False)
