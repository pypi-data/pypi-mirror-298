...
def extract_masks(
    ...
    """
    Extract spatial components (neuronal masks) and their corresponding fluorescence
    traces from multi-photon microscopy scans using Constrained Non-negative Matrix
    Factorization (CNMF).

    The function operates either on full images or on patches of the images, allowing for
    scalable processing of large datasets. It includes multiple stages of processing to
    optimize the extraction of neuronal signals and background activity.

    Parameters
    ----------
    scan : np.array
        A 3D numpy array representing the imaging data with dimensions
        (image_height, image_width, num_frames).
    mmap_scan : np.memmap
        Memory-mapped numpy array with dimensions (image_height * image_width, num_frames).
        Used to efficiently handle large datasets that do not fit in memory.
    num_components : int, optional
        Estimated number of spatial components (e.g., neurons) to extract.
    num_background_components : int, optional
        Number of components used to model the background fluorescence.
    merge_threshold : float, optional
        Threshold for merging spatially overlapping components based on their
        temporal correlation. Higher values are more permissive.
    init_on_patches : bool, optional
        If True, initializes components on small patches of the full image, which
        is beneficial for large datasets.
    init_method : str, optional
        Method used for initializing components. Options include:
        'greedy_roi' - Greedy region of interest selection followed by rank-1 NMF.
        'sparse_nmf' - Sparse NMF implemented as in sklearn, using l1 regularization.
    soma_diameter : tuple of float, optional
        Estimated diameter of neurons in pixels, specified as (y_size, x_size).
    snmf_alpha : float, optional
        Regularization parameter for sparse NMF, controlling sparsity of the solution.
    patch_size : tuple of float, optional
        Dimensions of the patches used for initial component estimation in pixels,
        specified as (y_size, x_size).
    proportion_patch_overlap : float, optional
        Proportion of overlap between adjacent patches, ranging from 0 (no overlap) to
        close to 1 (high overlap).
    num_components_per_patch : int, optional
        Number of components to find per patch, used when `init_on_patches` is True.
    num_processes : int, optional
        Number of processes to run in parallel. Using more processes can speed up
        computations but requires more memory.
    num_pixels_per_process : int, optional
        Number of pixels handled by each process during computation.
    fps : int, optional
        Frames per second of the scan, used for temporal calculations and adjustments.

    Returns
    -------
    tuple
        A tuple containing the following elements:
        - Weighted masks (np.array): Array of shape (image_height, image_width, num_components).
        - Denoised fluorescence traces (np.array): Array of shape (num_components, num_frames).
        - Background masks (np.array): Array of shape (image_height, image_width, num_background_components).
        - Background traces (np.array): Array of shape (num_background_components, num_frames).
        - Raw fluorescence traces (np.array): Array of shape (num_components, num_frames), representing
          the raw fluorescence from each component, adjusted for overlapping signals and background.

    Raises
    ------
    PipelineException
        If inputs are invalid, if parameters are out of expected range, or if computation fails due to data or
        memory constraints.

    Notes
    -----
    - The actual number of extracted components may differ from `num_components` due to merging and component rejection.
    - Best performance and results are typically achieved with scans that have been preprocessed to ensure non-negativity
      and noise reduction.
    """
    ...
    if init_on_patches:
        ...
        options = {
            ...
            "init_params": {
                ...
                "min_corr": "UNUSED",
                "min_pnr": "UNUSED",
                "deconvolve_options_init": "UNUSED",
                "ring_size_factor": "UNUSED",
                "center_psf": "UNUSED",
            },
...
def deconvolve(trace, AR_order=2):
    """
    Deconvolve fluorescence traces using noise-constrained deconvolution.

    This function applies a noise-constrained sparse non-negative deconvolution algorithm
    to infer neural activity (spike train) from fluorescence traces, as per the method
    described by Pnevmatikakis et al. (2016).

    Parameters
    ----------
    trace : np.array
       A 1-dimensional array representing the fluorescence trace over time (num_frames).
    AR_order : int, optional
       The order of the autoregressive process used to model the calcium impulse response function.
       E.g., 0 implies no modeling, 2 models rise and exponential decay.

    Returns
    -------
    np.array
       The deconvolved spike trace, indicating inferred neural activity.
    np.array
       The autoregressive coefficients used to model the calcium response dynamics.

    Example
    -------
    >>> trace = np.random.rand(1000)
    >>> spike_trace, AR_coeffs = deconvolve(trace, AR_order=2)
...
def deconvolve_detrended(trace, scan_fps, detrend_period=600, AR_order=2):
    """
    Detrend and deconvolve fluorescence traces to infer neural activity.

    The function first detrends the fluorescence trace using a moving percentile method
    before applying noise-constrained sparse non-negative deconvolution.

    Parameters
    ----------
    trace : np.array
        A 1-dimensional array of the fluorescence trace over time (num_frames).
    scan_fps : float
        The frame rate of the scan, used to calculate detrending window.
    detrend_period : float, optional
        The period over which to compute percentiles for detrending, in seconds.
    AR_order : int, optional
        The order of the autoregressive process used to model the calcium impulse response function.
    ...
    -------
    np.array
        The deconvolved spike trace after detrending.
    np.array
        The autoregressive coefficients used to model the calcium response dynamics.

    Example
    -------
    >>> trace = np.random.rand(3000)
    >>> scan_fps = 10
    >>> spike_trace, AR_coeffs = deconvolve_detrended(trace, scan_fps, detrend_period=60, AR_order=2)
    """
...
def get_centroids(masks):
    """
    Calculate the centroids of each mask for imaging data analysis.

    This function computes the center of mass (centroid) for each spatial component (mask) in a set
    of masks typically representing neural or cellular structures in imaging data.

    Parameters
    ----------
    masks : np.array
        A 3-dimensional array where each slice along the third dimension represents a mask
        (image_height x image_width x num_components).

    Returns
    -------
    np.array
        An array containing the centroids of each component, with each row corresponding to
        the centroid coordinates (y, x) of a component.

    Example
    -------
    >>> masks = np.random.rand(100, 100, 10) > 0.95  # Generate random binary masks
    >>> centroids = get_centroids(masks)
    """
    # Reshape masks
...
def classify_masks(masks, soma_diameter=(12, 12)):
    """
    Classify each mask in a given set of masks as soma or non-soma using a pretrained convolutional neural network.

    This function takes a stack of binary masks representing potential cellular structures in multi-photon
    microscopy data and applies a CNN to predict the likelihood of each mask being a soma. The function assumes
    that a trained model exists and is accessible.

    Parameters
    ----------
    masks : np.array
        A 3-dimensional numpy array where each slice along the third dimension represents a binary mask
        (image_height x image_width x num_components). Each mask should be a 2D binary array representing
        the spatial footprint of a potential cellular structure.
    soma_diameter : tuple of int, optional
        A tuple representing the expected diameter of somas in pixels (height, width). This is used to scale
        the masks appropriately for the CNN model.

    Returns
    -------
    np.array
        A 1D array of size num_components containing the probability that each mask corresponds to a soma.
        Each element in the array is a float between 0 and 1, where higher values indicate higher likelihood
        of the mask being a soma.

    Example
    -------
    >>> masks = np.random.randint(0, 2, (100, 100, 10), dtype=bool)  # Example random binary masks
    >>> soma_probabilities = classify_masks(masks)
    >>> print(soma_probabilities)
    ...
    - Ensure that the masks are properly preprocessed (e.g., noise reduction, thresholding) before using this
      function to achieve optimal classification results.
    """
    ...
    probs, _ = components_evaluation.evaluate_components_CNN(
        masks, (image_height, image_width), soma_radius, model_name=model_path
    )
...
def demix_and_deconvolve_with_cnmf(
    scan,
    num_components=200,
    AR_order=2,
    merge_threshold=0.8,
    ...
    num_pixels_per_process=5000,
    block_size=10000,
    num_background_components=4,
    init_method="greedy_roi",
    soma_radius=(5, 5),
    snmf_alpha=None,
    init_on_patches=False,
    ...
    # 'Load' scan
    mmap_scan, (image_height, image_width), num_frames = caiman.load_memmap(
        mmap_filename
    )
    ...
    if init_on_patches:
        ...
        patch_size = min(
            patch_size, smaller_dimension
        )  # if bigger than small dimension

        # Calculate num_components_per_patch
        num_nonoverlapping_patches = (image_height / patch_size) * (
        ...
        model = cnmf.CNMF(
            ...
            p=0,
            rf=int(round(patch_size / 2)),
            stride=overlap_in_pixels,
            k=num_components_per_patch,
    ...
    background_location_matrix = background_location_matrix.reshape(
        new_shape, order="F"
    )
    AR_coefficients = np.array(
        list(AR_coefficients)
    )  # unwrapping it (num_components x 2)
