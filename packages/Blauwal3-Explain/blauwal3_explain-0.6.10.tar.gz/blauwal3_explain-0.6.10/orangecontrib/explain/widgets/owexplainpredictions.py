...
def __(key):
    return i18n.t('explain.owexplainpredictions.' + key)
...
class OWExplainPredictions(OWWidget, ConcurrentWidgetMixin):
    ...
    def _add_controls(self):
        ...
        box = gui.vBox(self.controlArea, __("instance_order"))
        ...
        box = gui.vBox(self.controlArea, __("annotation"))
    ...
    def setup_plot(self):
        ...
        if self.order_index == 0:
            order = __("hierarchical_clustering")
        elif self.order_index == 1:
            order = __("output_value")
        elif self.order_index == 2:
            order = __("original_ordering")
        x_label = __(f"instances_ordered_by", order)
