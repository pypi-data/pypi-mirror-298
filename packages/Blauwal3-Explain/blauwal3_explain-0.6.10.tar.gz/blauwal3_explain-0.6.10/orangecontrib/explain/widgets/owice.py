...
def __(key):
    return i18n.t('explain.owice.' + key)
...
class OWICE(OWWidget, ConcurrentWidgetMixin):
    ...
    class Error(OWWidget.Error):
        ...
        unknown_err = Msg("{}")
        not_enough_data = Msg(__("not_enough_data"))
        no_cont_features = Msg(__("no_cont_features"))
    ...
    def _add_controls(self):
        ...
        gui.checkBox(box, self, "order_by_importance", __("order_by_importance"),
                     callback=self.__on_order_changed)

        box = gui.vBox(self.controlArea, __("display"))
        self._color_model = DomainModel(placeholder="None", separators=False,
                                        valid_types=DiscreteVariable)
        gui.comboBox(box, self, "color_var", label=__("color"), searchable=True,
                     model=self._color_model, orientation=Qt.Horizontal,
                     contentsLength=12, callback=self.__on_parameter_changed)
        gui.checkBox(box, self, "centered", __("centered"),
                     callback=self.__on_parameter_changed)
        gui.checkBox(box, self, "show_mean", __("show_mean"),
