...
from Orange.widgets.utils.concurrents import TaskState
...
def __(key):
    return i18n.t('explain.owpermutationimportance.' + key)
...
class OWPermutationImportance(OWExplainFeatureBase):
    ...
    class Warning(OWExplainFeatureBase.Warning):
        missing_target = Msg(__("missing_target"))

    # GUI setup
    def _add_controls(self):
        box = gui.vBox(self.controlArea, __("parameters"))
        self._score_combo: QComboBox = gui.comboBox(
            box, self, "score_index", label=__("score"), items=[],
            orientation=Qt.Horizontal, contentsLength=12,
            callback=self.__parameter_changed
        )
        gui.spin(
            box, self, "n_repeats", 1, 1000, label=__("permutations"),
