...
def __(key):
    return i18n.t('explain.owexplainfeaturebase.' + key)
...
class OWExplainFeatureBase(OWWidget, ConcurrentWidgetMixin, openclass=True):
    ...
    class Error(OWWidget.Error):
        ...
        unknown_err = Msg(__("unknown_err"))

    class Information(OWWidget.Information):
        data_sampled = Msg(__("data_sampled"))
    ...
    def _add_controls(self):
        self.display_box = box = gui.vBox(self.controlArea, __("display"))
        gui.spin(box, self, "n_attributes", 1, MAX_N_ITEMS,
                 label=__("top_features"), controlWidth=50,
                 callback=self.__n_spin_changed)
        gui.hSlider(box, self, "zoom_level", minValue=0, maxValue=200,
                    width=150, label=__("zoom"), createLabel=False,
