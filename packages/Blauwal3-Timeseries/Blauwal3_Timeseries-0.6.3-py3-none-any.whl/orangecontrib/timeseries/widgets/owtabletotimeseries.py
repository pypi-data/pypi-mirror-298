...
# 用于界面显示的中文标签映射
label_mapping = {
    "Seconds": __("data.seconds"),
    "Minutes": __("data.minutes"),
    "Hours": __("data.hours"),
    "Days": __("data.days"),
    "Weeks": __("data.weeks"),
    "Months": __("data.months"),
    "Years": __("data.years"),
    "Centuries": __("data.centuries"),
}
...
class OWTableToTimeseries(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        invalid_time = widget.Msg(__("msg.invalid_time"))
        year_out_of_range = widget.Msg("{}.")

    class Warning(widget.OWWidget.Warning):
        nan_times = widget.Msg(__("msg.missing_values"))
    ...
    start_date = Setting((2024, 1, 1))
    start_time = Setting((0, 0, 0))
    attribute = Setting(None)
    autocommit = Setting(True)

    extra_part_labels = [__("box.include_time"),
                         __("box.include_date")]
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(
            group, __("btn.select_column"), insertInto=outbox)
        hbox = gui.indentedBox(outbox, indent, orientation=Qt.Horizontal)
        hbox.layout().addWidget(QLabel(__("btn.variable")))
        ...
        gui.appendRadioButton(
            group, __("btn.sequence_imply"), insertInto=outbox)
    ...
    def _add_start_controls(self, layout, row, numwidth):
        layout.addWidget(QLabel(__("btn.start")), row, 0)
        ...
        custom_month_names_full = [
            __("month.january"), __("month.february"), __("month.march"),
            __("month.april"), __("month.may"), __("month.june"),
            __("month.july"), __("month.august"), __("month.september"),
            __("month.october"), __("month.november"), __("month.december")
        ]
        combo.addItems(custom_month_names_full)
        combo.setCurrentIndex(m - 1)
        combo.setMinimumContentsLength(max(map(len,custom_month_names_full)))
    ...
    def _add_step_controls(self, layout, row, numwidth):
        layout.addWidget(QLabel(__("box.step")), row, 0)
        box = gui.hBox(None)
        ...
        gui.comboBox(
            box, self, "unit", items=[label_mapping[label] for label in StepOptions.keys()],
