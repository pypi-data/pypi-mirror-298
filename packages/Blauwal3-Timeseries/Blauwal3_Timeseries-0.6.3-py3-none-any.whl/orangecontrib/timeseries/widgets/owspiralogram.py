...
class SpiralogramContextHandler(DomainContextHandler):
    ...
    def match(self, context, domain, attrs, metas, is_time):
        if "x_var" in context.values and context.values["x_var"][1] == -2 and not is_time:
...
class OWSpiralogram(OWWidget):
    ...
    class Error(OWWidget.Error):
        no_useful_vars = Msg(__("msg_no_time_variable"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("time_period"))
        ...
        box = gui.vBox(self.controlArea, __("box.radial"))
        self.rad_model = DomainModel(
            placeholder=__("btn.none"), separators=False,
        ...
        gui.checkBox(
            ibox, self, "hide_r_labels", __("btn.hide_label"),
            callback=self.redraw)

        box = gui.vBox(self.controlArea, __("btn.color"))
        self.var_model = DomainModel(
            placeholder=__("btn.show_instance"), separators=False,
            valid_types=(DiscreteVariable, ContinuousVariable))
        gui.comboBox(
    ...
    def draw_segments(self):
        ...
        if self.x_var in (i18n.t("timeseries.adddesc.day_of_year"), i18n.t("timeseries.adddesc.day_of_year"),i18n.t("timeseries.adddesc.day_of_month")):
            lab_off = 1
            x_col -= 1
        else:
            lab_off = 0
