...
class OWDifference(widget.OWWidget):
    ...
    Operations = [
        OpDesc(__("gbox.difference"), "Δ"),
        OpDesc(__("gbox.difference2"), "ΔΔ"),
        OpDesc(__("gbox.quotient"), "q"),
        OpDesc(__("gbox.percentage_change"), "%"),
    ]
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("gbox.operation"))
        ...
        sp = gui.spin(
            box, self, 'shift_period', 1, 100, label=__("label.shift"),
        ...
        gui.checkBox(
            box, self, 'invert_direction', __("checkbox_invert_direction"),
            callback=self._operation_or_direction_changed)

        gui.checkBox(
            box, self, 'assume_zero_before',
            __("assume_zeros_before_start"), stateWhenDisabled=False,
            callback=self.commit.deferred)

        self._update_gui_state()
        gui.auto_commit(self.buttonsArea, self, 'autocommit', __("gbox.apply"))
