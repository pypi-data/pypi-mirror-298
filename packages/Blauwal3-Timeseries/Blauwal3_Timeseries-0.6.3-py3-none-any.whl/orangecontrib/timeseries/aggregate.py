...
def __(key):
    return i18n.t("timeseries.adddesc." + key)
...
AggDesc("mean", pmw(np.nanmean), np.nanmean, __("mean"),
        same_scale=True)
AggDesc("sum", moving_sum, np.nansum, __("sum"))
AggDesc('product', pmw(np.nanprod), np.nanprod, __("product"))
AggDesc('min', pmw(np.nanmin), np.nanmin, __("min"),
        same_scale=True)
AggDesc('max', pmw(np.nanmax), np.nanmax, __("max"),
        same_scale=True)
AggDesc('span', windowed_span,
        lambda x: np.nanmax(x) - np.nanmin(x), __("span"))
AggDesc('median', pmw(np.nanmedian), np.nanmedian,__("median"),
        same_scale=True)
AggDesc('mode', windowed_mode, block_mode, __("mode"),
        supports_discrete=True, same_scale=True)
AggDesc('std', pmw(np.nanstd), np.nanstd, __("std"), same_scale=True)
AggDesc('var', pmw(np.nanvar), np.nanvar, __("var"))
AggDesc('lin. MA', windowed_linear_MA, None, __("lin_ma"), same_scale=True)
AggDesc('exp. MA', windowed_exponential_MA, None, __("exp_ma"),
        same_scale=True)
AggDesc('harmonic', windowed_harmonic_mean, stats.hmean, __("harmonic"),
        same_scale=True)
AggDesc('geometric', pmw(stats.gmean), stats.gmean, __("geometric"),
        same_scale=True)
AggDesc('non-zero', moving_count_nonzero,
        lambda x: np.sum((x != 0) & np.isfinite(x)),  __("non_zero"),
        supports_discrete=True, count_aggregate=True)
AggDesc('defined', moving_count_defined,
        lambda x: np.sum(np.isfinite(x)), __("defined"),
        supports_discrete=True, count_aggregate=True)
AggDesc('cumsum', windowed_cumsum, None, __("cumsum"),
        cumulative=np.nancumsum)
AggDesc('cumprod', windowed_cumprod, None, __("cumprod"),
...
PeriodOptions = {}
PeriodDesc(__("years"), 0, False, "Time"),
PeriodDesc(__("months"), 1, False, "Time"),
PeriodDesc(__("days"), 2, False, "Time"),
PeriodDesc(__("hours"), 3, False, "Time"),
PeriodDesc(__("minutes"), 4, False, "Time"),
PeriodDesc(__("seconds"), 5, False, "Time"),
PeriodDesc(__("month_of_year"), 1, 12, "Month",
           names=calendar.month_name[1:],
           names_option="Use month names",
           value_offset=-1),
PeriodDesc(__("week_of_year"), 2, 53, "Week",
           value_as_period=False),
PeriodDesc(__("day_of_year"), 2, 366, "Day",
           value_as_period=False),
PeriodDesc(__("day_of_month"), 2, 31, "Day"),
PeriodDesc(__("day_of_week"), 2, 7, "Day",
           value_as_period=False,
           names_option="Use day names",
           names=calendar.day_name),
PeriodDesc(__("hour_of_day"), 3, 24, "Hour")
...
def time_blocks(data: Timeseries,
    ...
    if period.periodic:
        ...
        elif period.name == __("day_of_week"):
            times = [d.weekday() for d in times]
        elif period.name == __("day_of_year"):
            times = [d.toordinal() - date(d.year, 1, 1).toordinal() + 1
                     for d in times]
        elif period.name == __("week_of_year"):
    ...
    if period.name == __("month_of_year") and not use_period_names:
        periods += 1
