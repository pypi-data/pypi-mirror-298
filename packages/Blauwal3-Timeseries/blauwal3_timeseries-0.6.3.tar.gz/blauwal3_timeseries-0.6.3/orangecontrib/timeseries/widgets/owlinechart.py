...
# TODO - add spline
class CurveTypes:
    ITEMS = [__("gbox.line"), __("gbox.step_line"), __("gbox.column"), __("gbox.area")]
...
class LineChartEditor(QWidget):
    ...
    def __init__(self, model: PyListModel):
        ...
        hlayout.addWidget(QLabel(__("label_type")))
        ...
        self.__log_check_box = QCheckBox(__("checkbox_logarithmic_axis"))
...
class OWLineChart(OWWidget):
    ...
    class Inputs:
        ...
        forecast = Input('Forecast', Timeseries,  label=i18n.t("timeseries.common.forecast"))
    ...
    def setup_gui(self):
        ...
        self._add_button = gui.button(self.controlArea, self, __("btn_add_plot"),
    ...
    def apply_settings(self):
        ...
        for attrs, log, typ in zip(self.attrs, self.is_logit, self.plot_type):
            if typ == "spline":  # TODO - spline is missing
                typ = __("gbox.line")
            if typ not in CurveTypes.ITEMS:
                typ = CurveTypes.ITEMS[0]
