...
class OWMovingTransform(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        no_aggregations = widget.Msg(__("msg.no_selected"))
        inapplicable_aggregations = \
            widget.Msg(__("msg.only_sliding_window"))
        window_to_large = widget.Msg(__("msg.window_large"))
        block_to_large = widget.Msg(__("msg.block_large"))

    class Error(widget.OWWidget.Error):
        migrated_aggregate = widget.Msg(__("msg.replaced"))

    DiscardOriginal, KeepFirst, KeepMiddle, KeepLast = range(4)
    REF_OPTIONS = (__("ref_options.discard_original"),__("ref_options.keep_first_instance"),
                   __("ref_options.keep_middle_instance"),__("ref_options.keep_last_instance"))

    KeepComplete, KeepAll = range(1, 3)
    KEEP_OPTIONS = (__("keep_options.discard_original"),
                    __("keep_options.keep_original"),
                    __("keep_options.include_leading_instances"))
    KEEP_TOOLTIPS = (__("keep_options_tooltips.discard_original"),
                     __("keep_options_tooltips.keep_original"),
                     __("keep_options_tooltips.include_leading_instances"))
    ...
    period_width = settings.Setting(i18n.t('timeseries.adddesc.years'))
    ...
    def __init__(self):
        ...
        vbox = gui.vBox(box, __("btn.aggregation_type"))
        buttons = gui.radioButtons(
            vbox, self, "method", callback=self._method_changed)

        gui.appendRadioButton(buttons, __("btn.sliding_window"))
        indbox = gui.indentedBox(buttons)
        gui.spin(
            indbox, self, 'window_width',
            2, 1000, label=__("btn.window_width"),
        ...
        gui.appendRadioButton(buttons, __("btn.consecutive_block"), buttons)
        indbox = gui.indentedBox(buttons)
        gui.spin(
            indbox, self, 'block_width',
            2, 1000, label=__("btn.block_width"),
            controlWidth=80, alignment=Qt.AlignRight,
        ...
        self.rb_period = gui.appendRadioButton(
            buttons, __("btn.aggregate_time_periods"), buttons)
        indbox = gui.indentedBox(buttons)
        cb = gui.comboBox(
        ...
        # 确保 period_width 是有效的
        if self.period_width not in PeriodOptions:
            self.period_width = list(PeriodOptions.keys())[0]

        gui.rubber(buttons)

        vbox = gui.vBox(self.mainArea, True)
        self.filter_line = QLineEdit(placeholderText=__("btn.filter"))
        ...
        gui.checkBox(vbox, self, "only_numeric", __("btn.show_numeric"),
        ...
        gui.auto_commit(self.buttonsArea, self, 'autocommit', __("btn.apply"))
