...
if sys.version_info < (3, 8):
    print(
        "SymPy requires Python 3.8 or newer. Python %d.%d detected"
        % sys.version_info[:2]
    )
    sys.exit(-1)
...
class antlr(Command):
    ...
    def run(self):
        from sympy.parsing.latex._build_latex_antlr import (
            build_parser as build_latex_parser,
        )

        if not build_latex_parser():
            sys.exit(-1)

        from sympy.parsing.autolev._build_autolev_antlr import (
            build_parser as build_autolev_parser,
        )
...
class sdist_sympy(sdist):
    def run(self):
        ...
        try:
            ...
            print(
                "Successfully removed temporary file {}.".format(commit_hash_filepath)
            )
        except OSError as e:
...
if __name__ == "__main__":
    setup(
        ...
        url="https://sympy.org",
        project_urls={
            "Source": "https://github.com/majiayu000/sympy",
        },
