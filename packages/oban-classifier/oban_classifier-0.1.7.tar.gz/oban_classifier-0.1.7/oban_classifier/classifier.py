...
class ObanModule(nn.Module):
    ...
    def forward(self, X, **kwargs):
        X = X.float()  # Burada girdiyi float32'ye zorlayın
...
def oban_classifier(X, y, num_units=128, num_classes=2, nonlin=nn.ReLU(), dropout_rate=0.5, 
    ...
    X_train = scaler.fit_transform(X_train).astype(np.float32)  # float32'ye dönüştürme
    X_test = scaler.transform(X_test).astype(np.float32)  # float32'ye dönüştürme
