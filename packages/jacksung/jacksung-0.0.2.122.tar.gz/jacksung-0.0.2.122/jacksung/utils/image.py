...
def zoomAndDock(img, zoom_rectangle, docker, scale_factor=2, border=10):
    ...
    img[docker[1]:docker[1] + corp_png.shape[0], docker[0]:docker[0] + corp_png.shape[1], :] = corp_png
    corp_png[:, 0:border * 2, :] = [0, 0, 0]
    corp_png[:, corp_png.shape[1] - border * 2:corp_png.shape[1], :] = [0, 0, 0]
    corp_png[0:border * 2, :, :] = [0, 0, 0]
    corp_png[corp_png.shape[0] - border * 2:corp_png.shape[0], :, :] = [0, 0, 0]
