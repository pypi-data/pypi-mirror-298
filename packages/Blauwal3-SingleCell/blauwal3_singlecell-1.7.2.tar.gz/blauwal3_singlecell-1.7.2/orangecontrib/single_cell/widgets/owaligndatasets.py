...
def __(key):
    return i18n.t('single_cell.owaligndatasets.' + key)
...
SCORINGS = OrderedDict([
    (__("gbox.pearson_correlation"), "pearson"),
    (__("gbox.spearman_correlation"), "spearman"),
    (__("gbox.biweights_midcorrelation"), "bicor"),
])
...
class OWAlignDatasets(widget.OWWidget):
    ...
    class Outputs:
        transformed_data = Output("Transformed Data", Table, label=i18n.t("single_cell.common.transformed_data"))
        genes_components = Output("Genes per n. Components", Table, label=i18n.t('single_cell.common.genes_components'))
    ...
    class Error(widget.OWWidget.Error):
        no_features = widget.Msg(__('msg.no_features'))
        no_instances = widget.Msg(__("msg.no_instances"))
        no_class = widget.Msg(__('msg.no_class'))
        nan_class = widget.Msg(__('msg.nan_class'))
        nan_input = widget.Msg(__('msg.nan_input'))
        sparse_data = widget.Msg(__('msg.sparse_data'))
        only_one_dataset = widget.Msg(__('msg.only_one_dataset'))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __('box.data_source_indicator'))
        ...
        box = gui.vBox(self.controlArea, __('box.canonical_correlation_analysis'))
        gui.spin(
            box, self, "ncomponents", 1, MAX_COMPONENTS,
            callback=self._update_selection_component_spin,
            keyboardTracking=False,
            label=__("label.num_components")
        )

        # Shared genes
        box = gui.vBox(self.controlArea, __("box.shared_genes"))
        gui.spin(
        ...
        # Post-processing
        box = gui.vBox(self.controlArea, __("box.post-process"))
        gui.doubleSpin(
            ...
            label=__("gbox.quantile_normalization"), checked="quantile_normalization",
        )
        ...
        gui.checkBox(
            ...
            label=__("gbox.dynamic_time_warping")
        )

        self.controlArea.layout().addStretch()

        gui.auto_commit(self.controlArea, self, "auto_commit", __("btn.apply"),
                        callback=self._invalidate_selection(),
                        checkbox_label=__('btn.apply_automatically'))

        self.plot = pg.PlotWidget(background="w")

        axis = self.plot.getAxis("bottom")
        axis.setLabel(__('label.correlation_component'))
        axis = self.plot.getAxis("left")
        axis.setLabel(__('label.correlation_strength'))
        self.plot_horlabels = []
        self.plot_horlines = []
    ...
    def send_report(self):
        if self.data is None:
            return
        self.report_items((
            (__("report.source_id"), self.source_id),
            (__('report.num_components'), self.ncomponents),
            (__("report.num_genes"), self.ngenes),
            (__("report.score"), self.scoring),
            (__("report.quantile_normalization"), True if self.quantile_normalization else "False"),
            (__("report.quantile_normalization_percentage"),
             self.quantile_normalization_perc if self.quantile_normalization else False),
            (__("report.dynamic_time_warping"), True if self.dynamic_time_warping else "False")
        ))
        self.report_plot()
