...
def path2path(
    ...
    ----
    src_fp : PathLike
        源文件或目录的路径。支持字符串、pathlib.Path 等路径类型。

    dst_fp : PathLike
        目标文件或目录的路径。支持字符串、pathlib.Path 等路径类型。

    operation : Literal["COPY", "MOVE"]
        操作类型，可选值为 "COPY" 或 "MOVE"。

    cpy_type : Literal["replace", "append"], 默认为 "replace"
        复制或移动类型，可选值为 "replace" 或 "append"。
    ...
    ----
    ValueError
        如果 `operation` 或 `cpy_type` 不是允许的选项之一，则抛出此异常。
    """
...
def _copy_dir2path(
    ...
    ----
    src_dir_fp : PathLike
        源目录的路径。支持字符串、pathlib.Path 等路径类型。

    dst_dir_fp : PathLike
        目标目录的路径。支持字符串、pathlib.Path 等路径类型。

    cpy_type : Literal["replace", "append"], 默认为 "replace"
        复制类型，可选值为 "replace" 或 "append"。
...
def _move_file2path(
    ...
    ----
    src_fp : PathLike
        源文件的路径。支持字符串、pathlib.Path 等路径类型。

    dst_fp : PathLike
        目标文件的路径。支持字符串、pathlib.Path 等路径类型。
