...
def setup_logger(
    log_file: str = "", name: typing.Optional[str] = None, level: int = logging.INFO
) -> logging.Logger:
    """
    ...
    参数
    ----
    log_file : Optional[str], 默认为空字符串
        日志文件的路径。如果提供，则会添加一个RotatingFileHandler。

    name : Optional[str], 默认为None
        日志记录器的名字。如果为None，则默认使用根记录器。

    level : int, 默认为logging.INFO
        日志记录级别。
