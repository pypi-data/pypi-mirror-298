...
class TaskAdapter:
  ...
  def _build_youtube_filters(self) -> list[str]:
    ...
    has_latin = (
      'YOUTUBE_CHANNEL_INFO:title has_latin_letters '
  ...
  def _build_task_output(self) -> task.TaskOutput:
    ...
    if not self.task_v1_data['emailAlerts'] and self.task_v1_data['schedule']:
      return task.TaskOutput.EXCLUDE
    if self.task_v1_data['emailAlerts'] and not self.task_v1_data['schedule']:
