from v4_proto.gogoproto import gogo_pb2 as _gogo_pb2
from v4_proto.dydxprotocol.vault import params_pb2 as _params_pb2
from v4_proto.dydxprotocol.vault import vault_pb2 as _vault_pb2
from google.protobuf.internal import containers as _containers
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from typing import ClassVar as _ClassVar, Iterable as _Iterable, Mapping as _Mapping, Optional as _Optional, Union as _Union

DESCRIPTOR: _descriptor.FileDescriptor

class GenesisState(_message.Message):
    __slots__ = ("params", "vaults")
    PARAMS_FIELD_NUMBER: _ClassVar[int]
    VAULTS_FIELD_NUMBER: _ClassVar[int]
    params: _params_pb2.Params
    vaults: _containers.RepeatedCompositeFieldContainer[Vault]
    def __init__(self, params: _Optional[_Union[_params_pb2.Params, _Mapping]] = ..., vaults: _Optional[_Iterable[_Union[Vault, _Mapping]]] = ...) -> None: ...

class Vault(_message.Message):
    __slots__ = ("vault_id", "total_shares", "owner_shares", "vault_params", "most_recent_client_ids")
    VAULT_ID_FIELD_NUMBER: _ClassVar[int]
    TOTAL_SHARES_FIELD_NUMBER: _ClassVar[int]
    OWNER_SHARES_FIELD_NUMBER: _ClassVar[int]
    VAULT_PARAMS_FIELD_NUMBER: _ClassVar[int]
    MOST_RECENT_CLIENT_IDS_FIELD_NUMBER: _ClassVar[int]
    vault_id: _vault_pb2.VaultId
    total_shares: _vault_pb2.NumShares
    owner_shares: _containers.RepeatedCompositeFieldContainer[_vault_pb2.OwnerShare]
    vault_params: _vault_pb2.VaultParams
    most_recent_client_ids: _containers.RepeatedScalarFieldContainer[int]
    def __init__(self, vault_id: _Optional[_Union[_vault_pb2.VaultId, _Mapping]] = ..., total_shares: _Optional[_Union[_vault_pb2.NumShares, _Mapping]] = ..., owner_shares: _Optional[_Iterable[_Union[_vault_pb2.OwnerShare, _Mapping]]] = ..., vault_params: _Optional[_Union[_vault_pb2.VaultParams, _Mapping]] = ..., most_recent_client_ids: _Optional[_Iterable[int]] = ...) -> None: ...
