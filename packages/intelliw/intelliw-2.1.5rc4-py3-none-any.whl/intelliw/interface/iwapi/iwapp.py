    ...
    def handler_process(self, routers):
        ...
        for router, info in Application.HANDLERS:
            ...
            self.app.add_api_route(
                router, endpoint=iwhandler.api_handler, methods=[method])
            if router in _route_dup_dict:
                logger.warning("方法: %s, 访问路径：%s 将被 方法: %s 覆盖", _route_dup_dict[router], router, func)

            _route_dup_dict[router] = func
