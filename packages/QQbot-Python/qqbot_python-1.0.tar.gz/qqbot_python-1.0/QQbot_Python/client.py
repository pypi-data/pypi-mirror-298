...
from .gateway import BotWebSocket,NewWebSocket
...
class Client:
    ...
    def __init__(
        ...
        self._ws_ap: Dict = {}
        self.ws_url: str = ""
        self.show_heartbeat_info: bool = False
        self.show_system_message: bool = False
    ...
    async def bot_connect(self, session):
        """
        启动两个新的连接，能够同时监听两个 WebSocket。

        :param session: 第一个 session 对象
        """
        _log.info("[botpy] 会话启动中...")

        client1 = BotWebSocket(session, self._connection)
        client2 = NewWebSocket("ws://localhost:3001", self.show_heartbeat_info,self.show_system_message)  # 假设 session2 有一个 URL 属性

        try:
            await asyncio.gather(client1.ws_connect(), client2.connect())  # 直接在 await 中调用
            _log.info(f"robot 「{self.robot.name}」 on_ready!")
        except (Exception, KeyboardInterrupt, SystemExit) as e:
            await asyncio.gather(client1.on_error(e), client2.on_error(e))  # 使用 asyncio.gather 处理两个连接的错误
