        ...
        def decorator(f: Callable) -> Callable:
            ...
            def wrapped(*_args, **_kwargs):
                ...
                if auth_method:
                    ...
                    # Apply the authentication decorators
                    for method in auth_method:
                        if method == "jwt":
                            f_decorated = jwt_authentication(f_decorated)
                        elif method == "basic":
                            pass
                            # f_decorated = basic_authentication(f_decorated)
                        elif method == "api_key":
                            pass
                            # f_decorated = api_key_authentication(f_decorated)
                        elif method == "custom":
                            custom_auth = get_config_or_model_meta("API_CUSTOM_AUTH")
                            f_decorated = custom_auth(f_decorated)
                        else:
                            # Handle unknown authentication methods if necessary
                            pass

                # Apply input/output handling decorators
                f_decorated = (
            ...
            # Set route information for documentation purposes
            route_info = {
                "function": wrapped,
                "output_schema": output_schema,
                "input_schema": input_schema,
