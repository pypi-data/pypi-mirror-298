...
def process_leiden(adata, save_path):
    img_path = os.path.join(save_path, 'images')
    ...
    umap_info['leiden'] = adata.obs['leiden']
    umap_info['id'] = adata.obs.index
    umap_info.to_parquet(os.path.join(data_path, 'umap_leiden.parquet'))

    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Leiden clustering completed in {elapsed_time:.2f} seconds.")

def process_hvg(adata, save_path):
    img_path = os.path.join(save_path, 'images')
    data_path = os.path.join(save_path, 'data_front')
...
def process_co_occurrence(adata, save_path):
    img_path = os.path.join(save_path, 'images')
    if not os.path.exists(img_path):
        os.makedirs(img_path)
    ...
    for i in tqdm(adata.obs['leiden'].unique(), desc="Processing clusters", unit="cluster"):
        sq.pl.co_occurrence(
            adata_subsample,
            cluster_key="leiden",
            clusters=i,
...
def process_nhood_enrichment(adata, save_path):
    img_path = os.path.join(save_path, 'images')
    download_path = os.path.join(save_path, 'data_download')
    if not os.path.exists(img_path):
        os.makedirs(img_path)
