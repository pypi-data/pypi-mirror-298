...
def cat_plot(df,target,title='',drop=[]):
    ...
    #target='Rent'
    sleep(0.1)
    cols=df[cat_col+num_but_cat].columns.drop(drop)
    n_cols=2
    n_rows=len(cols)
    ...
    graph_max=3 #Max chart count per page
    g_height=2 #chart height const
    len_col=len(cols.to_list()) # Len col size
    loop_count=int(len_col/graph_max) # Loop count for page
    leap_count=len_col%graph_max # Chart count for last page . For example if len_col 7 , len_col%graph_max equal to 1 so last page chart count is 1 
...
def pairplot(df,title,n_cols=3,drop=[],render_local=False):
    ...
    df_sampled=df.sample(round(df.shape[0]*0.1),replace=False)
    ...
    for i, var_name in enumerate(cols,start=1):
        ...
        if row_n==n_rows+1:
            row_n=1
        #compress eklendi 
        boxfig= go.Figure(data=[go.Box(x=df_sampled[var_name], showlegend=False, notched=True, marker_color=random_color, name=var_name)])
        for k in range(len(boxfig.data)):
            fig.add_trace(boxfig.data[k], row=row_n, col=col_n)
        group_labels = [var_name]
        hist_data=[df_sampled[var_name].values]
