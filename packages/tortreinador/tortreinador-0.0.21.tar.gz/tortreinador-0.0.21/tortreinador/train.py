...
class TorchTrainer:
    ...
    def fit(self, t_l, v_l, checkpoint_=None, **kwargs):
        ...
        for e in range(START_EPOCH, self.epoch):
            ...
            i = 0
            if 'mix' in kwargs.keys() and e < kwargs['mix']['warmup_epoch']:
                self.current_error_rate = (FINAL_RATE - INIT_RATE) * (
                            e / (kwargs['mix']['warmup_epoch'] - 1)) + INIT_RATE
...
def config_generator(model_save_path: str, warmup_epochs: int = None, lr_milestones: list = None,
                     ...
                     auto_save: int = 10, if_mix: bool = False, initial_rate: float = 0.1, final_rate: float = 1, cov: object = None, warmup_epoch: int = 10):
    """
