...
@alru_cache()
async def image2nowatermark_image(url, oss: str = 'glm', token: Optional[str] = None):
    if not url: return
    ...
    request = BDAITPZSRequest(picInfo=image_to_base64(url))
    ...
    if oss == 'glm':
        ...
        file_task = await glm_video.upload_task(file)
        return file_task and file_task.url
    elif oss == 'kling':  # kuaishou
