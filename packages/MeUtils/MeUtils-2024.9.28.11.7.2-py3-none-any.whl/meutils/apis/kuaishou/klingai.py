...
async def get_task_plus(task_id, token: str, oss: Optional[str] = None):  # image video
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, headers=headers, timeout=60) as client:
        ...
        if response.is_success:
            ...
            if any(urls):  # 任务完成
                if not vip and task_type == "images":  # 非会员图片去水印

                    func = partial(image2nowatermark_image, oss=oss or "kling", picinfo=PICINFO2_RIGHT_BOTTOM)
                    urls = await asyncio.gather(*map(func, urls))

                else:  # 会员获取无水印链接
                    func = partial(download, token=token)
                    urls = await asyncio.gather(*map(func, work_ids))
...
@retrying(max_retries=3, title="kling-download")
async def download(work_id, token):
    ...
    # "https://klingai.com/api/works/batch_download_v2?workIds=100331495&fwm=false"
