...
def generate_dicts_4_kb(kb=None, mode='reel', terms_filename=None, 
    ...
    if mode == 'reel':
        ...
        if kb_obj.alt_id_to_id is not None:
            alt_id_to_id = json.dumps(kb_obj.alt_id_to_id)
            
            with open(out_dir + '/alt_id_to_id.json', 'wb') as outfile6:
                outfile6.write(alt_id_to_id)
        
        if kb_obj.gene_id_2_alt_ids is not None:
            gene_id_2_alt_ids = json.dumps(kb_obj.gene_id_2_alt_ids)
            
            with open(out_dir + '/gene_id_2_alt_ids.json', 'wb') as outfile7:
                outfile7.write(gene_id_2_alt_ids)
