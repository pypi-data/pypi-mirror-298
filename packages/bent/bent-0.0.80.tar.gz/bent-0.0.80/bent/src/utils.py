...
def _import_reel_results(doc_id, nel_run_ids):
    ...
    for run_id in nel_run_ids:
        ...
        if target_filename in results_files:
            filepath = f"{results_dir}{target_filename}"
...
def _update_ner_file_with_nel_output(
        ner_dir, 
        nel_run_ids, 
        gene_present=False,
        out_dir=None):
    ...
    if gene_present:
        #Load the mapping of the gene IDs in CTD-Gene to the alternative IDs
        with open(f'{cfg.root_path}/data/kbs/dicts/ctd_gene/gene_id_2_alt_ids.json', "r", encoding="utf-8") as infile:
            gene_id_2_alt_ids = json.loads(infile.read())
            infile.close()
    ...
    for filename in ner_filenames:
        ...
        for line in ner_output:
            ...
            if line != "\n":
                ...
                entity_start = line.split("\t")[1].split(" ")[1]
                final_output += line

                if final_output[-1:] != "\n":
                    final_output += "\n"

                key_name = f"{entity_text.replace(' ', '_')}_{entity_start}_{entity_type}"
                
                if key_name in linked_entities and term_id[0] == "T":
                    kb_id = linked_entities[key_name][0]

                    if 'gene' in entity_type or 'Gene' in entity_type:
                        #add alternative IDs to the
                        alt_ids = gene_id_2_alt_ids.get(kb_id)
                        
                        if alt_ids is not None:
                            kb_id = kb_id + '|' + '|'.join(alt_ids)
