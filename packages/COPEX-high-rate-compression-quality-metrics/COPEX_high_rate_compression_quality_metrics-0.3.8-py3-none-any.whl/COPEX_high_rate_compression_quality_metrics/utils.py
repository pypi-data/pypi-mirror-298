...
Optional[str]:
    ...
    try:
        ...
        ValueError("no result found from root dir ", root_dir, " with nnvvpp = ", nnvvpp_algoname,
                   " in bracket number 0")
...
def check_bracket_content(folder_name: str, search_str: str, bracket_num: int, verbose = False) -> bool:
    ...
    if 0 <= bracket_num <= len(fields):
        ...
        if verbose: print("---- check_bracket_content ---- selected_field = ",selected_field)
...
def list_matching_folders(root_dir: str, search_str: str, bracket_num: int, verbose = False) -> List[str]:
    ...
    if verbose: print("list_matching_folders --> LIST OF DIRS : ",os.listdir(root_dir))
    for folder_name in os.listdir(root_dir):
        if verbose : print("list_matching_folders --> folder name [",folder_name,"]...")
        ...
        if os.path.isdir(folder_path):
            ...
            if check_bracket_content(folder_name, search_str, bracket_num, verbose=verbose):
                if verbose : print("list_matching_folders --> folder FOUND [", folder_name, "].")
...
#get_algorithm_results_full_path("..\\test\\data","S2_L1C",1,"01-01-005_JJ2000_x250",verbose=True)
