...
class Client:
    ...
    def _remove(
        self, ctzn: spin.Name, p: spin.Path
    ) -> tuple[typing.Optional[data.Entry], typing.Optional[spin.Error]]:
        if self.options.dir_server is None:
            return None, spin.ErrNotImplemented
        ...
        if resp.error != "":
            if "not found" in resp.error:
                return None, spin.ErrNotFound
            return None, spin.error_from_string(resp.error)

        if len(resp.entries) != 1:
            raise ValueError(f"expected 1 entry, got {len(resp.entries)}")
    ...
    def _remove_all(
        ...
        list_resp = self.options.dir_server.list(
            data.DirListRequest(
                public=self.options.public,
                private=self.options.private,
                citizen=ctzn,
        ...
        if list_resp.error != "":
            return None, spin.error_from_string(list_resp.error)

        ops = []
        for entry in reversed(list_resp.entries):
            ops.append(
                data.DirOp(
                    type=data.DirOpType.DelDirOp,
                    entry=data.Entry(
        ...
        # Ensure that the only changes to this tree are from our removals.
        if ops:
            ops[-1].entry.version = list_resp.entries[0].version
