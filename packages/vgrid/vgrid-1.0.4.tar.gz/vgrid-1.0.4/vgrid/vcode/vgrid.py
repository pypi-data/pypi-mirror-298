...
def main():
    ...
    for zoom_level in range(args.minzoom, args.maxzoom + 1):
        ...
        # create_tiles(list_tiles, args.output, zoom_level)
                # Process tiles in chunks
        tiles_len = len(list_tiles)        
        if tiles_len <= 10000:
            create_tiles(list_tiles, args.output, zoom_level)
        else:
            print (f'There are {tiles_len} tiles at zoom level {zoom_level}. Process tiles by chunk size = {args.chunksize}')
            for tile_chunk in chunk_list(list_tiles, args.chunksize):
