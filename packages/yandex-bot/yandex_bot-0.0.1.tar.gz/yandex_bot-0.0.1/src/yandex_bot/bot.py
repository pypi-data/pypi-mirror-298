...
# Usage example
bot_api = BotAPI(token="y0_AgAAAAB46fHYAATIlgAAAAESFKpMAAB1QU9cdB5Ga6zSVA_3Tb59OGOjpw")


# Register message handlers using decorators
@bot_api.message_handler(r"^Привет")
async def handle_hello(message: Message):
    print(f"Handling 'Hello' message: {message.text}")
    # Now we just pass the message object to the send method without worrying about login or chat_id
    await bot_api.send_text_message(message, "Hi there!")


# Start polling for messages
asyncio.run(bot_api.start_polling())
