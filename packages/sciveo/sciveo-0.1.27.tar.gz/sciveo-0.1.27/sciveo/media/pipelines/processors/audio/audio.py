...
class AudioExtract(BaseProcessor):
  ...
  def plot_new_process(self, w, h, sampling_rate, aac_audio_local_path, image_audio_local_path):
    cmd = "python pipelines/processors/audio/audio_extractor_process.py "
    cmd += f"--width {w} --height {h} "
    cmd += f"--sampling_rate {sampling_rate} "
    cmd += f"--aac_audio_local_path '{aac_audio_local_path}' "
    cmd += f"--image_audio_local_path '{image_audio_local_path}' "
