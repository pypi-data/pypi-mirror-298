...
window.vm = new Vue({
  ...
  methods: {
    ...
    sourceToJstree: function (source) {
      ...
      n.id = source._id;
      n.text = source._type
      let node = this.originNodeMaps[source._id]
      if (node.mappingId) {
        n.text += " - " + node.mappingId;
      } else if (node.name) {
        n.text += " - " + node.name;
      } else if (node.resourceId) {
        n.text += " - " + node.resourceId;
      }
    ...
    initJstree: function () {
      ...
      $("#jstree-search").on('propertychange input', function (e) {
        var value = $(this).val();
        if (!value) {
          $("#search-results").text("0 results");
        }
        var ret = $jstree.jstree(true).search(value);
      })
      // 监听 jstree 搜索事件，更新搜索结果数量
      $jstree.on("search.jstree", function (e, data) {
        var resultCount = data.nodes.length;  // 获取搜索结果数量
        $("#search-results").text(resultCount + " results");  // 更新显示结果数量
      });
    },
    ...
    reloadNodeMappingID() {
      if (Object.keys(this.mappingMaps).length > 0) {
        for (let nodeIndex in this.originNodeMaps) {
          let nodeInfoMap = this.originNodeMaps[nodeIndex];
          let resourceId = nodeInfoMap["resourceId"];
          if (resourceId === undefined) {
            continue;
          }
          // 调整 mappingID 展示位置
          let searchKey = resourceId.substring(resourceId.indexOf('/') + 1)
          let mappingId = this.mappingMaps[searchKey]
          if (mappingId) {
            let newInfoMap = {}
            for (let key in nodeInfoMap) {
              newInfoMap[key] = nodeInfoMap[key]
              if (key === "resourceId") {
                newInfoMap["mappingId"] = mappingId
              }
            }
            this.originNodeMaps[nodeIndex] = newInfoMap;
          }
