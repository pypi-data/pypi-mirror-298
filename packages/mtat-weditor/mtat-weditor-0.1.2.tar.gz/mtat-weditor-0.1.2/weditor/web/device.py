...
class _AppleDevice(DeviceMeta):
    def __init__(self, device_url):
        ...
        if device_url.startswith('http'):
            ...
            self.__start_wda_for_17(device_url, True)
    ...
    def check_wda(self):
        try:
            ...
            if not self.__device_url.startswith('http'):
                self.__start_wda_for_17(self.__device_url, False)
                logger.info("ios connect: %s", self.__device_url)
                c = wda.USBClient(self.__device_url)
                self._client = c
                self.__scale = max(c.screenshot().size) / max(c.window_size())

    def __start_wda_for_17(self, device_url, check_process):
        if device_url.startswith('http'):
            return
        d = IOSDeviceHelper(device_url)
        if not d.is_version_ge_17():
            return
        if check_process and d.is_app_started("com.facebook.WebDriverAgentRunner.xctrunner"):
            return
        d.launch_wda_for_17()
