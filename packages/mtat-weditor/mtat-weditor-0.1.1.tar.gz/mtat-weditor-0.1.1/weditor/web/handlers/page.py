...
class DeviceIndexHandler(BaseHandler):
    def post(self):
        ...
        frame = data["frame"]
        label = data["label"]
        x, y, w, h = frame.replace("{", "").replace("}", "").replace(" ", "").split(",")
        d = get_device(device_id)
        s = d.device.session()
        elements = s(className=elem_type).find_elements()
        result = -1
        index = 0
        for elem in elements:
            ex, ey, ew, eh = elem.bounds
            if int(x) == ex and int(y) == ey and int(w) == ew and int(h) == eh and label == elem.label:
                result = index
                break
            index += 1
        self.write({
            "index": str(result),
            "maxIndex": str(len(elements)-1),
            "success": True
        })
