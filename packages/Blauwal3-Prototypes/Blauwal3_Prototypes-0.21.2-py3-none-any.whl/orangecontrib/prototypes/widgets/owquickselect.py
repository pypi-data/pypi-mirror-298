...
def __(key):
    return i18n.t('prototypes.owquickselect.' + key)
...
class OWQuickSelect(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        no_categorical = Msg(__("no_categorical"))
    ...
    def __init__(self):
        ...
        value_combo = gui.comboBox(
            None, self, "value", label=__("value"),
            model=self.value_model, callback=self._on_value_changed,
            contentsLength=50,
            sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))

        form.addRow(__("variable"), var_combo)
        form.addRow(__("value"), value_combo)
