...
def __(key):
    return i18n.t('prototypes.owgrep.' + key)
...
class OWGrep(OWWidget, RecentPathsWComboMixin):
    ...
    class Warning(OWWidget.Warning):
        no_lines = Msg(__("no_lines"))
        no_header_row = Msg(__("no_header_row"))

    class Error(OWWidget.Error):
        unreadable = Msg(__("unreadable"))
        file_not_found = Msg(__("file_not_found"))
        block_too_short = Msg(__("block_too_short"))
    ...
    def __init__(self):
        ...
        box = gui.widgetBox(
            self.controlArea, box=__("File"), orientation=QGridLayout())
        box.layout().addWidget(self.file_combo, 0, 0, 1, 2)
        self.file_combo.activated[int].connect(self.select_file)
        box.layout().addWidget(
            gui.button(
                None, self, __("Open"), callback=self.browse_file,
                autoDefault=False,
        ...
        box = gui.widgetBox(self.controlArea, box=__("Pattern"))
        lineedit = gui.lineEdit(box, self, "pattern")
        lineedit.returnPressed.connect(self.grep_lines)
        gui.checkBox(
            box, self, "case_sensitive", label=__("case_sensitive"),
            callback=self.grep_lines)
        gui.checkBox(
            box, self, "regular_expression", label=__("regular_expression"),
            callback=self.grep_lines)

        box = gui.widgetBox(self.controlArea, box=__("Format"))
        gui.spin(
            box, self, "skip_lines", 0, 10, label=__("skip_lines"),
            callback=self.grep_lines)
        gui.spin(
            box, self, "block_length", 1, 100000, label=__("block_length"),
            callback=self.grep_lines)
        gui.checkBox(
            box, self, "has_header_row", label=__("has_header_row"),
            tooltip=__("has_header_row_tooltip"),
        ...
        gui.widgetLabel(box, __("input_text"))
        gui.rubber(box)
        find_line = gui.lineEdit(
            box, self, "find_text", label=__("find"), orientation=Qt.Horizontal,
        ...
        gui.widgetLabel(self.mainArea, __("used_lines"))
