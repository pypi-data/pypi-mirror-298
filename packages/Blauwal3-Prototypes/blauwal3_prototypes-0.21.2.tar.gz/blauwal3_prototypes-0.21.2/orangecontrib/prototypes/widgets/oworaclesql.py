...
def __(key):
    return i18n.t('prototypes.oworaclesql.' + key)
...
class OWOracleSQL(OWWidget):
    ...
    class Error(OWWidget.Error):
        no_backends = Msg(__("no_backends"))
    ...
    def __init__(self):
        ...
        #Control Area layout
        self.connectBox = gui.widgetBox(self.controlArea, __("database_connection"))
        self.connectBox.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)

        #Database
        self.userLabel = gui.label(self.connectBox, self, __("user_name"))
        self.connectUser = QLineEdit(self.username, self)
        self.connectBox.layout().addWidget(self.connectUser)
        self.passwordLabel = gui.label(self.connectBox, self, __("password"))
        ...
        self.dbLabel = gui.label(self.connectBox, self, __("database"))
        ...
        self.runSQL = gui.auto_commit(self.connectBox, self, 'autocommit',
                                      label=__("run_sql"), commit=self.commit)
        # query
