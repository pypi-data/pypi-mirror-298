...
class SpeechText:
    ...
    def set_ssml_format(ssml: str,
                        ...
                        output_format: str,
                        region='westeurope'):
        """Converte SSML em áudio com progresso usando yield."""
        endpoint = f'https://{region}.tts.speech.microsoft.com/cognitiveservices/v1'
        headers = {
            'Authorization': f'Bearer {get_access_token()}',
            'Content-Type': 'application/ssml+xml',
        ...
        try:
            # Iniciar a requisição com streaming ativado
            response = requests.post(endpoint, headers=headers, data=body.encode('utf-8'), stream=True)
            ...
            with open(file_path, 'wb') as audio_file:
                # Ler o conteúdo do stream em chunks
                for chunk in response.iter_content(chunk_size=1024):
                    if chunk:  # Apenas processa se o chunk não estiver vazio
                        audio_file.write(chunk)
                        yield chunk  # Retorna o chunk progressivamente para o cliente

            yield file_path  # Retorna o caminho final do arquivo após o download
