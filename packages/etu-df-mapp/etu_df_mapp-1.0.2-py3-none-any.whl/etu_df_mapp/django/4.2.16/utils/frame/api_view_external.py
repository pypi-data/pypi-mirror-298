...
from utils.frame.custom_authentication_classes import WebTokenAuthentication
...
class BaseApiView(APIView):
    ...
    @classmethod
    def as_v1_view(cls):
        """自定义接口视图，去除固定格式，只返回 data 的 JSON 格式"""
        v3_view = cls.as_view()

        def view(request, *args, **kwargs):
            response = v3_view(request, *args, **kwargs)
            # 判断是的符合标准基类返回格式，去除固定格式，只返回 data 的 JSON 格式
...
class BaseAdminApiView(BaseApiView):
    ...
    authentication_classes = (TokenAuthentication, WebTokenAuthentication)
...
class WebBaseAdminApiView(BaseApiView):
    """Web认证接口基类"""
    # 接口认证类注册
...
class WebCustomBaseAdminApiView(WebBaseAdminApiView, CustomBaseFilter):
    """Web自定义认证接口基类"""
