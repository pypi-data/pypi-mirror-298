...
def plot_stats(
    ...
    type_plot: str = "box",
    type_test: str = "mann-whitney",
    type_correction: str | None = None,
    subcategory: str | None = None,
    cutoff_pvalue: float = 0.05,
    filename: str | None = None,
    export_size: tuple[int, int, int] = (800, 600, 3),
    **kwargs
) -> None:
    """Make statistical tests and add statistical annotations on Plotly graph.
    ...
    pairs : list[tuple[str, str]] | None, optional
        Pairs of categories to compare. Default is None.
    order : list[str] | None, optional
        Order of categories on the x-axis. Default is None.
    type_plot : str, optional
        Type of plot to create. Default is "box".
    type_test : str, optional
        Type of statistical test to perform. Default is "mann-whitney".
    type_correction : str | None, optional
        Type of p-value correction to apply. Default is None.
    subcategory : str | None, optional
        Column name for subcategories. Default is None.
    cutoff_pvalue : float, optional
        P-value cutoff for significance. Default is 0.05.
    filename : str | None, optional
        Filename to save the plot. Default is None.
    export_size : tuple[int, int, int], optional
        Size of the exported plot (width, height, scale). Default is (800, 600, 3).
    **kwargs : dict
    ...
    match type_plot:
        ...
        case "bar":
            # Compute average and standard deviation for each category on the X axis
            agg_df = df.groupby(x)[y].agg(['mean', 'std']).reset_index()

            # Create a bar chart with error bars for standard deviation
            fig = px.bar(agg_df, x=x, y='mean', error_y='std', color=(x if subcategory is None else subcategory), category_orders={f"{x}": order}, **kwargs)

            scatter_colors = [bar.marker.color for bar in fig.data]  # Extract colors from each bar trace

            # Add scatter points for each category
            for idx, color in enumerate(scatter_colors):
                # Filter the dataframe for each category to match the scatter with its corresponding bar
                category_data = df[df[x] == fig.data[idx].name]
                fig.add_trace(
                    go.Scatter(
                        x=category_data[x],
                        y=category_data[y],
                        mode='markers',
                        marker=dict(color=color),  # Use the same color as the corresponding bar
                        name=fig.data[idx].name,  # Optional, to match the legend entry
                        showlegend=False  # Set to True if you want scatter to also show in legend
                    )
                )
    ...
    for _element in p_values_obj:
        ...
        fig.add_annotation(
            ...
            text=get_significance_stars(_element["p_value"]),
            showarrow=False,
