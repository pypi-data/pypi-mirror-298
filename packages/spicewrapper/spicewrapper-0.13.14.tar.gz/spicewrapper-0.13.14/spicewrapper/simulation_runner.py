...
from .spice_utils import create_symbolic_link, extract_and_format_parameters, cleanup_temp_files, read_parameters_from_cir, read_simulation_duration, modify_output_filename, parse_outtran, parse_variable_names, identify_voltage_sources, modify_cir_with_param_dict
...
from .workers import worker_essentials, generate_basinhopping_work_list, generate_grid_work_list,worker_grid, worker_basinhopping,BasinhoppingParams
...
def run_spicemanager_single(
    ...
    '''
    return run_spicemanager(cir_file_path,subcircuit_path, None, None, 1e7, 1e7, interpolation_timestep, None, None, 1, temp_folder,None,False)[1]
...
def run_spicemanager(
    ...
    sym_path = temp_folder + 'subcircuits'
    create_symbolic_link(subcircuit_path, sym_path)
    ...
    original_duration = read_simulation_duration(original_contents)
...
def submit_new_job(cir_file_contents,timeout,progress_queue,temp_folder = 'temp_sim_files/'):
    ...
    cir_file_contents = modify_output_filename(cir_file_contents,output_file_name)
...
def prepare_and_run_new_job(original_contents,input_params,timeout,interpolation_timestep, progress_queue, temp_folder = 'temp_sim_files/'):
    new_contents = modify_cir_with_param_dict(original_contents, input_params)
    all_params = read_parameters_from_cir_contents(new_contents)
    voltage_sources = identify_voltage_sources(new_contents)
