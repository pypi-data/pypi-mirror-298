# from .simulation_runner import prepare_and_run_new_job
import subprocess
import time
import logging
import os
...
def worker_grid(args):
    ...
    try:
        result_df = prepare_and_run_new_job(original_contents, input_params, timeout, interpolation_timestep, progress_queue, worker_essentials.temp_folder)
    except Exception as e:
        logging.error(f"Error in prepare_and_run_new_job: {str(e)}")
...
def worker_basinhopping(args):
    ...
    def objective_function(x_norm):
        ...
        try:
            ...
            try:
                result_df = prepare_and_run_new_job(worker_essentials.original_contents, params, worker_essentials.timeout, worker_essentials.interpolation_timestep, progress_queue, worker_essentials.temp_folder)
            except Exception as e:
                logging.error(f"Error in prepare_and_run_new_job: {str(e)}")
                return pd.DataFrame(), np.inf
            if result_df is None or result_df.empty or result_df.isnull().any().any():
