...
import numpy as np
import pandas as pd
from .spice_utils import convert_spice_to_sci, read_parameters_from_cir, read_parameters_from_cir_contents, format_and_copy_parameters, extract_and_format_parameters, create_symbolic_link, remove_symbolic_link, modify_cir_file_with_parameters, modify_cir_with_param_dict, modify_output_filename, cleanup_temp_files, parse_variable_names, parse_outtran, identify_voltage_sources, prepare_cir_for_ngspice_input, add_circbyline_prefix, strip_spice_comments, modify_transient_simulation, get_value_at_time, get_values_in_time_range
...
def convert_spice_to_sci(value):
    ...
    match = re.match(r'(-?\d*\.?\d+)([afpnumkmegKtT]?)', value)
    if match:
        number, suffix = match.groups()
        if suffix.lower() in SPICE_TO_SCI_NOTATION:
            return float(f'{number}{SPICE_TO_SCI_NOTATION[suffix.lower()]}')
        return float(number)
    try:
        return float(value)
    except ValueError:
        raise ValueError(f"Invalid SPICE value: {value}")
