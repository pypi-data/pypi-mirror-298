...
# from .spice_utils import create_symbolic_link, remove_symbolic_link, modify_cir_file_with_parameters, modify_output_filename, read_parameters_from_cir, parse_outtran, cleanup_temp_files, read_simulation_duration, identify_voltage_sources, modify_cir_with_param_dict
...
def run_spicemanager(
    ...
    if(waveforms is None):
        ...
        waveforms = data_processing.replace_functional_waveforms_with_tvps(waveforms, interpolation_timestep)
...
def submit_new_job(cir_file_contents,timeout,progress_queue,temp_folder = 'temp_sim_files/'):
    ...
    if result:
        spice_df = spice_utils.parse_outtran(output_file_name, temp_folder)
        try:
            os.remove(temp_cir)
        except OSError:
            pass
...
def build_result_df(original_contents,spice_df,params,voltage_sources,interpolation_timestep):
    ...
    df = data_processing.interpolate_to_regular_timestep(spice_df,interpolation_timestep)
    df = data_processing.calculate_power_consumption(df,voltage_sources,params)
    df = data_processing.calculate_total_power_dissipation(df)
    

    result_df = pd.DataFrame({
        'cir_file_orig_contents': [original_contents],
        'total_energy': [data_processing.calculate_total_energy(df)],
        'spice_df': [df]
    })
