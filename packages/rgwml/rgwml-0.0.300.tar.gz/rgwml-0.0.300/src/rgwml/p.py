...
SELECT * FROM `project_id.dataset_id.your_table_name` ORDER BY your_date_column DESC LIMIT 100;
    ...
    def des(self):
        """INSPECT::[d.des()] Describe and calculate additional statistics."""
        if self.df is not None:
            ...
            sum_values = self.df[numeric_columns].sum()
            count = self.df[numeric_columns].count()
            ...
            for column in numeric_columns:
                column_diff = self.df[column].diff().dropna()  # Calculate differences and drop NaN
                ...
                rows = len(self.df[column].dropna())
                if rows >= 10:  # Ensure that the column has at least 10 non-NaN values
                    ...
                    interval_diffs = self.df[column].iloc[interval_indices].diff().dropna()
            ...
            # Prepare the sum statistics DataFrame with shorter column names
            sum_stats = pd.DataFrame({
                ...
                'acrl3i': avg_change_rate_last_3_10pct,  # Average Change Rate Over Last 3 10-Percentile Intervals
            ...
            print("  acrl3i: Average Change Rate Over Last 3 10-Percentile Intervals (mean of changes over last 3 intervals)")
