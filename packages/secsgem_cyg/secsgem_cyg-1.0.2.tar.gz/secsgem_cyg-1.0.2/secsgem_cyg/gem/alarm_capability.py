...
class AlarmCapability(GemHandler, Capability):
    ...
    def set_alarm(self, alid: int | str):
        ...
        if self.alarms[alid].enabled:
            self.send_and_waitfor_response(self.stream_function(5, 1)(
                {
                    "ALCD": self.alarms[alid].code | secsgem_cyg.secs.data_items.ALCD.ALARM_SET,
                    "ALID": alid,
    ...
    def _on_s05f03(self,
        ...
        result = secsgem_cyg.secs.data_items.ACKC5.ACCEPTED

        alid = function.ALID.get()
        if alid not in self._alarms:
            result = secsgem_cyg.secs.data_items.ACKC5.ERROR
        else:
            self.alarms[alid].enabled = function.ALED.get() == secsgem_cyg.secs.data_items.ALED.ENABLE
    ...
    def _on_s05f05(self,
        ...
        result = [{
            "ALCD": self.alarms[alid].code | (secsgem_cyg.secs.data_items.ALCD.ALARM_SET if self.alarms[alid].set else 0),
            "ALID": alid,
            "ALTX": self.alarms[alid].text,
        } for alid in alids]
