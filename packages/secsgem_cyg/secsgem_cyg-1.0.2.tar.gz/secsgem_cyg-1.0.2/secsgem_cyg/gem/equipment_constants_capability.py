...
class EquipmentConstantsCapability(GemHandler, Capability):
    ...
    def on_ec_value_request(self,
                            equipment_constant_id: secsgem_cyg.secs.variables.Base,
                            equipment_constant: EquipmentConstant) -> secsgem_cyg.secs.variables.Base:
    ...
    def _get_ec_value(self, equipment_constant: EquipmentConstant) -> secsgem_cyg.secs.variables.Base:
        """Get the equipment constant value depending on its configuation.
        ...
        if len(function) == 0:
            ...
            for equipment_constant_id in function:  # type: ignore[attr-defined]
                if equipment_constant_id not in self._equipment_constants:
                    responses.append(secsgem_cyg.secs.variables.Array(secsgem_cyg.secs.data_items.ECV, []))
                else:
