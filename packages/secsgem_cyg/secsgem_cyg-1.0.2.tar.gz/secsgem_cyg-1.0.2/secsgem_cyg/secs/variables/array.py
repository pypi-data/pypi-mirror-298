...
class Array(Base):
    ...
    def get_format(data_format, showname=False):
        ...
        if isinstance(data_format, list):
            return (
                f"{array_name}[\n"
                f"{secsgem_cyg.common.indent_block(List.get_format(data_format), 4)}\n"
                f"    ...\n]"
            )

        return (
            f"{array_name}[\n"
            f"{secsgem_cyg.common.indent_block(data_format.get_format(not showname), 4)}\n"
            f"    ...\n]"
        )
    ...
    def __repr__(self):
        ...
        for value in self.data:
            data += f"{secsgem_cyg.common.indent_block(value.__repr__())}\n"
