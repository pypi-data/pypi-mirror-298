...
class List(Base):
        ...
        if isinstance(data_format, list):
            ...
            for item in data_format:
                ...
                if isinstance(item, list):
                    if len(item) == 1:
                        items.append(secsgem_cyg.common.indent_block(Array.get_format(item[0], True), 4))
                    else:
                        items.append(secsgem_cyg.common.indent_block(List.get_format(item, True), 4))
                else:
                    items.append(secsgem_cyg.common.indent_block(item.get_format(), 4))
            return array_name + "{\n" + "\n".join(items) + "\n}"
        return None
    ...
    def __repr__(self):
        ...
        for field_name in self.data:
            data += f"{secsgem_cyg.common.indent_block(self.data[field_name].__repr__())}\n"
