...
class HsmsProtocol(secsgem_cyg.common.Protocol[HsmsMessage, HsmsBlock]):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, settings: HsmsSettings):
        ...
        Example:
            ...
            settings = secsgem_cyg.hsms.HsmsSettings(
                address="10.211.55.33",
                port=5000,
                active=secsgem_cyg.hsms.HsmsConnectMode.ACTIVE,
                name="test"
            )
            def onConnect(event, data):
                print ("Connected")

            client = secsgem_cyg.hsms.HsmsProtocol(settings)
