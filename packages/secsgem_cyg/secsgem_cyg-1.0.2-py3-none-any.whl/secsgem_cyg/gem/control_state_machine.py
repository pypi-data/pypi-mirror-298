...
class ControlStateMachine(secsgem_cyg.common.StateMachine):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        self.init = secsgem_cyg.common.State(
            ControlState.INIT,
            "INIT",
            initial=True)
        self.control = secsgem_cyg.common.State(
            ControlState.CONTROL,
            "CONTROL")
        self.offline = secsgem_cyg.common.State(
            ControlState.OFFLINE,
            "OFFLINE")
        self.equipment_offline = secsgem_cyg.common.State(
            ControlState.EQUIPMENT_OFFLINE,
            "EQUIPMENT_OFFLINE")
        self.attempt_online = secsgem_cyg.common.State(
            ControlState.ATTEMPT_ONLINE,
            "ATTEMPT_ONLINE")
        self.host_offline = secsgem_cyg.common.State(
            ControlState.HOST_OFFLINE,
            "HOST_OFFLINE")
        self.online = secsgem_cyg.common.State(
            ControlState.ONLINE,
            "ONLINE")
        self.online_local = secsgem_cyg.common.State(
            ControlState.ONLINE_LOCAL,
            "ONLINE_LOCAL")
        self.online_remote = secsgem_cyg.common.State(
            ControlState.ONLINE_REMOTE,
            "ONLINE_REMOTE")

        # transition 1
        self._current_state: secsgem_cyg.common.State = self.init
