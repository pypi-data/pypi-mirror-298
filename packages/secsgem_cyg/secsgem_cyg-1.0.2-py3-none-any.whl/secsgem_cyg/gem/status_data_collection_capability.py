...
class StatusDataCollectionCapability(GemHandler, Capability):
    ...
    def on_sv_value_request(self,
                            svid: secsgem_cyg.secs.variables.Base,
                            status_variable: StatusVariable) -> secsgem_cyg.secs.variables.Base:
    ...
    def _get_sv_value(self, status_variable: StatusVariable) -> secsgem_cyg.secs.variables.Base:
        """Get the status variable value depending on its configuation.
        ...
        if status_variable.svid == StatusVariableId.CLOCK.value:
            ...
            result = status_variable.value_type(secsgem_cyg.secs.data_items.SV, events)
        elif status_variable.svid == StatusVariableId.ALARMS_ENABLED.value:
            alarms = self._get_alarms_enabled()
            result = status_variable.value_type(secsgem_cyg.secs.data_items.SV, alarms)
    ...
    def _on_s01f03(self,
                   handler: secsgem_cyg.secs.SecsHandler,
                   message: secsgem_cyg.common.Message) -> secsgem_cyg.secs.SecsStreamFunction | None:
        ...
        if len(function) == 0:
            ...
            for status_variable_id in function:
                if status_variable_id not in self._status_variables:
                    responses.append(secsgem_cyg.secs.variables.Array(secsgem_cyg.secs.data_items.SV, []))
                else:
