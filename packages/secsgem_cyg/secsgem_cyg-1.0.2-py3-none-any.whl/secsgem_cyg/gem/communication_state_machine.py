...
class CommunicationStateMachine(secsgem_cyg.common.StateMachine):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, settings: secsgem_cyg.common.Settings) -> None:
        ...
        self.disabled = secsgem_cyg.common.State(
            CommunicationState.DISABLED,
            "DISABLED",
            initial=True)
        self.enabled = secsgem_cyg.common.State(
            CommunicationState.ENABLED,
            "ENABLED")
        self.not_communicating = secsgem_cyg.common.State(
            CommunicationState.NOT_COMMUNICATING,
            "NOT_COMMUNICATING",
            parent=self.enabled)
        self.host_initiated_connect = secsgem_cyg.common.State(
            CommunicationState.HOST_INITIATED_CONNECT,
            "HOST_INITIATED_CONNECT",
            parent=self.enabled)
        self.wait_cr_from_host = secsgem_cyg.common.State(
            CommunicationState.WAIT_CR_FROM_HOST,
            "WAIT_CR_FROM_HOST",
            parent=self.enabled)
        self.equipment_initiated_connect = secsgem_cyg.common.State(
            CommunicationState.EQUIPMENT_INITIATED_CONNECT,
            "EQUIPMENT_INITIATED_CONNECT",
            parent=self.enabled)
        self.wait_delay = secsgem_cyg.common.State(
            CommunicationState.WAIT_DELAY,
            "WAIT_DELAY",
            parent=self.enabled)
        self.wait_cra = secsgem_cyg.common.State(
            CommunicationState.WAIT_CRA,
            "WAIT_CRA",
            parent=self.enabled)
        self.communicating = secsgem_cyg.common.State(
            CommunicationState.COMMUNICATING,
            "COMMUNICATING",
            parent=self.enabled)

        # transition 1
        self._current_state: secsgem_cyg.common.State = self.disabled

        self._transitions: list[secsgem_cyg.common.Transition] = [
            secsgem_cyg.common.Transition("enable", self.disabled, self.not_communicating),  # 2 and 4
            secsgem_cyg.common.Transition("disable",
                                          [
                    self.enabled,
                    ...
                    self.wait_cr_from_host,
                                          ...
                                          self.disabled),  # 3
            secsgem_cyg.common.Transition("select", [self.not_communicating, self.communicating],
                                          self.wait_cra),  # 5
            secsgem_cyg.common.Transition("communicationreqfail", self.wait_cra, self.wait_delay),  # 6
            secsgem_cyg.common.Transition("delayexpired", self.wait_delay, self.wait_cra),  # 7
            secsgem_cyg.common.Transition("messagereceived", self.wait_delay, self.wait_cra),  # 8
            secsgem_cyg.common.Transition("s1f14received", self.wait_cra, self.communicating),  # 9
            secsgem_cyg.common.Transition("communicationfail", self.communicating, self.not_communicating),  # 14
            secsgem_cyg.common.Transition("s1f13received",
                                          [
                    self.wait_cr_from_host,
                    self.wait_delay,
                    self.wait_cra,
