...
class SamuraiSudokuSolver:
    ...
    def __constrain_sudoku(self, model, template) -> List[List[int]]:
        board_width = 9
        subboard_width = 3
        board = [
    ...
    def __link__sudokus(self, center_board: List[List[int]], left_top_to_right_bottom_boards: List[List[List[str]]]):


    def solve(self, templates: List[List[List[str]]]) -> List[List[List[int]]]:
        model = cp_model.CpModel()
        solver = cp_model.CpSolver()
        constrained_sudokus = [self.__constrain_sudoku(model, template) for template in templates]
