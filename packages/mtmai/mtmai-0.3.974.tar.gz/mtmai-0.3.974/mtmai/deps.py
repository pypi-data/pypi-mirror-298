...
from mtmai.core.config import HEADER_SITE_ID, settings
...
async def get_site(session: SessionDep, request: Request) -> Site:
    # 通过 http header 明确指定 siteId
    siteId = request.headers.get(HEADER_SITE_ID)
    if not siteId:
        # 通过 host 获取 siteId
        site = await get_site_domain(session, request.headers.get("Host"))
