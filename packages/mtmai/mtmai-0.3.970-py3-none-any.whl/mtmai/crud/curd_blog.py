...
async def update_post(*, session: AsyncSession, post_update: BlogPostUpdateReq) -> Post:
    ...
    # return blog_post
    return post_update.id  # 直接返回 id


async def get_post_content(*, session: AsyncSession, post_id: uuid.UUID):
    a = await session.exec(select(PostContent).where(Post.id == post_id))
    post_content = a.one()
    return post_content


async def save_post_content(*, session: AsyncSession, post_id: uuid.UUID, content: str):
    a = await session.exec(select(PostContent).where(PostContent.post_id == post_id))
    blog_post = a.one()
    if not blog_post:
