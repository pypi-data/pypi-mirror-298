    ...
    *, session: AsyncSession, blog_post_create: BlogPostCreateReq
    ...
    if input_data.tags:
        for tag_name in input_data.tags:
            ...
            result = await session.exec(select(Tag).where(Tag.name == tag_name).where(Tag.site_id == site_id))
...
async def update_post(*, session: AsyncSession, post_update: BlogPostUpdateRequest) -> Post:
    ...
    if post_update.slug and post_update.slug != blog_post.slug:
        a = await session.exec(select(Post).where(Post.slug == post_update.slug))
        existing_post = a.first()
        if existing_post:
    ...
    #     blog_post_content = Document(content=post_update.content)
    #     session.add(blog_post_content)
    #     await session.commit()
    #     await session.refresh(blog_post_content)
    #     blog_post.content_id = blog_post_content.id
    # else:
    #     blog_post_content.content = post_update.content
    await save_post_content(
        session=session, post_id=blog_post.id, content=post_update.content
    )


    post_tags = await get_tags_by_post(session,blog_post.id)
    existing_tag_names = set(tag.name for tag in post_tags)
    new_tag_names = set(post_update.tags)

    siteId=blog_post.site_id # 存储siteId的值，因为 delete_tags_by_post 导致blog_post变化（暂时不理解）
    # 删除多余的tags
    await delete_tags_by_post(session, post_update.id, existing_tag_names - new_tag_names)
    await add_tags_to_posts(session,siteId, post_update.id, (new_tag_names - existing_tag_names) )
...
async def save_post_content(*, session: AsyncSession, post_id: uuid.UUID, content: str):
    blog_post = await session.exec(select(PostContent).where(PostContent.post_id == post_id))
...
async def delete_tags_by_post(session: AsyncSession, post_id: uuid.UUID, tags_to_remove: set[str]):
...
async def add_tags_to_posts(session: AsyncSession, site_id:uuid.UUID, post_id: uuid.UUID, tags_to_add: set[str]):
