...
from .saveBuilder import SaveManager,mkDirPath
from abstract_it.specializations.responseContentParser import *
import aiohttp
import asyncio
import logging
import time
...
class ResponseManager:
    def __init__(self, prompt_mgr=None, api_mgr=None, title: str = None, directory: str = None, rate_limit_per_minute: int = 80000):
        ...
        self.response_directory = os.path.join(self.directory, 'response_data')
        os.makedirs(self.response_directory,exist_ok=True)
        self.raw_response_directory = os.path.join(self.directory,'response_data','raw_data')
        os.makedirs(self.raw_response_directory,exist_ok=True)
    ...
    def get_response(self):
        ...
        created = get_created(self.response) or time.time()
        self.current_title = title = get_title(self.response) or created or 'response'
        self.current_model = get_any_value(self.response,'model') or self.model or 'default'
        usage = get_any_value(self.response,"usage")
        self.total_tokens_list.append({"time":created,"tokens":usage})
        response_content = safe_json_loads(get_response_content(self.response))
        self.save_manager = SaveManager(data=self.response, title=self.current_title, directory=self.raw_response_directory, model=self.current_model)
        self.get_all_instruction(response_content)
        self.query_js["response"]=self.response
        self.save_manager = SaveManager(data=self.query_js, title=self.current_title, directory=self.response_directory, model=self.current_model)
