...
def update_text_with_responses(self)->None:
    ...
    output_keys = []
    self.logger.info(f"current_output = {self.current_output}")
    ...
    self.logger.info(f"last_model = {self.last_model}")
    self.last_api_response = self.last_response_content.get('api_response')
    self.logger.info(f"last_api_response = {self.last_api_response}")
