       ...
       if (n_structures != 0):
          ...
          for i in range(n_structures):
              ...
              if (mismatch_type == 0):
                 ...
                 F = np.dot(Lattice_D, Lattice_B_inv)
                 ...
                 if (n_Lattice == 3): 
                    ...
                    F = np.dot(Lattice_D, Lattice_C_inv)
              ...
              if (mismatch_type == 1):
                 ...
                 #---------------------------------------------
                 Lattice_A_inv = np.linalg.inv(Lattice_A)
                 F = np.dot(Lattice_A, Lattice_A_inv)
                 MStrain_A = 0.5 * (np.dot(F.T, F) - I)
                 for aa in range(2):
                     for bb in range(2):
                 ...
                 #--------------------------------------------------------
                 Lattice_B_inv = np.linalg.inv(Lattice_B)
                 F = np.dot(Lattice_A, Lattice_B_inv)
                 MStrain_B = 0.5 * (np.dot(F.T, F) - I)
                 for aa in range(2):
                     for bb in range(2):
                 ...
                 if (n_Lattice == 3): 
                    Lattice_C_inv = np.linalg.inv(Lattice_C)
                    F = np.dot(Lattice_A, Lattice_C_inv)
                    MStrain_C = 0.5 * (np.dot(F.T, F) - I)
                    for aa in range(2):
                        for bb in range(2):
