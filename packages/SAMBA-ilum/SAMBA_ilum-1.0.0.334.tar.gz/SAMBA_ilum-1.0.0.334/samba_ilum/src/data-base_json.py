...
if os.path.isfile('output/POSCAR'):
   ...
   if (VTemp[0] == 'SAMBA'):
      ...
      #--------------------
      if (n_materials > 1):
         rotate_materials = [];  mismatch_materials = [];  matrix_transf = [];  matrix_deform = []
         passo = n_materials +1
         ...
         if (n_materials > 1):
            for i in range(n_materials):
                ...
                passo += 4
                temp1 = str(VTemp[passo]).replace('_', ' ').split()
                matrix_deform.append([[float(temp1[0]), float(temp1[1])], [float(temp1[2]), float(temp1[3])]])
         #----------------------------------------------------------------------------------------------------
         passo += 1
         for i in range(n_materials):
...
for n in range(2):
    ...
    if (crit == 1):
       ...
       mismatch = '--';  transformation_matrix = '--';  deformation_matrix = '--';  rotate_angle = '--';
       ...
       dados0 = {
                ...
                "deformation_matrix": matrix_deform  if n_materials > 1 else None,
