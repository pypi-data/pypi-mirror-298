...
class API(BaseAPIClient):
    ...
    async def evaluate(self, request: EvaluateRequest) -> EvaluateResponse:
        ...
        rpm_limit = int(resp.response.headers.get("x-ratelimit-rpm-limit-requests") or 0)
        rpm_remaining = int(resp.response.headers.get("x-ratelimit-rpm-remaining-requests") or 1)
        monthly_limit = int(resp.response.headers.get("x-ratelimit-monthly-limit-requests") or 0)
        monthly_remaining = int(resp.response.headers.get("x-ratelimit-monthly-remaining-requests") or 1)
