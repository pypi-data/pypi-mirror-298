...
try:
    ...
    from extras import load_json, save_json, date, find_path
except:
    from .jin import fmt_det_lst, build_report
    from .extras import load_json, save_json, date, find_path
...
def build_initial(config: dict) -> dict:
    ...
    if "FILE_CONFIG" not in config["paths"]:
        config["paths"]["FILE_CONFIG"] = str(
            pathlib.Path(config["paths"]["CWD"]) / "glog.json"
        )
    ...
    if "DIR_OUTPUT" not in config["paths"]:
        config["paths"]["DIR_OUTPUT"] = str(
            pathlib.Path(config["paths"]["CWD"]) / "ch-logs"
        )
    ...
    if "FILE_LOG" not in config["paths"]:
        config["paths"]["FILE_LOG"] = str(
            pathlib.Path(config["paths"]["DIR_OUTPUT"]) / "log_store.json"
        )
    ...
    if "FILE_OUTPUT" not in config["paths"]:
        config["paths"]["FILE_OUTPUT"] = str(
            pathlib.Path(config["paths"]["DIR_OUTPUT"]) / "changelog.md"
        )
...
def main():
    ...
    data = start_up(pathlib.Path.cwd())
    ...
    if args.collect:
        ...
        try:
            target = pathlib.Path(data["paths"]["CWD"])
            # target.unlink()
