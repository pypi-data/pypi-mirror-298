...
from GENetLib.scaler_l2train import scaler_l2train
from GENetLib.scaler_mcp_l2train import scaler_mcp_l2train


'''Grid search for scaler_ge'''

pd.set_option('mode.chained_assignment', None)
def grid_scaler_ge(data, ytype, dim_G, dim_E, haveGE, num_hidden_layers, nodes_hidden_layer,
    ...
    for lr2 in Learning_Rate2:
        for l2 in L2:
            loss_train, loss_valid, index_tr, index_va, model = scaler_l2train(x_train, clinical_train, interaction_train, y_train,
    ...
    for l in L:
        for lrMCP in Learning_Rate1:
            loss_train, loss_valid, index_tr, index_va, MCPNet = scaler_mcp_l2train(x_train, clinical_train, interaction_train, y_train,
                                                                                    x_valid, clinical_valid, interaction_valid, y_valid,
                                                                                    In_Nodes, Interaction_Nodes, Clinical_Nodes, 
                                                                                    num_hidden_layers, nodes_hidden_layer, ytype, isfunc,
