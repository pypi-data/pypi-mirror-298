...
try:
    ...
    logger.error(
        f"Failed to import model module: {c.model_import_path}. Traceback: {tb}"
    )
    settings_dataclass = {k: str(v) for k, v in asdict(c).items()}
    raise Exception(
        f"Something failed when trying to import your model's code with your defined settings: Settings: {settings_dataclass}, CWD: {Path.cwd()}, Directory contents: {Path.cwd().rglob('*')}"
    ) from e
